...
public class RadiationMonitor : GameStateMachine<RadiationMonitor, RadiationMonitor.Instance>
{
  ...
  public static readonly StateMachine<RadiationMonitor, RadiationMonitor.Instance, IStateMachineTarget, object>.Parameter<float>.Callback COMPARE_GTE_NO_LONGER_DEADLY = (StateMachine<RadiationMonitor, RadiationMonitor.Instance, IStateMachineTarget, object>.Parameter<float>.Callback) ((smi, p) => (double) p < 900.0 * (double) smi.difficultySettingMod);
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.active.Update(new System.Action<RadiationMonitor.Instance, float>(RadiationMonitor.CheckRadiationLevel), UpdateRate.SIM_1000ms, false).DefaultState(this.active.idle);
    ...
    this.active.sick.deadly.ToggleAnims("anim_loco_radiation4_kanim", 4f).ToggleAnims("anim_idle_radiation4_kanim", 4f).ToggleExpression(Db.Get().Expressions.Radiation4).ParamTransition<float>((StateMachine<RadiationMonitor, RadiationMonitor.Instance, IStateMachineTarget, object>.Parameter<float>) this.radiationExposure, (GameStateMachine<RadiationMonitor, RadiationMonitor.Instance, IStateMachineTarget, object>.State) this.active.sick.extreme, RadiationMonitor.COMPARE_GTE_NO_LONGER_DEADLY).Enter((StateMachine<RadiationMonitor, RadiationMonitor.Instance, IStateMachineTarget, object>.State.Callback) (smi => smi.GetComponent<Health>().Incapacitate(GameTags.RadiationSicknessIncapacitation)));
  }
  ...
  {
    float num1 = Db.Get().Attributes.RadiationRecovery.Lookup(smi.gameObject).GetTotalValue() * dt;
    double num2 = (double) smi.master.gameObject.GetAmounts().Get(Db.Get().Amounts.RadiationBalance).ApplyDelta(num1);
    smi.master.Trigger(1556680150, (object) num1);
  }
  ...
  private static void CheckRadiationLevel(RadiationMonitor.Instance smi, float dt)
  {
    ...
    if (Grid.IsValidCell(cell))
    {
      ...
      if ((double) smi.sm.timeUntilNextExposureReact.Get(smi) <= 0.0 && !smi.HasTag(GameTags.InTransitTube) && RadiationMonitor.COMPARE_REACT(smi, p))
      {
        ...
        smi.master.gameObject.GetSMI<ReactionMonitor.Instance>().AddSelfEmoteReactable(smi.master.gameObject, (HashedString) "RadiationReact", radiationGlare, true, Db.Get().ChoreTypes.EmoteHighPriority);
      }
    }
    if ((double) smi.sm.timeUntilNextSickReact.Get(smi) <= 0.0 && smi.sm.isSick.Get(smi) && !smi.HasTag(GameTags.InTransitTube))
    {
      ...
      smi.master.gameObject.GetSMI<ReactionMonitor.Instance>().AddSelfEmoteReactable(smi.master.gameObject, (HashedString) "RadiationReact", radiationItch, true, Db.Get().ChoreTypes.RadiationPain);
    }
    ...
    GameStateMachine<RadiationMonitor, RadiationMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
    ...
    {
      return (float) (600.0 * ((double) Mathf.Max(0.0f, this.sm.radiationExposure.Get(this.smi) - 100f * this.difficultySettingMod) / 100.0));
    }
