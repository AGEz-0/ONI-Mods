...
public class DebugTool : DragTool
{
    ...
    switch (this.type)
    {
      ...
      case DebugTool.Type.FillReplaceSubstance:
        ...
        GameUtil.FloodFillConditional(cell, (Func<int, bool>) (check_cell =>
        {
          ...
          return flag;
        }), (ICollection<int>) GameUtil.FloodFillVisited.Value);
        break;
  ...
  {
    foreach (GameObject original in new List<GameObject>()
    {
      Grid.Objects[cell, 2],
      Grid.Objects[cell, 1],
      Grid.Objects[cell, 12],
  ...
  public void DoStoreSubstance(int cell)
  {
    ...
    if ((double) mass == -1.0)
      mass = element.defaultValues.mass;
    int diseaseCount = DebugPaintElementScreen.Instance.paintDiseaseCount.isOn ? DebugPaintElementScreen.Instance.diseaseCount : 0;
    if (element.IsGas)
      component.AddGasChunk(element.id, mass, temperature, disease_idx, diseaseCount, false);
    else if (element.IsLiquid)
    {
      component.AddLiquid(element.id, mass, temperature, disease_idx, diseaseCount);
    }
    else
    {
      if (!element.IsSolid)
        return;
      component.AddOre(element.id, mass, temperature, disease_idx, diseaseCount);
    }
  }
