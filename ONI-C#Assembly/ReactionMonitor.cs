  ...
  GameStateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.reacting.Enter("Reactable.Begin", (StateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.State.Callback) (smi => this.reactable.Get(smi).Begin(smi.gameObject))).Enter((StateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.State.Callback) (smi => smi.master.Trigger(-909573545))).Enter("Reactable.AddChorePreventionTag", (StateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.State.Callback) (smi =>
    {
      if (!this.reactable.Get(smi).preventChoreInterruption)
        return;
      smi.GetComponent<KPrefabID>().AddTag(GameTags.PreventChoreInterruption);
    })).Update("Reactable.Update", (System.Action<ReactionMonitor.Instance, float>) ((smi, dt) => this.reactable.Get(smi).Update(dt))).Exit((StateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.State.Callback) (smi => smi.master.Trigger(824899998))).Exit("Reactable.End", (StateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.State.Callback) (smi => this.reactable.Get(smi).End())).Exit("Reactable.RemoveChorePreventionTag", (StateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.State.Callback) (smi =>
    {
      if (!this.reactable.Get(smi).preventChoreInterruption)
        return;
    ...
    GameStateMachine<ReactionMonitor, ReactionMonitor.Instance, IStateMachineTarget, ReactionMonitor.Def>.GameInstance
  {
    ...
    {
      return (UnityEngine.Object) this.animController != (UnityEngine.Object) null && e.IsValidForController(this.animController);
    }
    ...
    public void PollForReactables(Navigator.ActiveTransition transition)
    {
      ...
      {
        if (this.TryReact(gathered_entries[index].obj as Reactable, time, transition))
        {
          num = time;
          break;
        }
      ...
      List<Reactable.ReactablePrecondition> emotePreconditions = null)
    {
      ...
      return this.AddSelfEmoteReactable(target, (HashedString) reactionId, emote, isOneShot, choreType, globalCooldown, localCooldown, maxTriggerTime, maxInitialDelay, emotePreconditions);
    }
