...
public class TouristModule : StateMachineComponent<TouristModule.StatesInstance>
{
  ...
  protected override void OnSpawn()
  {
    ...
    this.smi.StartSM();
    this.partitionerEntry = GameScenePartitioner.Instance.Add("TouristModule.gantryChanged", (object) this.gameObject, Grid.PosToCell(this.gameObject), GameScenePartitioner.Instance.validNavCellChangedLayer, new Action<object>(this.OnGantryChanged));
  ...
  {
    if (data != null || !this.gameObject.HasTag(GameTags.RocketOnGround) || this.GetComponent<MinionStorage>().GetStoredMinionInfo().Count <= 0)
      return;
    ...
    GameStateMachine<TouristModule.States, TouristModule.StatesInstance, TouristModule, object>.GameInstance
  {
    public StatesInstance(TouristModule smi)
      : base(smi)
    {
      smi.gameObject.Subscribe(-887025858, (Action<object>) (data =>
      {
    ...
    GameStateMachine<TouristModule.States, TouristModule.StatesInstance, TouristModule>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.hasTourist.PlayAnim("grounded", KAnim.PlayMode.Loop).EventTransition(GameHashes.RocketLanded, this.idle).EventTransition(GameHashes.AssigneeChanged, this.idle);
    }
  }
}
