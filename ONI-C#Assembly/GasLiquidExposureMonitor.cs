  ...
  GameStateMachine<GasLiquidExposureMonitor, GasLiquidExposureMonitor.Instance, IStateMachineTarget, GasLiquidExposureMonitor.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.root.Update(new System.Action<GasLiquidExposureMonitor.Instance, float>(this.UpdateExposure), UpdateRate.SIM_33ms);
  ...
  {
    return (double) GameClock.Instance.GetTime() > (double) smi.lastReactTime + 60.0;
  }
  ...
  private void UpdateExposure(GasLiquidExposureMonitor.Instance smi, float dt)
  {
    ...
    if (Grid.IsValidCell(index))
    {
      ...
      if (smi.master.gameObject.HasTag(GameTags.HasSuitTank) && (bool) (UnityEngine.Object) smi.gameObject.GetComponent<SuitEquipper>().IsWearingAirtightSuit() || smi.master.gameObject.HasTag(GameTags.InTransitTube))
      {
    ...
    GameStateMachine<GasLiquidExposureMonitor, GasLiquidExposureMonitor.Instance, IStateMachineTarget, GasLiquidExposureMonitor.Def>.GameInstance
  {
    ...
    public Reactable GetReactable()
    {
      ...
      SelfEmoteReactable reactable = new SelfEmoteReactable(this.master.gameObject, (HashedString) "IrritatedEyes", Db.Get().ChoreTypes.Cough, localCooldown: 0.0f);
      reactable.SetEmote(iritatedEyes);
      reactable.preventChoreInterruption = true;
      reactable.RegisterEmoteStepCallbacks((HashedString) "irritated_eyes", (System.Action<GameObject>) null, (System.Action<GameObject>) (go => this.sm.reactFinished.Trigger(this)));
      return (Reactable) reactable;
    }

    public bool IsMinorIrritation()
    {
      return (double) this.exposure >= 8.0 && (double) this.exposure < 15.0;
    }
