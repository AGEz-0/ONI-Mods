...
public class Geyser : StateMachineComponent<Geyser.StatesInstance>, IGameObjectEffectDescriptor
{
  ...
  public void ShiftTimeTo(Geyser.TimeShiftStep step, bool shouldSurviveSaveLoad = false)
  {
    ...
    switch (step)
    {
      case Geyser.TimeShiftStep.ActiveState:
        this.AlterTime(this.timeShift - ((double) num3 > 0.0 ? this.configuration.GetYearOnDuration() - num3 : yearLength - num4), shouldSurviveSaveLoad);
        break;
      case Geyser.TimeShiftStep.DormantState:
        this.AlterTime(this.timeShift + ((double) num3 > 0.0 ? num3 : (float) -((double) this.configuration.GetYearOffDuration() - (double) num4)), shouldSurviveSaveLoad);
        break;
      case Geyser.TimeShiftStep.NextIteration:
        this.AlterTime(this.timeShift + ((double) num1 > 0.0 ? num1 + this.configuration.GetOffDuration() : num2), shouldSurviveSaveLoad);
        break;
      case Geyser.TimeShiftStep.PreviousIteration:
        float f = (double) num1 > 0.0 ? (float) -((double) this.configuration.GetOnDuration() - (double) num1) : (float) -((double) this.configuration.GetIterationLength() - (double) num2);
        if ((double) num1 > 0.0 && (double) Mathf.Abs(f) < (double) this.configuration.GetOnDuration() * 0.05000000074505806)
          f -= this.configuration.GetIterationLength();
        this.AlterTime(this.timeShift + f, shouldSurviveSaveLoad);
        break;
    }
  }
  ...
  private void RefreshGeotunerStatusItem()
  {
    ...
    if (this.GetAmountOfGeotunersPointingThisGeyser() > 0)
      ...
      component.RemoveStatusItem(Db.Get().BuildingStatusItems.GeyserGeotuned, (bool) (UnityEngine.Object) this);
  }
  ...
  private void RefreshStudiedMeter()
  {
    ...
    this.studyable.studiedIndicator.meterController.Play((HashedString) trackerMeterAnimNames.ToString(), KAnim.PlayMode.Loop);
  }

  public int GetAmountOfGeotunersPointingThisGeyser()
  {
    return Components.GeoTuners.GetItems(this.gameObject.GetMyWorldId()).Count<GeoTuner.Instance>((Func<GeoTuner.Instance, bool>) (x => (UnityEngine.Object) x.GetAssignedGeyser() == (UnityEngine.Object) this));
  }

  public int GetAmountOfGeotunersPointingOrWillPointAtThisGeyser()
  {
    return Components.GeoTuners.GetItems(this.gameObject.GetMyWorldId()).Count<GeoTuner.Instance>((Func<GeoTuner.Instance, bool>) (x => (UnityEngine.Object) x.GetAssignedGeyser() == (UnityEngine.Object) this || (UnityEngine.Object) x.GetFutureGeyser() == (UnityEngine.Object) this));
  }
  ...
  protected override void OnSpawn()
  {
    ...
    else if ((double) this.configuration.geyserType.geyserTemperature - (double) this.gameObject.GetComponent<PrimaryElement>().Temperature != 0.0)
      this.gameObject.GetComponent<SimTemperatureTransfer>().onSimRegistered += new Action<SimTemperatureTransfer>(this.OnSimRegistered);
    this.ApplyConfigurationEmissionValues(this.configuration);
    this.GenerateName();
    ...
    Components.Geysers.Add(this.gameObject.GetMyWorldId(), this);
    this.gameObject.Subscribe(1763323737, new Action<object>(this.OnGeotunerChanged));
    this.RefreshStudiedMeter();
    this.UpdateModifier();
  }
  ...
  {
    if (!(this.nameable.savedName == (string) Strings.Get(new StringKey($"STRINGS.CREATURES.SPECIES.GEYSER.{this.configuration.geyserType.id.ToUpper()}.NAME"))))
      return;
    int cell = Grid.PosToCell(this.gameObject);
    Quadrant[] quadrantOfCell = this.gameObject.GetMyWorld().GetQuadrantOfCell(cell, 2);
    ...
    string str4 = strArray1[UnityEngine.Random.Range(0, strArray1.Length)];
    string[] strArray2 = new string[6]
    {
      UI.StripLinkFormatting(this.gameObject.GetProperName()),
      " ",
      str4,
      str3,
      "‑",
  ...
  public void Unentomb()
  {
    ...
    foreach (CellOffset occupiedCellsOffset in component.OccupiedCellsOffsets)
    {
      ...
      if (Grid.IsSolidCell(index) && Grid.Element[index].id != SimHashes.Unobtanium)
  ...
  {
    base.OnCleanUp();
    this.gameObject.Unsubscribe(1763323737, new Action<object>(this.OnGeotunerChanged));
    Components.Geysers.Remove(this.gameObject.GetMyWorldId(), this);
  }
  ...
  public List<Descriptor> GetDescriptors(GameObject go)
  {
    ...
    List<GeoTuner.Instance> items = Components.GeoTuners.GetItems(this.gameObject.GetMyWorldId());
    GeoTuner.Instance instance = items.Find((Predicate<GeoTuner.Instance>) (g => (UnityEngine.Object) g.GetAssignedGeyser() == (UnityEngine.Object) this));
    int num1 = items.Count<GeoTuner.Instance>((Func<GeoTuner.Instance, bool>) (x => (UnityEngine.Object) x.GetAssignedGeyser() == (UnityEngine.Object) this));
    bool flag = num1 > 0;
    ...
    if (flag)
    {
      ...
      string str2 = ((double) temp2 > 0.0 ? "+" : "") + GameUtil.GetFormattedTemperature(temp2, interpretation: GameUtil.TemperatureInterpretation.Relative);
      string str3 = ((double) mass2 > 0.0 ? "+" : "") + GameUtil.GetFormattedMass(mass2, GameUtil.TimeSlice.PerSecond, floatFormat: "{0:0.##}");
      string str4 = ((double) temp1 > 0.0 ? "+" : "") + GameUtil.GetFormattedTemperature(temp1, interpretation: GameUtil.TemperatureInterpretation.Relative);
      string str5 = ((double) mass1 > 0.0 ? "+" : "") + GameUtil.GetFormattedMass(mass1, GameUtil.TimeSlice.PerSecond, floatFormat: "{0:0.##}");
      tooltip1 = $"{$"{string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_PRODUCTION_GEOTUNED, (object) ElementLoader.FindElementByHash(this.configuration.GetElement()).name, (object) GameUtil.GetFormattedMass(this.configuration.GetEmitRate(), GameUtil.TimeSlice.PerSecond), (object) GameUtil.GetFormattedTemperature(this.configuration.GetTemperature())) + "\n"}\n{string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_PRODUCTION_GEOTUNED_COUNT, (object) affectingThisGeyser.ToString(), (object) num1.ToString())}" + "\n"}\n{string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_PRODUCTION_GEOTUNED_TOTAL, (object) str3, (object) str2)}";
      for (int index = 0; index < affectingThisGeyser; ++index)
      {
        string str6 = $"{"\n    • " + UI.UISIDESCREENS.GEOTUNERSIDESCREEN.STUDIED_TOOLTIP_GEOTUNER_MODIFIER_ROW_TITLE.ToString()}{str5} {str4}";
        tooltip1 += str6;
      }
    }
    descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_PRODUCTION, (object) str1, (object) GameUtil.GetFormattedMass(this.configuration.GetEmitRate(), GameUtil.TimeSlice.PerSecond), (object) GameUtil.GetFormattedTemperature(this.configuration.GetTemperature())), tooltip1));
    if (this.configuration.GetDiseaseIdx() != byte.MaxValue)
      descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_DISEASE, (object) GameUtil.GetFormattedDiseaseName(this.configuration.GetDiseaseIdx())), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_DISEASE, (object) GameUtil.GetFormattedDiseaseName(this.configuration.GetDiseaseIdx()))));
    ...
    {
      descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_YEAR_UNSTUDIED), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_YEAR_UNSTUDIED)));
      descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_YEAR_AVR_OUTPUT_UNSTUDIED), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_YEAR_AVR_OUTPUT_UNSTUDIED)));
    }
    else
    {
      ...
      if (this.smi.IsInsideState((StateMachine.BaseState) this.smi.sm.dormant))
        descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_YEAR_NEXT_ACTIVE, (object) GameUtil.GetFormattedCycles(this.RemainingDormantTime())), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_YEAR_NEXT_ACTIVE, (object) GameUtil.GetFormattedCycles(this.RemainingDormantTime()))));
      else
        descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_YEAR_NEXT_DORMANT, (object) GameUtil.GetFormattedCycles(this.RemainingActiveTime())), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_YEAR_NEXT_DORMANT, (object) GameUtil.GetFormattedCycles(this.RemainingActiveTime()))));
      string tooltip2 = UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_YEAR_AVR_OUTPUT.Replace("{average}", GameUtil.GetFormattedMass(this.configuration.GetAverageEmission(), GameUtil.TimeSlice.PerSecond)).Replace("{element}", this.configuration.geyserType.element.CreateTag().ProperName());
      if (flag)
      {
        string str7 = $"{tooltip2 + "\n"}\n{(string) UI.BUILDINGEFFECTS.TOOLTIPS.GEYSER_YEAR_AVR_OUTPUT_BREAKDOWN_TITLE}";
        int affectingThisGeyser = this.GetAmountOfGeotunersAffectingThisGeyser();
        ...
        tooltip2 = $"{str7}{GameUtil.AddPositiveSign(num5.ToString("0.0"), (double) num5 > 0.0)}%";
        for (int index = 0; index < affectingThisGeyser; ++index)
        {
      ...
      descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.GEYSER_YEAR_AVR_OUTPUT, (object) GameUtil.GetFormattedMass(this.configuration.GetAverageEmission(), GameUtil.TimeSlice.PerSecond)), tooltip2));
    }
    return descriptors;
  }
  ...
  public class StatesInstance(Geyser smi) : 
    GameStateMachine<Geyser.States, Geyser.StatesInstance, Geyser, object>.GameInstance(smi)
  {
  }
