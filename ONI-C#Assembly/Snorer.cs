...
public class Snorer : StateMachineComponent<Snorer.StatesInstance>
{
  ...
  public class StatesInstance(Snorer master) : 
    GameStateMachine<Snorer.States, Snorer.StatesInstance, Snorer, object>.GameInstance(master)
  {
    ...
    {
      this.snoreHandle = GameScheduler.Instance.Schedule("snorelines", 2f, new Action<object>(this.StartSmallSnoreInternal), (object) null, (SchedulerGroup) null);
    }
    ...
    private void StartSmallSnoreInternal(object data)
    {
      ...
      if (!symbolVisible)
        return;
      this.snoreEffect = FXHelpers.CreateEffect("snore_fx_kanim", (Vector3) symbolTransform.GetColumn(3) with
      {
