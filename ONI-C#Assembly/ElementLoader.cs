...
public class ElementLoader
{
  ...
  public static List<ElementLoader.ElementEntry> CollectElementsFromYAML()
  {
    ...
    FileSystem.GetFiles(FileSystem.Normalize(ElementLoader.path), "*.yaml", (ICollection<FileHandle>) result);
    ...
    foreach (FileHandle fileHandle in (List<FileHandle>) result)
    {
      ...
      {
        ElementLoader.ElementEntryCollection elementEntryCollection = YamlIO.LoadFile<ElementLoader.ElementEntryCollection>(fileHandle.full_path, (YamlIO.ErrorHandler) ((error, force_log_as_warning) => errors.Add(error)));
