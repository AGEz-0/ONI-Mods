...
public class LoopingSoundManager : KMonoBehaviour, IRenderEveryTick
{
    ...
    float value)
  {
    ...
    if (data.IsPlaying)
    {
      int num = (int) data.ev.setParameterByID(this.GetSoundDescription(data.path).GetParameterId(parameter), value);
    }
  ...
  public void RenderEveryTick(float dt)
  {
    ...
    foreach (int index in (List<int>) pooledList2)
    {
      ...
      {
        int num5 = (int) sound1.ev.setParameterByID(soundDescription.GetParameterId(sound1.firstParameter), sound1.firstParameterValue);
      }
      if (sound1.secondParameter != HashedString.Invalid)
      {
        int num6 = (int) sound1.ev.setParameterByID(soundDescription.GetParameterId(sound1.secondParameter), sound1.secondParameterValue);
      }
    ...
    foreach (int index in (List<int>) pooledList3)
    {
      ...
      {
        int num7 = (int) sound3.ev.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
      }
      else
      {
        int num8 = (int) sound3.ev.stop(FMOD.Studio.STOP_MODE.ALLOWFADEOUT);
      }
      sound3.flags &= ~LoopingSoundManager.Sound.Flags.PLAYING;
      int num9 = (int) sound3.ev.release();
    ...
    {
      ATTRIBUTES_3D attributes = SoundEvent.GetCameraScaledPosition(sound.pos, sound.objectIsSelectedAndVisible).To3DAttributes() with
      {
        velocity = ((Vector3) (sound.velocity * velocityScale)).ToFMODVector()
      };
  ...
  public void StopAllSounds()
  {
    foreach (LoopingSoundManager.Sound data in this.sounds.GetDataList())
    {
      if (data.IsPlaying)
      {
        int num1 = (int) data.ev.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
  ...
  public static void StopSound(HandleVector<int>.Handle handle)
  {
    ...
    if (data.IsPlaying)
    {
      int num1 = (int) data.ev.stop(LoopingSoundManager.Get().GameIsPaused ? FMOD.Studio.STOP_MODE.IMMEDIATE : FMOD.Studio.STOP_MODE.ALLOWFADEOUT);
      int num2 = (int) data.ev.release();
  ...
  public static void PauseSound(HandleVector<int>.Handle handle, bool paused)
  {
    ...
    if (!data.IsPlaying)
      return;
    int num = (int) data.ev.setPaused(paused);
  }
  ...
  public struct Sound
  {
    ...
    {
      get => (this.flags & LoopingSoundManager.Sound.Flags.PAUSE_ON_GAME_PAUSED) != 0;
    }

    public bool IsCullingEnabled
    {
      get => (this.flags & LoopingSoundManager.Sound.Flags.ENABLE_CULLING) != 0;
    }

    public bool ShouldCameraScalePosition
    {
      get => (this.flags & LoopingSoundManager.Sound.Flags.ENABLE_CAMERA_SCALED_POSITION) != 0;
    }
