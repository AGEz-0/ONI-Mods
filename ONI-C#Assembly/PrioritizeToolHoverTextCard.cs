...
public class PrioritizeToolHoverTextCard : HoverTextConfiguration
{
  ...
  protected override void ConfigureTitle(HoverTextScreen screen)
  {
    ...
    if (lastEnabledFilter == null || !(lastEnabledFilter != "ALL"))
      return;
    this.ToolName = string.Format((string) UI.TOOLS.CAPITALS, (object) ((string) Strings.Get(this.ToolNameStringKey) + string.Format((string) UI.TOOLS.FILTER_HOVERCARD_HEADER, (object) Strings.Get($"STRINGS.UI.TOOLS.FILTERLAYERS.{lastEnabledFilter}.TOOLTIP"))));
  }
}
