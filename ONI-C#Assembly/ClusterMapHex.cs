...
public class ClusterMapHex : MultiToggle, ICanvasRaycastFilter
{
    ...
    bool repeat)
  {
    ...
    if (pathLength > 0)
    {
      ...
      if (repeat)
        pathLength *= 2;
      this.m_tooltip.AddMultiStringTooltip(string.Format(format, (object) pathLength, (object) GameUtil.GetFormattedRocketRange(rocketRange)), this.informationTooltipStyle);
    }
    this.UpdateHoverColors(string.IsNullOrEmpty(fail_reason));
    if (string.IsNullOrEmpty(fail_reason))
      return;
  ...
  private void TrySelect()
  {
    if (DebugHandler.InstantBuildMode)
      SaveGame.Instance.GetSMI<ClusterFogOfWarManager.Instance>().RevealLocation(this.location);
    ClusterMapScreen.Instance.SelectHex(this);
  }

  private bool TryGoTo()
  {
    List<WorldContainer> list = ClusterGrid.Instance.GetVisibleEntitiesAtCell(this.location).Select<ClusterGridEntity, WorldContainer>((Func<ClusterGridEntity, WorldContainer>) (entity => entity.GetComponent<WorldContainer>())).Where<WorldContainer>((Func<WorldContainer, bool>) (x => (UnityEngine.Object) x != (UnityEngine.Object) null)).ToList<WorldContainer>();
    if (list.Count != 1)
      return false;
  ...
  {
    Vector2 position = (Vector2) this.rectTransform.position;
    float num1 = Mathf.Abs(inputPoint.x - position.x);
    float num2 = Mathf.Abs(inputPoint.y - position.y);
    Vector2 lossyScale = (Vector2) this.rectTransform.lossyScale;
    return (double) num1 <= (double) lossyScale.x && (double) num2 <= (double) lossyScale.y && (double) lossyScale.y * (double) lossyScale.x - (double) lossyScale.y / 2.0 * (double) num1 - (double) lossyScale.x * (double) num2 >= 0.0;
  }
