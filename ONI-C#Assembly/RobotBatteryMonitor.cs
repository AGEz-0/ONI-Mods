  ...
  GameStateMachine<RobotBatteryMonitor, RobotBatteryMonitor.Instance, IStateMachineTarget, RobotBatteryMonitor.Def>
{
  ...
  {
    return (double) smi.amountInstance.value <= 0.0 || GameClock.Instance.IsNighttime();
  }

  public static bool ChargeDecent(RobotBatteryMonitor.Instance smi)
  {
    return (double) smi.amountInstance.value >= (double) smi.amountInstance.GetMax() * (double) smi.def.lowBatteryWarningPercent;
  }

  public static bool ChargeFull(RobotBatteryMonitor.Instance smi)
  {
    return (double) smi.amountInstance.value >= (double) smi.amountInstance.GetMax();
  }

  public static bool ChargeComplete(RobotBatteryMonitor.Instance smi)
  {
    return (double) smi.amountInstance.value >= (double) smi.amountInstance.GetMax() && !GameClock.Instance.IsNighttime();
  }

  public static bool BatteryDead(RobotBatteryMonitor.Instance smi)
  {
    return !smi.def.canCharge && (double) smi.amountInstance.value == 0.0;
  }
