...
public class LineLayer : GraphLayer
{
  ...
  private void InitAreaTexture()
  {
    ...
    this.areaFill.sprite = Sprite.Create(this.areaTexture, new UnityEngine.Rect(0.0f, 0.0f, (float) this.areaTexture.width, (float) this.areaTexture.height), new Vector2(0.5f, 0.5f), 100f);
  }
  ...
  public virtual GraphedLine NewLine(Tuple<float, float>[] points, string ID = "")
  {
    ...
    if (this.fillAreaUnderLine)
    {
      ...
      for (int x = 0; x < this.areaTexture.width; ++x)
      {
        float num1 = min.x + vector2_1.x * ((float) x / (float) this.areaTexture.width);
        if (points.Length > 1)
        {
          int index1 = 1;
          for (int index2 = 1; index2 < points.Length; ++index2)
          {
            if ((double) points[index2].first >= (double) num1)
            {
              index1 = index2;
              break;
            }
          }
          Vector2 vector2_2 = new Vector2(points[index1].first - points[index1 - 1].first, points[index1].second - points[index1 - 1].second);
          float num2 = (num1 - points[index1 - 1].first) / vector2_2.x;
          bool flag = false;
          int num3 = -1;
          for (int y = this.areaTexture.height - 1; y >= 0; --y)
          {
            if (!flag && (double) min.y + (double) vector2_1.y * ((double) y / (double) this.areaTexture.height) < (double) points[index1 - 1].second + (double) vector2_2.y * (double) num2)
            {
              flag = true;
              num3 = y;
            }
            Color color = flag ? new Color(1f, 1f, 1f, Mathf.Lerp(1f, this.fillAlphaMin, Mathf.Clamp((float) (num3 - y) / this.fillFadePixels, 0.0f, 1f))) : Color.clear;
            this.areaTexture.SetPixel(x, y, color);
          }
        }
      }
  ...
  private void FillArea(Tuple<float, float>[] points)
  {
    ...
    for (int x = 0; x < this.areaTexture.width; ++x)
    {
      ...
      if (points.Length > 1)
      {
        ...
        vector2_2.x = points[index1].first - points[index1 - 1].first;
        vector2_2.y = points[index1].second - points[index1 - 1].second;
        float num2 = (num1 - points[index1 - 1].first) / vector2_2.x;
        bool flag = false;
        int num3 = -1;
        for (int y = this.areaTexture.height - 1; y >= 0; --y)
