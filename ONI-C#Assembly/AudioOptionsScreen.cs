...
public class AudioOptionsScreen : KModalScreen
{
  ...
  private void CheckMasterValue(float value) => this.jambell.enabled = (double) value == 0.0;
  ...
  private void BuildAudioDeviceList()
  {
    ...
    int numdrivers;
    int numDrivers = (int) RuntimeManager.CoreSystem.getNumDrivers(out numdrivers);
    for (int id = 0; id < numdrivers; ++id)
    {
      KFMOD.AudioDevice audioDevice = new KFMOD.AudioDevice();
      string name;
      int driverInfo = (int) RuntimeManager.CoreSystem.getDriverInfo(id, out name, 64 /*0x40*/, out audioDevice.guid, out audioDevice.systemRate, out audioDevice.speakerMode, out audioDevice.speakerModeChannels);
  ...
  {
    int num = (int) RuntimeManager.CoreSystem.setDriver(idx);
