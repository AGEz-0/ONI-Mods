...
public class ColonyDiagnosticScreen : KScreen, ISim1000ms
{
  ...
  protected override void OnSpawn()
  {
    ...
    this.seeAllButton.onClick += (System.Action) (() =>
    {
  ...
  public void RefreshAll()
  {
    foreach (ColonyDiagnosticScreen.DiagnosticRow diagnosticRow in this.diagnosticRows)
    {
      ...
      {
        int num = (int) this.UpdateDiagnosticRow(diagnosticRow);
      }
    }
    ...
    this.seeAllButton.GetComponentInChildren<LocText>().SetText(string.Format((string) STRINGS.UI.DIAGNOSTICS_SCREEN.SEE_ALL, (object) AllDiagnosticsScreen.Instance.GetRowCount()));
  }
  ...
  private class DiagnosticRow : ISim4000ms
  {
    ...
    public DiagnosticRow(int worldID, GameObject gameObject, ColonyDiagnostic diagnostic)
    {
      ...
      {
        KSelectable selectable = (KSelectable) null;
        Vector3 pos = Vector3.zero;
        if (diagnostic.LatestResult.clickThroughTarget != null)
        {
          pos = diagnostic.LatestResult.clickThroughTarget.first;
          selectable = (UnityEngine.Object) diagnostic.LatestResult.clickThroughTarget.second == (UnityEngine.Object) null ? (KSelectable) null : diagnostic.LatestResult.clickThroughTarget.second.GetComponent<KSelectable>();
        }
        else
        {
          GameObject clickThroughObject = diagnostic.GetNextClickThroughObject();
          if ((UnityEngine.Object) clickThroughObject != (UnityEngine.Object) null)
          {
            selectable = clickThroughObject.GetComponent<KSelectable>();
            pos = clickThroughObject.transform.GetPosition();
          }
        }
        if ((UnityEngine.Object) selectable == (UnityEngine.Object) null)
          CameraController.Instance.ActiveWorldStarWipe(diagnostic.worldID);
        else
          SelectTool.Instance.SelectAndFocus(pos, selectable);
      });
    ...
    public void Update(bool force = false)
    {
      ...
      Debug.Assert(this.diagnostic.LatestResult.opinion != 0, (object) $"{this.diagnostic} criteria returned no opinion. Make sure the DiagnosticResult parameters are used or an opinion result is otherwise set in all of its criteria");
      ...
      this.tooltip.SetSimpleTooltip($"{(this.diagnostic.LatestResult.Message.IsNullOrWhiteSpace() ? STRINGS.UI.COLONY_DIAGNOSTICS.GENERIC_STATUS_NORMAL.text : this.diagnostic.LatestResult.Message)}\n\n{STRINGS.UI.COLONY_DIAGNOSTICS.MUTE_TUTORIAL.text}");
      ...
      if (!string.IsNullOrEmpty(this.diagnostic.icon))
        this.image.sprite = Assets.GetSprite((HashedString) this.diagnostic.icon);
    ...
    {
      return (double) this.timeOfLastNotification == 0.0 || (double) GameClock.Instance.GetTime() >= (double) this.timeOfLastNotification + 300.0;
    }
    ...
    private IEnumerator VisualNotificationRoutine()
    {
      ...
      {
        indicator.sizeDelta = this.defaultIndicatorSizeDelta + Vector2.one * (float) Mathf.RoundToInt(Mathf.Sin((float) (6.0 * (3.1415927410125732 * ((double) i / (double) bounceDuration)))));
    ...
    public void ResolveNotificationRoutine()
    {
      ...
      this.gameObject.GetComponent<HierarchyReferences>().GetReference<RectTransform>("Content").localPosition = (Vector3) Vector2.zero;
