...
public class GeneShuffler : Workable
{
  ...
  public bool WorkComplete
  {
    get
    {
      return this.geneShufflerSMI.IsInsideState((StateMachine.BaseState) this.geneShufflerSMI.sm.working.complete);
    }
  }
  ...
  public bool IsWorking
  {
    get
    {
      return this.geneShufflerSMI.IsInsideState((StateMachine.BaseState) this.geneShufflerSMI.sm.working);
    }
  }
  ...
  private void OnStorageChange(object data)
  {
    ...
    if (this.IsConsumed)
    {
      for (int index = this.storage.items.Count - 1; index >= 0; --index)
      {
        ...
        if (!((UnityEngine.Object) gameObject == (UnityEngine.Object) null) && gameObject.IsPrefabID(GeneShuffler.RechargeTag))
        {
  ...
  protected override bool OnWorkTick(WorkerBase worker, float dt) => base.OnWorkTick(worker, dt);
  ...
  private void ApplyRandomTrait(WorkerBase worker)
  {
    ...
    if (stringList.Count > 0)
    {
      ...
      string text = string.Format((string) UI.GENESHUFFLERMESSAGE.BODY_SUCCESS, (object) worker.GetProperName(), (object) trait.GetName(), (object) trait.GetTooltip());
  ...
  private void ActivateChore()
  {
    ...
    this.chore = (Chore) new WorkChore<Workable>(Db.Get().ChoreTypes.GeneShuffle, (IStateMachineTarget) this, on_complete: (Action<Chore>) (o => this.CompleteChore()), override_anims: Assets.GetAnim((HashedString) "anim_interacts_neuralvacillator_kanim"), allow_prioritization: false, priority_class: PriorityScreen.PriorityClass.high);
  }
    ...
    GameStateMachine<GeneShuffler.GeneShufflerSM, GeneShuffler.GeneShufflerSM.Instance, GeneShuffler>
  {
    ...
    public new class Instance(GeneShuffler master) : 
      GameStateMachine<GeneShuffler.GeneShufflerSM, GeneShuffler.GeneShufflerSM.Instance, GeneShuffler, object>.GameInstance(master)
    {
    }
  }
}
