...
public class ConduitFlow : IConduitFlow
{
    ...
    ConduitFlow.FlowDirections current)
  {
    switch (current)
    {
      ...
      default:
        Debug.Assert(false, (object) "multiple bits are set in 'FlowDirections'...can't compute next direction");
  ...
  public ConduitFlow.ConduitContents GetContents(int cell)
  {
    ...
    if ((double) contents.mass > 0.0 && (double) contents.temperature <= 0.0)
      Debug.LogError((object) $"unexpected temperature {contents.temperature}");
    return contents;
  }
  ...
  private float ComputeMovableMass(ConduitFlow.GridNode grid_node)
  {
    ...
    return contents.element == SimHashes.Vacuum ? 0.0f : contents.movable_mass;
  }
  ...
  private bool UpdateConduit(ConduitFlow.Conduit conduit)
  {
    ...
    float movableMass = this.ComputeMovableMass(grid_node);
        ...
        if ((permittedFlowDirections & flowDirections1) != ConduitFlow.FlowDirections.None)
        {
          ...
          else
          {
            ...
            if ((double) num2 > 0.0 && flag3)
            {
              ...
              flag1 = 0.0 < (double) this.ComputeMovableMass(grid_node);
              break;
            }
          }
        }
  ...
  {
    get => Mathf.Clamp01((float) (((double) Time.time - (double) this.lastUpdateTime) / 1.0));
  }

  public float DiscreteLerpPercent => Mathf.Clamp01(this.elapsedTime / 1f);

  public float GetAmountAllowedForMerging(
    ConduitFlow.ConduitContents from,
    ConduitFlow.ConduitContents to,
    float massDesiredtoBeMoved)
  {
    return Mathf.Min(massDesiredtoBeMoved, this.MaxMass - to.mass);
  }

  public bool CanMergeContents(
    ConduitFlow.ConduitContents from,
    ConduitFlow.ConduitContents to,
    float massToMove)
  {
    return (from.element == to.element || to.element == SimHashes.Vacuum || (double) massToMove <= 0.0) && (double) this.GetAmountAllowedForMerging(from, to, massToMove) > 0.0;
  }
  ...
  private void OnDeserialized()
  {
    ...
    for (int index = 0; index < this.versionedSerializedContents.Length; ++index)
    {
      ...
      if (float.IsNaN(contents.temperature) || (double) contents.temperature <= 0.0 && contents.element != SimHashes.Vacuum || 10000.0 < (double) contents.temperature)
      {
        ...
        DeserializeWarnings.Instance.PipeContentsTemperatureIsNan.Warn($"Invalid pipe content temperature of {contents.temperature} detected. Resetting temperature. (x={xy.x}, y={xy.y}, cell={cell})");
  ...
  public void FreezeConduitContents(int conduit_idx)
  {
    ...
    if (!((UnityEngine.Object) conduitGo != (UnityEngine.Object) null) || (double) this.soaInfo.GetConduit(conduit_idx).GetContents(this).mass <= (double) this.MaxMass * 0.10000000149011612)
      return;
  ...
  public class SOAInfo
  {
      ...
      ConduitFlow.FlowDirections direction)
    {
      ...
      {
        case ConduitFlow.FlowDirections.Down:
          return conduitConnection.down == Grid.InvalidCell ? ConduitFlow.Conduit.Invalid : this.conduits[conduitConnection.down];
        case ConduitFlow.FlowDirections.Left:
          return conduitConnection.left == Grid.InvalidCell ? ConduitFlow.Conduit.Invalid : this.conduits[conduitConnection.left];
        case ConduitFlow.FlowDirections.Right:
          return conduitConnection.right == Grid.InvalidCell ? ConduitFlow.Conduit.Invalid : this.conduits[conduitConnection.right];
        case ConduitFlow.FlowDirections.Up:
          return conduitConnection.up == Grid.InvalidCell ? ConduitFlow.Conduit.Invalid : this.conduits[conduitConnection.up];
        default:
  ...
  [Serializable]
  public struct Conduit(int idx) : IEquatable<ConduitFlow.Conduit>
  {
  ...
  private struct BuildNetworkTask : IWorkItem<ConduitFlow>
  {
    ...
    private void ReverseTopologicalOrderingPush(
      ConduitFlow outer,
      List<int> result,
      int start_cell)
    {
      Debug.Assert(this.order_dfs_stack.Count == 0);
      ConduitFlow.BuildNetworkTask.OrderNode orderNode1 = new ConduitFlow.BuildNetworkTask.OrderNode()
      {
        idx = outer.grid[start_cell].conduitIdx,
        direction = ConduitFlow.FlowDirections.Down
      };
      orderNode1.permited = outer.soaInfo.GetPermittedFlowDirections(orderNode1.idx);
      if (orderNode1.idx == -1 || !this.visited.Add(orderNode1.idx))
        return;
      this.order_dfs_stack.Push(orderNode1);
      while (this.order_dfs_stack.Count > 0)
      {
        ...
        if ((orderNode2.direction & ConduitFlow.FlowDirections.All) != ConduitFlow.FlowDirections.None)
        {
          ...
          orderNode2.direction = (ConduitFlow.FlowDirections) ((uint) direction << 1);
          this.order_dfs_stack.Push(orderNode2);
          if ((orderNode2.permited & direction) != ConduitFlow.FlowDirections.None)
          {
            orderNode1.idx = outer.soaInfo.GetConduitConnections(orderNode2.idx).GetConnection(direction);
            orderNode1.direction = ConduitFlow.FlowDirections.Down;
            orderNode1.permited = outer.soaInfo.GetPermittedFlowDirections(orderNode1.idx);
            if (orderNode1.idx != -1 && this.visited.Add(orderNode1.idx))
              this.order_dfs_stack.Push(orderNode1);
          }
    ...
    private void ReverseTopologicalOrderingPull(
      ConduitFlow outer,
      List<int> result,
      int start_cell)
    {
      ...
      while (this.order_dfs_stack.Count > 0)
      {
        ...
        if ((orderNode2.direction & ConduitFlow.FlowDirections.All) != ConduitFlow.FlowDirections.None)
        {
          ...
          orderNode2.direction = (ConduitFlow.FlowDirections) ((uint) direction << 1);
          this.order_dfs_stack.Push(orderNode2);
          orderNode1.idx = outer.soaInfo.GetConduitConnections(orderNode2.idx).GetConnection(ConduitFlow.Opposite(direction));
          if (orderNode1.idx != -1)
          {
            orderNode1.direction = ConduitFlow.FlowDirections.Down;
            orderNode1.permited = outer.soaInfo.GetPermittedFlowDirections(orderNode1.idx);
            if ((orderNode1.permited & direction) != ConduitFlow.FlowDirections.None && this.visited.Add(orderNode1.idx))
              this.order_dfs_stack.Push(orderNode1);
          }
        }
        else
    ...
    private void ComputeOrder(ConduitFlow outer)
    {
      ...
      foreach (int source in (HashSet<int>) this.from_sources_graph.sources)
        this.ReverseTopologicalOrderingPush(outer, this.network.cells, source);
      foreach (int deadEnd in (HashSet<int>) this.from_sources_graph.dead_ends)
        this.ReverseTopologicalOrderingPush(outer, this.network.cells, deadEnd);
      int count = this.network.cells.Count;
      foreach (int source in (HashSet<int>) this.from_sinks_graph.sources)
        this.ReverseTopologicalOrderingPull(outer, this.network.cells, source);
      foreach (int deadEnd in (HashSet<int>) this.from_sinks_graph.dead_ends)
        this.ReverseTopologicalOrderingPull(outer, this.network.cells, deadEnd);
    ...
    private struct Graph(FlowUtilityNetwork network)
    {
      private ConduitFlow conduit_flow = (ConduitFlow) null;
      private HashSetPool<int, ConduitFlow>.PooledHashSet vertex_cells = HashSetPool<int, ConduitFlow>.Allocate();
      private ListPool<ConduitFlow.BuildNetworkTask.Graph.Edge, ConduitFlow>.PooledList edges = ListPool<ConduitFlow.BuildNetworkTask.Graph.Edge, ConduitFlow>.Allocate();
      private ListPool<ConduitFlow.BuildNetworkTask.Graph.Edge, ConduitFlow>.PooledList cycles = ListPool<ConduitFlow.BuildNetworkTask.Graph.Edge, ConduitFlow>.Allocate();
      private QueuePool<ConduitFlow.BuildNetworkTask.Graph.Vertex, ConduitFlow>.PooledQueue bfs_traversal = QueuePool<ConduitFlow.BuildNetworkTask.Graph.Vertex, ConduitFlow>.Allocate();
      private HashSetPool<int, ConduitFlow>.PooledHashSet visited = HashSetPool<int, ConduitFlow>.Allocate();
      private ListPool<ConduitFlow.BuildNetworkTask.Graph.Vertex, ConduitFlow>.PooledList pseudo_sources = ListPool<ConduitFlow.BuildNetworkTask.Graph.Vertex, ConduitFlow>.Allocate();
      public HashSetPool<int, ConduitFlow>.PooledHashSet sources = HashSetPool<int, ConduitFlow>.Allocate();
      private HashSetPool<int, ConduitFlow>.PooledHashSet sinks = HashSetPool<int, ConduitFlow>.Allocate();
      private HashSetPool<ConduitFlow.BuildNetworkTask.Graph.DFSNode, ConduitFlow>.PooledHashSet dfs_path = HashSetPool<ConduitFlow.BuildNetworkTask.Graph.DFSNode, ConduitFlow>.Allocate();
      private ListPool<ConduitFlow.BuildNetworkTask.Graph.DFSNode, ConduitFlow>.PooledList dfs_traversal = ListPool<ConduitFlow.BuildNetworkTask.Graph.DFSNode, ConduitFlow>.Allocate();
      public HashSetPool<int, ConduitFlow>.PooledHashSet dead_ends = HashSetPool<int, ConduitFlow>.Allocate();
      private ListPool<ConduitFlow.BuildNetworkTask.Graph.Vertex, ConduitFlow>.PooledList cycle_vertices = ListPool<ConduitFlow.BuildNetworkTask.Graph.Vertex, ConduitFlow>.Allocate();
        ...
        while (this.bfs_traversal.Count != 0)
        {
          ...
          for (int index = 0; index != num; ++index)
          {
            ...
            if (new_node.is_valid)
            {
              ...
              else if (!this.edges.Any<ConduitFlow.BuildNetworkTask.Graph.Edge>((Func<ConduitFlow.BuildNetworkTask.Graph.Edge, bool>) (edge => edge.vertices[0].cell == new_node.cell && edge.vertices[1].cell == node.cell)) && !this.edges.Contains(edge2))
              {
      ...
      private bool IsSink(int cell) => this.sinks.Contains(cell);
      ...
      public void Merge(ConduitFlow.BuildNetworkTask.Graph inverted_graph)
      {
        foreach (ConduitFlow.BuildNetworkTask.Graph.Edge edge1 in (List<ConduitFlow.BuildNetworkTask.Graph.Edge>) inverted_graph.edges)
        {
          ...
          if (!this.edges.Any<ConduitFlow.BuildNetworkTask.Graph.Edge>((Func<ConduitFlow.BuildNetworkTask.Graph.Edge, bool>) (edge => edge.Equals(inverted_edge) || edge.Equals(candidate))))
          {
        ...
        for (int index1 = 0; index1 != num; ++index1)
        {
          ...
          foreach (int vertexCell in (HashSet<int>) this.vertex_cells)
          {
            ...
            if (!this.IsSink(cell) && !this.edges.Any<ConduitFlow.BuildNetworkTask.Graph.Edge>((Func<ConduitFlow.BuildNetworkTask.Graph.Edge, bool>) (edge => edge.vertices[0].cell == cell)))
            {
              ...
              if (index2 != -1)
              {
                ConduitFlow.BuildNetworkTask.Graph.Edge edge2 = inverted_graph.edges[index2];
                for (int index3 = 0; index3 != this.edges.Count; ++index3)
                {
                  ConduitFlow.BuildNetworkTask.Graph.Edge edge3 = this.edges[index3];
                  if (edge3.vertices[0].cell == edge2.vertices[0].cell && edge3.vertices[1].cell == edge2.vertices[1].cell)
                    this.edges[index3] = edge3.Invert();
                }
                flag = true;
                break;
              }
  ...
  private struct ConnectTask(int start, int end) : IWorkItem<ConduitFlow.ConnectContext>
  {
      ...
      for (int start = this.start; start != this.end; ++start)
      {
        ...
        if (conduitIdx != -1)
        {
          ...
          if (connections != (UtilityConnections) 0)
          {
            ...
            int cell4 = Grid.CellBelow(cell1);
            ...
            int cell5 = Grid.CellAbove(cell1);
  ...
  private struct UpdateNetworkTask(ConduitFlow.Network network) : IWorkItem<ConduitFlow>
  {
    private ConduitFlow.Network network = network;

    public bool continue_updating { get; private set; } = true;
