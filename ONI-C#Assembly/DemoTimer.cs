...
public class DemoTimer : MonoBehaviour
{
  ...
  private void Start()
  {
    ...
    if (GenericGameSettings.instance != null)
    {
      ...
      this.gameObject.SetActive(false);
    this.duration = (float) GenericGameSettings.instance.demoTime;
    this.fadeOutScreen = Util.KInstantiateUI(this.Prefab_FadeOutScreen, GameScreenManager.Instance.ssOverlayCanvas.gameObject);
  ...
  public void EndDemo()
  {
    if (this.demoOver)
      return;
    this.demoOver = true;
    Util.KInstantiateUI(this.Prefab_DemoOverScreen, GameScreenManager.Instance.ssOverlayCanvas.gameObject).GetComponent<DemoOverScreen>().Show();
  }
}
