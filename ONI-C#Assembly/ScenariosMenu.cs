...
public class ScenariosMenu : KModalScreen, SteamUGCService.IClient
{
  ...
  private void RebuildUGCButtons()
  {
    ...
    if (pooledList.Count != 0)
    {
      ...
      {
        GameObject gameObject = Util.KInstantiateUI(this.ugcButtonPrefab, this.ugcContainer);
        ...
        component1.GetReference<LocText>("Title").SetText(pooledList[index1].title);
        Texture2D previewImage = pooledList[index1].previewImage;
        if ((UnityEngine.Object) previewImage != (UnityEngine.Object) null)
          component1.GetReference<Image>("Image").sprite = Sprite.Create(previewImage, new UnityEngine.Rect(Vector2.zero, new Vector2((float) previewImage.width, (float) previewImage.height)), Vector2.one * 0.5f);
  ...
  private void LoadScenario(PublishedFileId_t item)
  {
    ...
    string pchFolder;
    uint punTimeStamp;
    SteamUGC.GetItemInstallInfo(item, out punSizeOnDisk, out pchFolder, 1024U /*0x0400*/, out punTimeStamp);
  ...
  private void HideDetails() => this.detailsRoot.gameObject.SetActive(false);
