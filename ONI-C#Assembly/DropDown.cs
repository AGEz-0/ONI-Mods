...
public class DropDown : KMonoBehaviour
{
  ...
  private void Update()
  {
    if (!this.open || !Input.GetMouseButtonDown(0) && (double) Input.GetAxis("Mouse ScrollWheel") == 0.0 && !KInputManager.steamInputInterpreter.GetSteamInputActionIsDown(Action.MouseLeft))
      return;
    ...
    if (this.addEmptyRow)
    {
      ...
      this.emptyRow.GetComponent<KButton>().onClick += (System.Action) (() =>
      {
        ...
        if (this.displaySelectedValueWhenClosed)
          this.selectedLabel.text = this.emptyRowLabel ?? (string) STRINGS.UI.DROPDOWN.NONE;
        this.Close();
      });
      string str = this.emptyRowLabel ?? (string) STRINGS.UI.DROPDOWN.NONE;
      this.emptyRow.GetComponent<DropDownEntry>().label.text = str;
  ...
  public void Open()
  {
    ...
    UnityEngine.Rect rect = this.dropdownAlignmentTarget.rect;
    double x = (double) rect.x;
    rect = this.dropdownAlignmentTarget.rect;
    double y = (double) rect.y;
    Vector3 vector3 = dropdownAlignmentTarget.TransformPoint((float) x, (float) y, 0.0f);
    Vector2 vector2 = new Vector2(Mathf.Min(0.0f, (float) Screen.width - (vector3.x + (this.rowEntryPrefab.GetComponent<LayoutElement>().minWidth * this.canvasScaler.GetCanvasScale() + DropDown.edgePadding.x))), -Mathf.Min(0.0f, vector3.y - (this.scrollRect.rectTransform().sizeDelta.y * this.canvasScaler.GetCanvasScale() + DropDown.edgePadding.y)));
    Vector3 position = vector3 + (Vector3) vector2;
