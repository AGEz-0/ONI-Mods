...
public class LogicDiseaseSensor : Switch, ISaveLoadable, IThresholdSwitch, ISim200ms
{
  ...
  {
    get => (string) UI.UISIDESCREENS.THRESHOLD_SWITCH_SIDESCREEN.DISEASE_TOOLTIP_ABOVE;
  }

  public string BelowToolTip
  {
    get => (string) UI.UISIDESCREENS.THRESHOLD_SWITCH_SIDESCREEN.DISEASE_TOOLTIP_BELOW;
  }
  ...
  private void UpdateVisualState(bool force = false)
  {
    ...
    if (this.switchedOn)
    {
      ...
      if (idx != byte.MaxValue)
        color32 = GlobalAssets.Instance.colorSet.GetColorByName(Db.Get().Diseases[(int) idx].overlayColourName);
