...
public class AllResourcesScreen : ShowOptimizedKScreen, ISim4000ms, ISim1000ms
{
  ...
  public void Init()
  {
    ...
    this.searchInputField.OnValueChangesPaused = (System.Action) (() => this.SearchFilter(this.searchInputField.text));
  ...
  private void SpawnCategoryRow(Tag categoryTag, GameUtil.MeasureUnit unit)
  {
    ...
    foreach (Tag tag in DiscoveredResources.Instance.GetDiscoveredResourcesFromTag(categoryTag))
    {
      if (!this.resourceRows.ContainsKey(tag))
      {
        ...
        component.GetReference<LocText>("NameLabel").SetText(tag.ProperNameStripLink());
        Tag targetTag = tag;
        ...
        component.GetReference<SparkLayer>("Chart").GetComponent<GraphBase>().axis_x.min_value = 0.0f;
  ...
  private bool PassesSearchFilter(Tag tag, string filter)
  {
    ...
    string upper = tag.ProperNameStripLink().ToUpper();
    return !(filter != "") || upper.Contains(filter);
  }
  ...
  private void EnableCategoriesByActiveChildren()
  {
    foreach (KeyValuePair<Tag, AllResourcesScreen.CategoryRow> categoryRow in this.categoryRows)
    {
      ...
      else
      {
        ...
        foreach (KeyValuePair<Tag, AllResourcesScreen.ResourceRow> resourceRow in this.resourceRows)
        {
          if (!((UnityEngine.Object) resourceRow.Value.GameObject.transform.parent.gameObject != (UnityEngine.Object) container))
  ...
  public void RefreshRows()
  {
    ...
    foreach (KeyValuePair<Tag, AllResourcesScreen.CategoryRow> categoryRow in this.categoryRows)
    {
      ...
      {
        float amount = worldInventory.GetAmount(categoryRow.Key, false);
        float totalAmount = worldInventory.GetTotalAmount(categoryRow.Key, false);
        if (!worldInventory.HasValidCount)
        {
          categoryRow.Value.availableLabel.SetText((string) STRINGS.UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
          categoryRow.Value.totalLabel.SetText((string) STRINGS.UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
          categoryRow.Value.reservedLabel.SetText((string) STRINGS.UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
        }
        else
        {
          switch (this.units[categoryRow.Key])
          {
            ...
            case GameUtil.MeasureUnit.kcal:
              float calories = WorldResourceAmountTracker<RationTracker>.Get().CountAmount((Dictionary<string, float>) null, ClusterManager.Instance.activeWorld.worldInventory);
              if (categoryRow.Value.CheckAvailableAmountChanged(amount, true))
    ...
    foreach (KeyValuePair<Tag, AllResourcesScreen.ResourceRow> resourceRow in this.resourceRows)
    {
      ...
      {
        float amount = worldInventory.GetAmount(resourceRow.Key, false);
        float totalAmount = worldInventory.GetTotalAmount(resourceRow.Key, false);
        if (!worldInventory.HasValidCount)
        {
          resourceRow.Value.availableLabel.SetText((string) STRINGS.UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
          resourceRow.Value.totalLabel.SetText((string) STRINGS.UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
          resourceRow.Value.reservedLabel.SetText((string) STRINGS.UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
        }
        else
        {
          switch (this.units[resourceRow.Key])
          {
            case GameUtil.MeasureUnit.mass:
              if (resourceRow.Value.CheckAvailableAmountChanged(amount, true))
                resourceRow.Value.availableLabel.SetText(GameUtil.GetFormattedMass(amount));
              if (resourceRow.Value.CheckTotalResourceAmountChanged(totalAmount, true))
                resourceRow.Value.totalLabel.SetText(GameUtil.GetFormattedMass(totalAmount));
              if (resourceRow.Value.CheckReservedResourceAmountChanged(totalAmount - amount, true))
              {
                resourceRow.Value.reservedLabel.SetText(GameUtil.GetFormattedMass(totalAmount - amount));
                break;
              }
              break;
            case GameUtil.MeasureUnit.kcal:
              float num = WorldResourceAmountTracker<RationTracker>.Get().CountAmountForItemWithID(resourceRow.Key.Name, ClusterManager.Instance.activeWorld.worldInventory);
              ...
              if (resourceRow.Value.CheckTotalResourceAmountChanged(totalAmount, true))
                resourceRow.Value.totalLabel.SetText(GameUtil.GetFormattedCalories(totalAmount));
              if (resourceRow.Value.CheckReservedResourceAmountChanged(totalAmount - amount, true))
              {
                resourceRow.Value.reservedLabel.SetText(GameUtil.GetFormattedCalories(totalAmount - amount));
                break;
              }
              break;
            case GameUtil.MeasureUnit.quantity:
              if (resourceRow.Value.CheckAvailableAmountChanged(amount, true))
                resourceRow.Value.availableLabel.SetText(GameUtil.GetFormattedUnits(amount));
              if (resourceRow.Value.CheckTotalResourceAmountChanged(totalAmount, true))
                resourceRow.Value.totalLabel.SetText(GameUtil.GetFormattedUnits(totalAmount));
              if (resourceRow.Value.CheckReservedResourceAmountChanged(totalAmount - amount, true))
              {
                resourceRow.Value.reservedLabel.SetText(GameUtil.GetFormattedUnits(totalAmount - amount));
                break;
              }
              break;
          }
  ...
  private void SetRowsActive()
  {
    ...
    foreach (KeyValuePair<Tag, AllResourcesScreen.ResourceRow> resourceRow in this.resourceRows)
    {
      if (resourceRow.Value.GameObject.activeSelf != this.currentlyDisplayedRows[resourceRow.Key])
      {
        ...
        if (!this.currentlyDisplayedRows[resourceRow.Key] && resourceRow.Value.horizontalLayoutGroup.enabled)
          resourceRow.Value.horizontalLayoutGroup.enabled = false;
      }
    }
  }
