...
public class AirConditioner : KMonoBehaviour, ISaveLoadable, IGameObjectEffectDescriptor, ISim200ms
{
  ...
  protected override void OnSpawn()
  {
    ...
    this.gameObject.AddOrGet<EntityCellVisualizer>().AddPort(EntityCellVisualizer.Ports.HeatSource, new CellOffset());
  ...
  private void UpdateState(float dt)
  {
    ...
    for (int index = 0; index < items.Count; ++index)
    {
      ...
      if ((double) component.Mass > 0.0 && (!this.isLiquidConditioner || !component.Element.IsGas) && (this.isLiquidConditioner || !component.Element.IsLiquid))
      {
        ...
        float f = (temperature - component.Temperature) * component.Element.specificHeatCapacity * num1;
        float display_dt = (double) this.lastSampleTime > 0.0 ? Time.time - this.lastSampleTime : 1f;
        this.lastSampleTime = Time.time;
        this.heatEffect.SetHeatBeingProducedValue(Mathf.Abs(f));
        GameComps.StructureTemperatures.ProduceEnergy(this.structureTemperature, -f, (string) BUILDING.STATUSITEMS.OPERATINGENERGY.PIPECONTENTS_TRANSFER, display_dt);
        break;
      }
    }
