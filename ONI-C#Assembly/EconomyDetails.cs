...
public class EconomyDetails
{
  ...
  public EconomyDetails()
  {
    ...
    foreach (EdiblesManager.FoodInfo allFoodType in EdiblesManager.GetAllFoodTypes())
      this.CreateResource(allFoodType.Id.ToTag(), this.amountResourceType);
    ...
    transformation1.AddDelta(new EconomyDetails.Transformation.Delta(this.GetResource(GameTags.Oxygen), -DUPLICANTSTATS.STANDARD.BaseStats.OXYGEN_USED_PER_SECOND));
    transformation1.AddDelta(new EconomyDetails.Transformation.Delta(this.GetResource(GameTags.CarbonDioxide), DUPLICANTSTATS.STANDARD.BaseStats.OXYGEN_USED_PER_SECOND * Assets.GetPrefab((Tag) MinionConfig.ID).GetComponent<OxygenBreather>().O2toCO2conversion));
    transformation1.AddDelta(new EconomyDetails.Transformation.Delta(this.duplicantTimeResource, 0.875f));
    transformation1.AddDelta(new EconomyDetails.Transformation.Delta(this.caloriesResource, DUPLICANTSTATS.STANDARD.BaseStats.GUESSTIMATE_CALORIES_BURNED_PER_SECOND * (1f / 1000f)));
    transformation1.AddDelta(new EconomyDetails.Transformation.Delta(this.CreateResource(new Tag(Db.Get().Amounts.Bladder.deltaAttribute.Id), this.amountResourceType), DUPLICANTSTATS.STANDARD.BaseStats.BLADDER_INCREASE_PER_SECOND));
  ...
  public void DumpTransformations(EconomyDetails.Scenario scenario, StreamWriter o)
  {
    ...
    {
      Tag tag = x.tag;
      string name3 = tag.Name;
      tag = y.tag;
      string name4 = tag.Name;
    ...
    for (int index = 0; index < this.transformations.Count; ++index)
    {
      ...
      if (scenario.IncludesTransformation(transformation))
      {
        ...
        if (!transformation.timeInvariant)
        {
          ...
          string str3 = $",\"{num5.ToString("0.00")}\"";
    ...
    for (int index = 0; index < num1; ++index)
    {
      ...
      {
        string str4 = ((char) (65 + index % 26)).ToString();
        int num10 = Mathf.FloorToInt((float) index / 26f);
        if (num10 > 0)
          str4 = ((char) (65 + num10 - 1)).ToString() + str4;
        o.Write($"\"=SUM({str4}2: {str4}{num2.ToString()})\"");
      }
      o.Write(",");
    }
    string str5 = "B" + (num2 + 5).ToString();
    o.Write("\n");
    o.Write("\nTiming:");
    StreamWriter streamWriter3 = o;
    float num11 = scenario.timeInSeconds;
    string str6 = "\nTimeInSeconds:," + num11.ToString();
    streamWriter3.Write(str6);
  ...
  public EconomyDetails.Transformation CreateTransformation(KPrefabID prefab_id, Tag tag)
  {
    ...
    if (transformation != null)
    {
      ...
      if ((bool) (UnityEngine.Object) component10)
      {
        ...
        ComplexRecipeManager.Get().recipes.Find((Predicate<ComplexRecipe>) (a => a.FirstResult == tag));
      }
      if ((UnityEngine.Object) component11 != (UnityEngine.Object) null)
      {
  ...
  private void CollectDietTransformations(KPrefabID prefab_id)
  {
    ...
    foreach (Diet.Info info in def1.diet.infos)
    {
      foreach (Tag consumedTag in info.consumedTags)
      {
        ...
        EconomyDetails.Transformation transformation = new EconomyDetails.Transformation(new Tag($"{prefab_id.PrefabTag.Name}Diet{consumedTag.Name}"), this.creatureTransformationType, time_in_seconds);
  ...
  private static void DumpEconomyDetails()
  {
    ...
    {
      EconomyDetails.Scenario scenario12 = new EconomyDetails.Scenario("transformation_groups/" + transformation1.tag.Name, 0.0f, (Func<EconomyDetails.Transformation, bool>) null);
  ...
  private static void DumpNameMapping()
  {
    ...
    using (StreamWriter streamWriter = new StreamWriter("assets/Tuning/Economy/name_mapping.csv"))
    {
      ...
      foreach (KPrefabID prefab in Assets.Prefabs)
      {
        string str = TagManager.StripLinkFormatting(prefab.GetProperName());
        Tag tag = prefab.PrefabID();
        if (!str.IsNullOrWhiteSpace() && !tag.Name.Contains("UnderConstruction") && !tag.Name.Contains("Preview"))
        {
          streamWriter.Write(str);
          ...
          if ((UnityEngine.Object) component != (UnityEngine.Object) null)
          {
            foreach (KAnimFile animFile in component.AnimFiles)
              streamWriter.Write("," + animFile.name);
          }
          else
            streamWriter.Write(",");
          streamWriter.Write("\n");
      ...
      {
        PermitResource permit = resource;
        if ((Blueprints.Get().skinsRelease.buildingFacades.Any<BuildingFacadeInfo>((Func<BuildingFacadeInfo, bool>) (info => info.id == permit.Id)) || Blueprints.Get().skinsRelease.clothingItems.Any<ClothingItemInfo>((Func<ClothingItemInfo, bool>) (info => info.id == permit.Id)) ? 0 : (!Blueprints.Get().skinsRelease.artables.Any<ArtableInfo>((Func<ArtableInfo, bool>) (info => info.id == permit.Id)) ? 1 : 0)) == 0)
        {
          string str1 = TagManager.StripLinkFormatting(permit.Name);
          streamWriter.Write(str1);
          string id = permit.Id;
          streamWriter.Write("," + id);
          string str2 = !(permit is BuildingFacadeResource buildingFacadeResource) ? (!(permit is ClothingItemResource clothingItemResource) ? (!(permit is ArtableStage artableStage) ? "" : artableStage.animFile) : clothingItemResource.AnimFile.name) : buildingFacadeResource.AnimFile;
  ...
  public class BiomeTransformation
  {
    ...
    {
      return this.resource.tag == element.tag ? this.ratio * amount : 0.0f;
    }
  }
