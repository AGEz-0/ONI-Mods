...
public class InputBindingsScreen : KModalScreen
{
  ...
  private void CollectScreens()
  {
    ...
    for (int index = 0; index < GameInputMapping.KeyBindings.Length; ++index)
    {
      ...
      if (keyBinding.mGroup != null && keyBinding.mRebindable && !this.screens.Contains(keyBinding.mGroup) && DlcManager.IsCorrectDlcSubscribed(keyBinding.dlcIds, (string[]) null))
      {
  ...
  private void BuildDisplay()
  {
    ...
    for (int index2 = 0; index2 < GameInputMapping.KeyBindings.Length; ++index2)
    {
      ...
      if (binding.mGroup == this.screens[this.activeScreen] && binding.mRebindable && DlcManager.IsCorrectDlcSubscribed(binding.dlcIds, (string[]) null))
      {
  ...
  private void Update()
  {
    if (!this.waitingForKeyPress)
      return;
    Modifier modifier = (Modifier) (0 | (this.IsKeyDown(KeyCode.LeftAlt) || this.IsKeyDown(KeyCode.RightAlt) ? 1 : 0) | (this.IsKeyDown(KeyCode.LeftControl) || this.IsKeyDown(KeyCode.RightControl) ? 2 : 0) | (this.IsKeyDown(KeyCode.LeftShift) || this.IsKeyDown(KeyCode.RightShift) ? 4 : 0) | (this.IsKeyDown(KeyCode.CapsLock) ? 8 : 0) | (this.IsKeyDown(KeyCode.BackQuote) ? 16 /*0x10*/ : 0));
    bool flag = false;
  ...
  private int NumUnboundActions()
  {
    ...
    {
      BindingEntry keyBinding = GameInputMapping.KeyBindings[index];
      if (keyBinding.mKeyCode == KKeyCode.None && keyBinding.mRebindable && (BuildMenu.UseHotkeyBuildMenu() || !keyBinding.mIgnoreRootConflics))
        ++num;
    }
    return num;
  }
  ...
  {
    BindingEntry bindingEntry = new BindingEntry(this.screens[this.activeScreen], GamepadButton.NumButtons, kkey_code, modifier, this.actionToRebind, ignore_root_conflicts: this.ignoreRootConflicts);
    for (int index = 0; index < GameInputMapping.KeyBindings.Length; ++index)
    {
      BindingEntry keyBinding = GameInputMapping.KeyBindings[index];
