...
public static class SymbolOverrideControllerUtil
{
  public static SymbolOverrideController AddToPrefab(GameObject prefab)
  {
    ...
    DebugUtil.Assert((Object) component != (Object) null, "SymbolOverrideController must be added after a KBatchedAnimController component.");
    ...
    for (int index = 0; index < anim_file.GetData().build.symbols.Length; ++index)
    {
      ...
      if (prefix != null && postfix != null)
      {
        if (str1.StartsWith(prefix) && str1.EndsWith(postfix))
        {
          ...
          string target_symbol = str2.Substring(0, str2.Length - postfix.Length);
          symbol_override_controller.AddSymbolOverride((HashedString) target_symbol, symbol, priority);
        }
      }
      else if (prefix != null && str1.StartsWith(prefix))
        symbol_override_controller.AddSymbolOverride((HashedString) str1.Substring(prefix.Length, str1.Length - prefix.Length), symbol, priority);
      else if (postfix != null && str1.EndsWith(postfix))
        symbol_override_controller.AddSymbolOverride((HashedString) str1.Substring(0, str1.Length - postfix.Length), symbol, priority);
    }
  }
}
