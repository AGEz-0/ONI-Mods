...
public class RetiredColonyData
{
    ...
    Dictionary<string, string> worldIdentities)
  {
    ...
    for (int index = 0; index < this.Duplicants.Length; ++index)
    {
      ...
      if ((Object) minions[index].GetTargetGameObject().GetComponent<Accessorizer>() != (Object) null)
      {
    ...
    if (buildingCompletes != null)
    {
      foreach (BuildingComplete buildingComplete in buildingCompletes)
      {
        ...
        dictionary[buildingComplete.PrefabID()]++;
      }
    }
    ...
    for (int index = 0; index < data7.Length; ++index)
    {
      int num1 = 0;
      float num2 = 0.0f;
      ReportManager.ReportEntry entry = ReportManager.Instance.reports[index].GetEntry(ReportManager.ReportType.WorkTime);
      for (int i = 0; i < entry.contextEntries.Count; ++i)
      {
        ++num1;
        num2 += entry.contextEntries[i].accPositive;
      }
      float b = num2 / (float) num1 / 600f * 100f;
      data8[index] = new Tuple<float, float>((float) ReportManager.Instance.reports[index].day, b);
    }
    ...
    for (int index = 0; index < data9.Length; ++index)
    {
      ...
      {
        ++num3;
        num4 += entry.contextEntries[i].accPositive;
      }
      float b = num4 / (float) num3 / 600f * 100f;
      data10[index] = new Tuple<float, float>((float) ReportManager.Instance.reports[index].day, b);
    }
    ...
    for (int index = 0; index < data12.Length; ++index)
    {
      int num5 = 0;
      float num6 = 0.0f;
      ReportManager.ReportEntry entry = ReportManager.Instance.reports[index].GetEntry(ReportManager.ReportType.StressDelta);
      for (int i = 0; i < entry.contextEntries.Count; ++i)
      {
        ++num5;
        num6 += entry.contextEntries[i].accPositive;
      }
      data12[index] = new Tuple<float, float>((float) ReportManager.Instance.reports[index].day, num6 / (float) num5);
    }
    Tuple<float, float>[] data13 = new Tuple<float, float>[ReportManager.Instance.reports.Count];
    for (int index = 0; index < data13.Length; ++index)
    {
      int num7 = 0;
      float num8 = 0.0f;
      ReportManager.ReportEntry entry = ReportManager.Instance.reports[index].GetEntry(ReportManager.ReportType.StressDelta);
      ...
      {
        ++num7;
        num8 += entry.contextEntries[i].accNegative;
      }
      float num9 = num8 * -1f;
      data13[index] = new Tuple<float, float>((float) ReportManager.Instance.reports[index].day, num9 / (float) num7);
    }
    ...
    this.Stats = new RetiredColonyData.RetiredColonyStatistic[16 /*0x10*/]
    {
