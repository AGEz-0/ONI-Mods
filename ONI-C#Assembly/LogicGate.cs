...
public class LogicGate : LogicGateBase, ILogicEventSender, ILogicNetworkConnection
{
  ...
  {
    this.inputOne = new LogicEventHandler(this.InputCellOne, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.Input);
    if (this.RequiresTwoInputs)
      this.inputTwo = new LogicEventHandler(this.InputCellTwo, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.Input);
    else if (this.RequiresFourInputs)
    {
      this.inputTwo = new LogicEventHandler(this.InputCellTwo, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.Input);
      this.inputThree = new LogicEventHandler(this.InputCellThree, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.Input);
      this.inputFour = new LogicEventHandler(this.InputCellFour, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.Input);
    }
    if (this.RequiresControlInputs)
    {
      this.controlOne = new LogicEventHandler(this.ControlCellOne, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.ControlInput);
      this.controlTwo = new LogicEventHandler(this.ControlCellTwo, new Action<int, int>(this.UpdateState), (Action<int, bool>) null, LogicPortSpriteType.ControlInput);
    }
    if (this.RequiresFourOutputs)
    {
      ...
      this.outputTwoSender = new LogicEventSender(LogicGateBase.OUTPUT_TWO_PORT_ID, this.OutputCellTwo, (Action<int, int>) ((new_value, prev_value) =>
      {
      ...
      this.outputThreeSender = new LogicEventSender(LogicGateBase.OUTPUT_THREE_PORT_ID, this.OutputCellThree, (Action<int, int>) ((new_value, prev_value) =>
      {
        if (!((UnityEngine.Object) this != (UnityEngine.Object) null))
          return;
        this.OnAdditionalOutputsLogicValueChanged(LogicGateBase.OUTPUT_THREE_PORT_ID, new_value, prev_value);
      }), (Action<int, bool>) null, LogicPortSpriteType.Output);
      this.outputFourSender = new LogicEventSender(LogicGateBase.OUTPUT_FOUR_PORT_ID, this.OutputCellFour, (Action<int, int>) ((new_value, prev_value) =>
      {
        if (!((UnityEngine.Object) this != (UnityEngine.Object) null))
          return;
  ...
  protected override void OnCleanUp()
  {
    ...
    this.Disconnect();
    this.Unsubscribe<LogicGate>(774203113, LogicGate.OnBuildingBrokenDelegate);
    this.Unsubscribe<LogicGate>(-1735440190, LogicGate.OnBuildingFullyRepairedDelegate);
    base.OnCleanUp();
  }
  ...
  private void UpdateState(int new_value, int prev_value)
  {
    ...
    switch (this.op)
    {
      ...
      case LogicGateBase.Op.Not:
        ...
        int inputCellOne = this.InputCellOne;
        GameObject gameObject = Grid.Objects[inputCellOne, 31 /*0x1F*/];
  ...
  private void OnAdditionalOutputsLogicValueChanged(
    HashedString port_id,
    int new_value,
    int prev_value)
  {
  ...
  {
    return (port != LogicGateBase.PortId.InputTwo || this.RequiresTwoInputs || this.RequiresFourInputs) && (port != LogicGateBase.PortId.InputThree || this.RequiresFourInputs) && (port != LogicGateBase.PortId.InputFour || this.RequiresFourInputs) && Game.Instance.logicCircuitManager.GetNetworkForCell(this.PortCell(port)) != null;
  }
