  ...
  GameStateMachine<HugMinionStates, HugMinionStates.Instance, IStateMachineTarget, HugMinionStates.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    double num;
    GameStateMachine<HugMinionStates, HugMinionStates.Instance, IStateMachineTarget, HugMinionStates.Def>.State state = this.waiting.Enter((StateMachine<HugMinionStates, HugMinionStates.Instance, IStateMachineTarget, HugMinionStates.Def>.State.Callback) (smi => smi.GetComponent<Navigator>().SetCurrentNavType(NavType.Floor))).ParamTransition<float>((StateMachine<HugMinionStates, HugMinionStates.Instance, IStateMachineTarget, HugMinionStates.Def>.Parameter<float>) this.timeout, this.behaviourcomplete, (StateMachine<HugMinionStates, HugMinionStates.Instance, IStateMachineTarget, HugMinionStates.Def>.Parameter<float>.Callback) ((smi, p) => (double) p > 60.0 && !smi.GetSMI<HugMonitor.Instance>().IsHugging())).Update((System.Action<HugMinionStates.Instance, float>) ((smi, dt) => num = (double) smi.sm.timeout.Delta(dt, smi))).PlayAnim("waiting_pre").QueueAnim("waiting_loop", true);
  ...
  private static int FindFlopLocation(HugMinionStates.Instance smi)
  {
    ...
    smi.targetFlopCell = floorCellQuery.result_cells.Count <= 0 ? Grid.InvalidCell : floorCellQuery.result_cells[UnityEngine.Random.Range(0, floorCellQuery.result_cells.Count)];
