...
public class RocketsInOrbitDiagnostic : ColonyDiagnostic
{
  ...
  public ColonyDiagnostic.DiagnosticResult CheckOrbit()
  {
    ...
    ColonyDiagnostic.DiagnosticResult diagnosticResult = new ColonyDiagnostic.DiagnosticResult(ColonyDiagnostic.DiagnosticResult.Opinion.Normal, this.NO_MINIONS);
    ...
    foreach (Clustercraft component in Components.Clustercrafts.Items)
    {
      ...
      if (myWorldLocation2 != myWorldLocation1 && ClusterGrid.Instance.IsInRange(myWorldLocation2, myWorldLocation1) && ClusterGrid.Instance.IsInRange(myWorldLocation1, destination))
      {
