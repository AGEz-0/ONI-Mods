...
public abstract class Disease : Resource
{
    ...
    : base(id)
  {
    ...
    if (statsOnly)
      return;
    this.overlayColourName = Assets.instance.DiseaseVisualization.GetInfo((HashedString) id).overlayColourName;
    ...
    Attribute attribute1 = new Attribute(id + "Min", "Minimum" + id.ToString(), "", "", 0.0f, Attribute.Display.Normal, false);
    Attribute attribute2 = new Attribute(id + "Max", "Maximum" + id.ToString(), "", "", 1E+07f, Attribute.Display.Normal, false);
    this.amountDeltaAttribute = new Attribute(id + "Delta", id.ToString(), "", "", 0.0f, Attribute.Display.Normal, false);
    this.amount = new Amount(id, $"{id} {(string) DUPLICANTS.DISEASES.GERMS}", $"{id} {(string) DUPLICANTS.DISEASES.GERMS}", attribute1, attribute2, this.amountDeltaAttribute, false, Units.Flat, 0.01f, true);
  ...
  public TagGrowthRule GetGrowthRuleForTag(Tag t)
  {
    if (this.growthRules != null)
    {
      for (int index = 0; index < this.growthRules.Count; ++index)
      {
        if (this.growthRules[index] is TagGrowthRule growthRule && growthRule.tag == t)
          return growthRule;
      }
    }
    ...
    ref Disease.RangeInfo half_lives)
  {
    ...
    double num4 = (double) range.GetValue(idx2);
    float t = 0.0f;
    double num5 = (double) num3;
    float num6 = (float) (num4 - num5);
    if ((double) num6 > 0.0)
      t = (range_value - num3) / num6;
    ...
    string item_tooltip)
  {
    ...
    if (rules.Count > 0)
    {
      descriptorList.Add(new Descriptor(section_text, section_tooltip, Descriptor.DescriptorType.Information));
      for (int index = 0; index < rules.Count; ++index)
        descriptorList.Add(new Descriptor(string.Format((string) DUPLICANTS.DISEASES.DESCRIPTORS.INFO.GROWTH_FORMAT, (object) rules[index].Name()), string.Format(item_tooltip, (object) GameUtil.GetFormattedCycles(Mathf.Abs(rules[index].populationHalfLife.Value))), Descriptor.DescriptorType.Information));
    }
    return descriptorList;
  }

  public struct RangeInfo(float min_viable, float min_growth, float max_growth, float max_viable)
  {
    public float minViable = min_viable;
    public float minGrowth = min_growth;
    public float maxGrowth = max_growth;
    public float maxViable = max_viable;
