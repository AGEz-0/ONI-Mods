...
public class AttributeLevel
{
  ...
  {
    float num = (float) ((double) Mathf.Pow((float) this.level / (float) this.maxGainedLevel, DUPLICANTSTATS.ATTRIBUTE_LEVELING.EXPERIENCE_LEVEL_POWER) * (double) DUPLICANTSTATS.ATTRIBUTE_LEVELING.TARGET_MAX_LEVEL_CYCLE * 600.0);
    return (float) ((double) Mathf.Pow(((float) this.level + 1f) / (float) this.maxGainedLevel, DUPLICANTSTATS.ATTRIBUTE_LEVELING.EXPERIENCE_LEVEL_POWER) * (double) DUPLICANTSTATS.ATTRIBUTE_LEVELING.TARGET_MAX_LEVEL_CYCLE * 600.0) - num;
  }
