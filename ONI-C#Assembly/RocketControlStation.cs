  ...
  IGameObjectEffectDescriptor
{
  ...
  {
    return Game.Instance.logicCircuitManager.GetNetworkForCell(this.GetComponent<LogicPorts>().GetPortCell(RocketControlStation.PORT_ID));
  }

  private void UpdateRestrictionAnimSymbol(object o = null)
  {
    this.GetComponent<KAnimControllerBase>().SetSymbolVisiblity((KAnimHashedString) "restriction_sign", this.BuildingRestrictionsActive);
  }
    ...
    GameStateMachine<RocketControlStation.States, RocketControlStation.StatesInstance, RocketControlStation>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.root.Enter("SetTarget", (StateMachine<RocketControlStation.States, RocketControlStation.StatesInstance, RocketControlStation, object>.State.Callback) (smi => this.AquireClustercraft(smi, true))).Target(this.masterTarget).Exit((StateMachine<RocketControlStation.States, RocketControlStation.StatesInstance, RocketControlStation, object>.State.Callback) (smi => this.SetRocketSpeedModifiers(smi, 0.5f)));
    ...
    public void AquireClustercraft(RocketControlStation.StatesInstance smi, bool force = false)
    {
      ...
      if (!((UnityEngine.Object) rocket != (UnityEngine.Object) null))
        return;
    ...
    private Chore CreateChore(RocketControlStation.StatesInstance smi)
    {
      ...
      chore.AddPrecondition(ChorePreconditions.instance.IsRocketTravelling, (object) null);
      return (Chore) chore;
    }
    ...
    {
      this.clusterCraft.Get(smi).GetComponent<Clustercraft>().Launch();
    }
  ...
  public class StatesInstance(RocketControlStation smi) : 
    GameStateMachine<RocketControlStation.States, RocketControlStation.StatesInstance, RocketControlStation, object>.GameInstance(smi)
  {
    ...
    public void SetPilotSpeedMult(WorkerBase pilot)
    {
      ...
      this.pilotSpeedMult = Mathf.Max(1f + pilot.GetComponent<AttributeConverters>().GetConverter(pilotingSpeed.Id).Evaluate(), 0.1f);
    }
  }
}
