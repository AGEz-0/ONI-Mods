...
public class LiquidCooledRefinery : ComplexFabricator
{
  ...
  protected override void OnSpawn()
  {
    ...
    this.workable.OnWorkTickActions = (Action<WorkerBase, float>) ((worker, dt) => this.meter_metal.SetPositionPercent(this.workable.GetPercentComplete()));
  }
  ...
  {
    return (double) this.inStorage.GetAmountAvailable(this.coolantTag) + (double) this.buildStorage.GetAmountAvailable(this.coolantTag) >= (double) this.minCoolantMass;
  }
  ...
  {
    return (double) storage.GetAmountAvailable(this.coolantTag) >= (double) this.minCoolantMass && base.HasIngredients(recipe, storage);
  }
  ...
  public override List<Descriptor> AdditionalEffectsForRecipe(ComplexRecipe recipe)
  {
    ...
    descriptorList.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.REFINEMENT_ENERGY, (object) GameUtil.GetFormattedJoules(joules)), string.Format(format, (object) GameUtil.GetFormattedJoules(joules), (object) cmp.GetProperName(), (object) GameUtil.GetFormattedTemperature(temperatureChange, interpretation: GameUtil.TemperatureInterpretation.Relative))));
    return descriptorList;
  }

  public class StatesInstance(LiquidCooledRefinery master) : 
    GameStateMachine<LiquidCooledRefinery.States, LiquidCooledRefinery.StatesInstance, LiquidCooledRefinery, object>.GameInstance(master)
  {
  }
