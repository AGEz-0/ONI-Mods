...
public class SoundEvent : AnimEvent
{
    ...
    bool is_dynamic)
  {
    ...
    position.x += offset.x;
    position.y += offset.y;
    if (!SoundCuller.IsAudibleWorld((Vector2) position))
      return false;
  ...
  {
    Vector3 sound_pos = behaviour.controller.transform.GetPosition() with
    {
      z = 0.0f
    };
    ...
    try
    {
      if (this.looping)
      {
        ...
        {
          if (component.StartSound(sound, behaviour, this.noiseValues, this.ignorePause, this.shouldCameraScalePosition))
            return;
          DebugUtil.LogWarningArgs((object) $"SoundEvent has invalid sound [{sound}] on behaviour [{behaviour.name}]");
        }
      }
      else
      {
  ...
  public static bool PlayOneShot(EventReference event_ref, Vector3 sound_pos, float volume = 1f)
  {
    ...
    if (!event_ref.IsNull)
    {
      FMOD.Studio.EventInstance instance = SoundEvent.BeginOneShot(event_ref, sound_pos, volume);
      if (instance.isValid())
    ...
    bool objectIsSelectedAndVisible = false)
  {
    ...
    {
      Vector3 vector3 = behaviour.controller.transform.GetPosition() with
      {
        z = 0.0f
      };
      if (objectIsSelectedAndVisible)
  ...
  {
    return sound != null && (UnityEngine.Object) Camera.main != (UnityEngine.Object) null && (double) Camera.main.orthographicSize > (double) AudioMixer.LOW_PRIORITY_CUTOFF_DISTANCE && !AudioMixer.instance.activeNIS && GlobalAssets.IsLowPriority(sound);
  }
