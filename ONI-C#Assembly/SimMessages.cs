...
public static class SimMessages
{
    ...
    int callbackIdx = -1)
  {
    ...
    if (!Grid.IsValidCell(cell))
      Debug.LogErrorFormat("Invalid Cell [{0}] Extents [{1},{2}] [{3},{4}]", (object) cell, (object) extents.x, (object) extents.y, (object) extents.width, (object) extents.height);
    if (!Grid.IsValidCell(cell))
      return;
  ...
  {
    MemoryStream output = new MemoryStream(1024 /*0x0400*/);
    BinaryWriter writer = new BinaryWriter((Stream) output);
    writer.Write(diseases.Count);
    List<Element> elements = ElementLoader.elements;
    writer.Write(elements.Count);
    for (int idx = 0; idx < diseases.Count; ++idx)
    {
      Klei.AI.Disease disease = diseases[idx];
      ...
      for (int index = 0; index < elements.Count; ++index)
        disease.elemGrowthInfo[index].Write(writer);
    }
  ...
  public static void SimDataInitializeFromCells(
    ...
    bool headless)
  {
    MemoryStream output = new MemoryStream(Marshal.SizeOf(typeof (int)) + Marshal.SizeOf(typeof (int)) + Marshal.SizeOf(typeof (uint)) + Marshal.SizeOf(typeof (bool)) + Marshal.SizeOf(typeof (bool)) + Marshal.SizeOf(typeof (Sim.Cell)) * width * height + Marshal.SizeOf(typeof (float)) * width * height + Marshal.SizeOf(typeof (Sim.DiseaseCell)) * width * height);
    BinaryWriter writer = new BinaryWriter((Stream) output);
    writer.Write(width);
    writer.Write(height);
    writer.Write(simSeed);
    ...
    List<Game.SimActiveRegion> activeRegions)
  {
    ...
    {
      Pair<Vector2I, Vector2I> region = activeRegion.region with
      {
        first = new Vector2I(MathUtil.Clamp(0, Grid.WidthInCells - 1, activeRegion.region.first.x), MathUtil.Clamp(0, Grid.HeightInCells - 1, activeRegion.region.first.y)),
        second = new Vector2I(MathUtil.Clamp(0, Grid.WidthInCells, activeRegion.region.second.x), MathUtil.Clamp(0, Grid.HeightInCells - 1, activeRegion.region.second.y))
      };
