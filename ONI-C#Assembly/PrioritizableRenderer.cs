...
public class PrioritizableRenderer
{
  ...
  {
    using (new KProfiler.Region(nameof (PrioritizableRenderer)))
    {
      ...
      foreach (ScenePartitionerEntry partitionerEntry in gathered_entries)
      {
        ...
        if ((Object) component != (Object) null && component.showIcon && component.IsPrioritizable() && this.tool.IsActiveLayer(this.tool.GetFilterLayerFromGameObject(component.gameObject)) && component.GetMyWorldId() == ClusterManager.Instance.activeWorldId)
      ...
      for (int index1 = 0; index1 < this.prioritizables.Count; ++index1)
      {
        ...
        double num1 = 0.10000000149011612;
        ...
        if (masterPriority.priority_class >= PriorityScreen.PriorityClass.topPriority)
          num2 += 0.0f;
        float x = (float) num1 * (num2 + (float) masterPriority.priority_value);
        float y = 0.0f;
        float num3 = (float) num1;
        float num4 = 1f;
        this.uvs[index2] = new Vector2(x, y);
        this.uvs[1 + index2] = new Vector2(x, y + num4);
        this.uvs[2 + index2] = new Vector2(x + num3, y);
        this.uvs[3 + index2] = new Vector2(x + num3, y + num4);
        int index3 = 6 * index1;
