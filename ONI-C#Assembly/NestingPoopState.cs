  ...
  GameStateMachine<NestingPoopState, NestingPoopState.Instance, IStateMachineTarget, NestingPoopState.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    state.ToggleStatusItem(name, tooltip, render_overlay: render_overlay, category: category).PlayAnim("poop").OnAnimQueueComplete(this.behaviourcomplete);
    this.behaviourcomplete.Enter((StateMachine<NestingPoopState, NestingPoopState.Instance, IStateMachineTarget, NestingPoopState.Def>.State.Callback) (smi => smi.SetLastPoopCell())).PlayAnim("idle_loop", KAnim.PlayMode.Loop).BehaviourComplete(GameTags.Creatures.Poop);
  }
    ...
    GameStateMachine<NestingPoopState, NestingPoopState.Instance, IStateMachineTarget, NestingPoopState.Def>.GameInstance
  {
    ...
    {
      if (!Grid.IsValidCell(cell) || Grid.Solid[cell] || !Grid.IsValidCell(Grid.CellBelow(cell)) || !Grid.Solid[Grid.CellBelow(cell)])
        return false;
    ...
    private int GetTargetPoopCell()
    {
      ...
      int cell = GameUtil.FloodFillFind<object>(new Func<int, object, bool>(NestingPoopState.Instance.IsValidNestingCell), (object) null, !(this.currentlyPoopingElement == this.smi.def.nestingPoopElement) || !(this.smi.def.nestingPoopElement != Tag.Invalid) || this.lastPoopCell == -1 ? Grid.PosToCell((StateMachine.Instance) this) : this.lastPoopCell, 8, false, true);
      if (cell == -1)
      {
        ...
        cell = Grid.OffsetCell(this.lastPoopCell, cellOffsetArray[UnityEngine.Random.Range(0, cellOffsetArray.Length)]);
