...
public class BuildingChoresPanelDupeRow : KMonoBehaviour
{
  ...
  public void Init(BuildingChoresPanel.DupeEntryData data)
  {
    ...
    {
      string newValue = (UnityEngine.Object) data.context.chore.driver == (UnityEngine.Object) data.consumer.choreDriver ? DUPLICANTS.CHORES.PRECONDITIONS.CURRENT_ERRAND.text : string.Format(DUPLICANTS.CHORES.PRECONDITIONS.RANK_FORMAT.text, (object) data.rank);
    ...
    {
      string str = data.context.chore.GetPreconditions()[data.context.failedPreconditionId].condition.description;
      DebugUtil.Assert(str != null, "Chore requires description!", data.context.chore.GetPreconditions()[data.context.failedPreconditionId].condition.id);
  ...
  {
    GameUtil.FocusCamera(this.choreConsumer.gameObject.transform.GetPosition() + Vector3.up);
  }
    ...
    int rank)
  {
    ...
    string str2 = str1.Replace("{Description}", (string) ((UnityEngine.Object) context.chore.driver == (UnityEngine.Object) choreConsumer.choreDriver ? STRINGS.UI.DETAILTABS.BUILDING_CHORES.DUPE_TOOLTIP_DESC_ACTIVE : STRINGS.UI.DETAILTABS.BUILDING_CHORES.DUPE_TOOLTIP_DESC_INACTIVE));
    ...
    return flag ? str3.Replace("{Rank}", rank.ToString()).Replace("{Groups}", newValue1).Replace("{BestGroup}", newValue2).Replace("{PersonalPriority}", JobsTableScreen.priorityInfo[personalPriority].name.text).Replace("{PersonalPriorityValue}", (personalPriority * 10).ToString()).Replace("{Building}", context.chore.gameObject.GetProperName()).Replace("{BuildingPriority}", priorityValue.ToString()).Replace("{TypePriority}", num4.ToString()).Replace("{TotalPriority}", num5.ToString()) : str3.Replace("{FailedPrecondition}", context.chore.GetPreconditions()[context.failedPreconditionId].condition.description);
  }
}
