...
public class Desalinator : StateMachineComponent<Desalinator.StatesInstance>
{
  ...
  private void OnRefreshUserMenu(object data)
  {
    if (this.smi.GetCurrentState() == this.smi.sm.full || !this.smi.HasSalt || this.smi.emptyChore != null)
      return;
    Game.Instance.userMenu.AddButton(this.gameObject, new KIconButtonMenu.ButtonInfo("status_item_desalinator_needs_emptying", (string) UI.USERMENUACTIONS.EMPTYDESALINATOR.NAME, (System.Action) (() => this.smi.GoTo((StateMachine.BaseState) this.smi.sm.earlyEmpty)), tooltipText: (string) UI.USERMENUACTIONS.CLEANTOILET.TOOLTIP));
  }
  ...
  public class StatesInstance(Desalinator smi) : 
    GameStateMachine<Desalinator.States, Desalinator.StatesInstance, Desalinator, object>.GameInstance(smi)
  {
    public Chore emptyChore;

    public bool HasSalt
    {
      get => this.master.storage.Has(ElementLoader.FindElementByHash(SimHashes.Salt).tag);
    }
    ...
    public void CreateEmptyChore()
    {
      ...
      DesalinatorWorkableEmpty component = this.master.GetComponent<DesalinatorWorkableEmpty>();
      ...
      this.smi.master.remoteChore.SetChore(this.emptyChore);
    }
    ...
    public void CancelEmptyChore()
    {
      ...
      this.smi.master.remoteChore.SetChore((Chore) null);
    }
