...
public class Scenario : KMonoBehaviour
{
  ...
  {
    SimMessages.ReplaceElement(Grid.OffsetCell(this.RootCell, x, y), id, CellEventLogger.Instance.Scenario, 10000f);
  }
  ...
  {
    SimMessages.ReplaceElement(Grid.OffsetCell(this.RootCell, x, y), SimHashes.Oxygen, CellEventLogger.Instance.Scenario, 10000f);
  }
    ...
    Grid.SceneLayer scene_layer = Grid.SceneLayer.Ore)
  {
    ...
    return (UnityEngine.Object) prefab == (UnityEngine.Object) null ? (GameObject) null : GameUtil.KInstantiate(prefab, Grid.CellToPosCBC(cell, scene_layer), scene_layer);
  }
  ...
  {
    this.InitDebugScenario();
    int num1 = Mathf.Min(64 /*0x40*/, Grid.WidthInCells);
    int num2 = Mathf.Min(64 /*0x40*/, Grid.HeightInCells);
