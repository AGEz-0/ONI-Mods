...
public class GameScenePartitioner : KMonoBehaviour
{
  ...
  protected override void OnSpawn()
  {
    ...
    navGrid.OnNavGridUpdateComplete += new Action<IEnumerable<int>>(this.OnNavGridUpdateComplete);
    ...
    where IteratorType : GameScenePartitioner.Iterator
  {
    ...
    {
      ScenePartitionerEntry partitionerEntry = gathered_entries[index];
  ...
  public IEnumerable<object> AsyncSafeEnumerate(int cell, int radius, ScenePartitionerLayer layer)
  {
    ...
    int y = 0;
    Grid.CellToXY(cell, out x, out y);
    return this.AsyncSafeEnumerate(x - radius, y - radius, radius * 2, radius * 2, layer);
  }
    ...
    ContextType context)
  {
    ...
    int y = 0;
    Grid.CellToXY(cell, out x, out y);
    this.AsyncSafeVisit<ContextType>(x - radius, y - radius, radius * 2, radius * 2, layer, visitor, context);
  }
  ...
  private void OnNavGridUpdateComplete(IEnumerable<int> dirty_nav_cells)
  {
    ...
    if (this.changedCells.Count <= 0)
      return;
    GameScenePartitioner.Instance.TriggerEvent((IEnumerable<int>) this.changedCells, GameScenePartitioner.Instance.validNavCellChangedLayer, (object) null);
