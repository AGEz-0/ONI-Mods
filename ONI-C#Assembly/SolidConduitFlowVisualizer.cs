...
public class SolidConduitFlowVisualizer
{
  ...
  {
    return this.showContents ? Color32.Lerp(GlobalAssets.Instance.colorSet.GetColorByName(this.visInfo.overlayTintName), GlobalAssets.Instance.colorSet.GetColorByName(this.visInfo.overlayInsulatedTintName), insulation_lerp) : Color32.Lerp(this.visInfo.tint, this.visInfo.insulatedTint, insulation_lerp);
  }
      ...
      for (int idx = 0; idx < this.flowManager.GetSOAInfo().NumEntries; ++idx)
      {
        ...
        if (!(xy1 < vector2I1) && !(xy1 > vector2I2))
        {
          ...
          if (flag)
          {
            ...
            Color backgroundColor = (Color) this.GetBackgroundColor(Mathf.Lerp(this.insulatedCells.Contains(cell) ? 1f : 0.0f, this.insulatedCells.Contains(cellFromDirection) ? 1f : 0.0f, lerp_percent));
          ...
          {
            int cell = conduit.GetCell(this.flowManager);
            Vector2 xy4 = (Vector2) Grid.CellToXY(cell);
            ...
            if (this.showContents)
            {
              this.staticBallMesh.AddQuad(xy4, (Color32) backgroundColor, this.tuning.size * num, 0.0f, 0.0f, uvbl, uvtl, uvbr, uvtr);
            }
            else
            {
              element = (Element) null;
            ...
            Color32 contentsColor = this.GetContentsColor(element, (Color32) backgroundColor);
            this.staticBallMesh.AddQuad(xy4, contentsColor, this.tuning.size * num, 1f, highlight, uvbl, uvtl, uvbr, uvtr);
          }
        }
      }
