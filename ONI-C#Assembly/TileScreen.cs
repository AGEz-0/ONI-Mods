...
public class TileScreen : KScreen
{
  ...
  {
    int cell = Grid.PosToCell(Camera.main.ScreenToWorldPoint(KInputManager.GetMousePos() with
    {
    ...
    if (Grid.IsValidCell(cell) && Grid.IsVisible(cell))
    {
      ...
      this.massAmtLabel.text = $"{num1:0.0} {str}";
      ...
      this.temperatureSliderText.text = GameUtil.GetFormattedTemperature((float) (int) num2);
      ...
      foreach (KeyValuePair<int, float> keyValuePair in info)
      {
        ...
        nameLabel.text = $"{nameLabel.text}\n{element2.name}{$" {keyValuePair.Value:0.00} kg"}";
      }
    }
    else
  ...
  private void DisplayConduitFlowInfo()
  {
    ...
    if (Grid.IsValidCell(cell) && utilityNetworkManager.GetConnections(cell, true) != (UtilityConnections) 0)
    {
      ...
      this.massAmtLabel.text = $"{mass:0.0} {str}";
      this.massTitleLabel.text = "mass";
