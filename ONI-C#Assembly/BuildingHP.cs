...
public class BuildingHP : Workable
{
  ...
  private void DestroyOnDamaged(object data) => Util.KDestroyGameObject(this.gameObject);
  ...
  private void DoDamagePopFX(BuildingHP.DamageSourceInfo info)
  {
    if (info.popString == null || (double) Time.time <= (double) this.lastPopTime + (double) this.minDamagePopInterval)
      return;
    PopFXManager.Instance.SpawnFX(PopFXManager.Instance.sprite_Building, info.popString, this.gameObject.transform);
  ...
  public class SMInstance(BuildingHP master) : 
    GameStateMachine<BuildingHP.States, BuildingHP.SMInstance, BuildingHP, object>.GameInstance(master)
  {
    private ProgressBar progressBar;

    public Notification CreateBrokenMachineNotification()
    {
      return new Notification((string) MISC.NOTIFICATIONS.BROKENMACHINE.NAME, NotificationType.BadMinor, (Func<List<Notification>, object, string>) ((notificationList, data) => (string) MISC.NOTIFICATIONS.BROKENMACHINE.TOOLTIP + notificationList.ReduceMessages(false)), (object) ("/t• " + this.master.damageSourceInfo.source), false);
    }
    ...
    private void CreateProgressBar()
    {
      if ((UnityEngine.Object) this.progressBar != (UnityEngine.Object) null)
        return;
      this.progressBar = Util.KInstantiateUI<ProgressBar>(ProgressBarsConfig.Instance.progressBarPrefab);
      ...
      this.progressBar.Update();
      float num = 0.15f;
      Vector3 vector3 = this.gameObject.transform.GetPosition() + Vector3.down * num;
      vector3.z += 0.05f;
  ...
  public class States : GameStateMachine<BuildingHP.States, BuildingHP.SMInstance, BuildingHP>
  {
    ...
    {
      return (Chore) new WorkChore<BuildingHP>(Db.Get().ChoreTypes.Repair, (IStateMachineTarget) smi.master, only_when_operational: false);
    }
