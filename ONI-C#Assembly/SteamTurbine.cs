...
public class SteamTurbine : Generator
{
  ...
  protected override void OnSpawn()
  {
    ...
    for (int index = 0; index < def.WidthInCells; ++index)
    {
      ...
      this.srcCells[index] = Grid.OffsetCell(cell, new CellOffset(x, -2));
    }
  ...
  public override void EnergySim200ms(float dt)
  {
    ...
    {
      float num1 = 0.0f;
      if ((UnityEngine.Object) this.gasStorage != (UnityEngine.Object) null && this.gasStorage.items.Count > 0)
      {
      ...
      this.meter.SetSymbolTint((KAnimHashedString) SteamTurbine.TINT_SYMBOL, (Color32) Color.Lerp(Color.red, Color.green, Game.Instance.accumulators.GetAverageRate(this.accumulator) / this.WattageRating));
    }
  }
  ...
  {
    return this.WattageRating * (float) Math.Pow(((double) steam.Temperature - (double) this.outputElementTemperature) / ((double) this.idealSourceElementTemperature - (double) this.outputElementTemperature), 1.0);
  }
  ...
  public class States : GameStateMachine<SteamTurbine.States, SteamTurbine.Instance, SteamTurbine>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.inoperational.EventTransition(GameHashes.OperationalChanged, this.operational.active, (StateMachine<SteamTurbine.States, SteamTurbine.Instance, SteamTurbine, object>.Transition.ConditionCallback) (smi => smi.master.GetComponent<Operational>().IsOperational)).QueueAnim("off");
      this.operational.DefaultState(this.operational.active).EventTransition(GameHashes.OperationalChanged, this.inoperational, (StateMachine<SteamTurbine.States, SteamTurbine.Instance, SteamTurbine, object>.Transition.ConditionCallback) (smi => !smi.master.GetComponent<Operational>().IsOperational)).Update("UpdateOperational", (Action<SteamTurbine.Instance, float>) ((smi, dt) => smi.UpdateState(dt))).Exit((StateMachine<SteamTurbine.States, SteamTurbine.Instance, SteamTurbine, object>.State.Callback) (smi => smi.DisableStatusItems()));
      ...
      this.operational.tooHot.Enter((StateMachine<SteamTurbine.States, SteamTurbine.Instance, SteamTurbine, object>.State.Callback) (smi => smi.GetComponent<KAnimControllerBase>().Play(SteamTurbine.States.TOOHOT_ANIMS, KAnim.PlayMode.Loop)));
    }
  ...
  public class Instance(SteamTurbine master) : 
    GameStateMachine<SteamTurbine.States, SteamTurbine.Instance, SteamTurbine, object>.GameInstance(master)
  {
    ...
    private bool CanSteamFlow(ref bool insufficient_mass, ref bool insufficient_temperature)
    {
      ...
      insufficient_mass = (double) a1 < (double) this.master.requiredMass;
