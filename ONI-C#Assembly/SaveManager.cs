...
public class SaveManager : KMonoBehaviour
{
  ...
  public const int SAVE_MINOR_VERSION_NEW_AUTOMATION_WARNING = 16 /*0x10*/;
  ...
  public const int SAVE_MINOR_VERSION_GEYSER_CAN_BE_RENAMED = 31 /*0x1F*/;
  public const int SAVE_MINOR_VERSION_SPACE_SCANNERS_TELESCOPES = 32 /*0x20*/;
  ...
  {
    int index = objectTags.FindIndex((Predicate<Tag>) (t => associatedTags.Contains(t)));
    if (index < 0)
      return;
    Tag tag = objectTags[index];
    foreach (Tag associatedTag in associatedTags)
    {
      if (associatedTag != tag && objectTags.Contains(associatedTag))
      {
        objectTags.Remove(associatedTag);
        objectTags.Insert(index + 1, associatedTag);
      }
    }
  }
  ...
  public void Save(BinaryWriter writer)
  {
    ...
    foreach (KeyValuePair<Tag, List<SaveLoadRoot>> sceneObject in this.sceneObjects)
    {
      ...
      {
        ++num;
        if (sceneObject.Value[0].associatedTag != Tag.Invalid)
        {
          if (!dictionary.ContainsKey(sceneObject.Value[0].associatedTag))
            dictionary.Add(sceneObject.Value[0].associatedTag, new List<Tag>());
          dictionary[sceneObject.Value[0].associatedTag].Add(sceneObject.Key);
        }
      }
    }
    writer.Write(num);
    ...
    this.orderedKeys = this.orderedKeys.OrderBy<Tag, bool>((Func<Tag, bool>) (a => a.Name == "StickerBomb")).ToList<Tag>();
    this.orderedKeys = this.orderedKeys.OrderBy<Tag, bool>((Func<Tag, bool>) (a => a.Name.Contains("UnderConstruction"))).ToList<Tag>();
    foreach (KeyValuePair<Tag, List<Tag>> keyValuePair in dictionary)
      this.SortAssociatedObjects(ref this.orderedKeys, keyValuePair.Value);
  ...
  public bool Load(IReader reader)
  {
    ...
    if (num1 != 7 || num2 > 36)
    {
      DebugUtil.LogWarningArgs((object) $"SAVE FILE VERSION MISMATCH! Expected {7}.{36} but got {num1}.{num2}");
      return false;
    }
    this.ClearScene();
    try
      ...
      for (int index1 = 0; index1 < num3; ++index1)
      {
        ...
        else
        {
          ...
          for (int index2 = 0; index2 < capacity; ++index2)
          {
            ...
            if (SaveManager.DEBUG_OnlyLoadThisCellsObjects == -1 && (UnityEngine.Object) saveLoadRoot == (UnityEngine.Object) null)
            {
