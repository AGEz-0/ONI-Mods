...
public class RoleStation : Workable, IGameObjectEffectDescriptor
{
  ...
  {
    return (Chore) new WorkChore<RoleStation>(Db.Get().ChoreTypes.LearnSkill, (IStateMachineTarget) this, allow_in_red_alert: false, override_anims: Assets.GetAnim((HashedString) "anim_hat_kanim"), allow_prioritization: false, priority_class: PriorityScreen.PriorityClass.personalNeeds, add_to_daily_report: false);
  }
    ...
    GameStateMachine<RoleStation.RoleStationSM, RoleStation.RoleStationSM.Instance, RoleStation>
  {
    ...
    public new class Instance(RoleStation master) : 
      GameStateMachine<RoleStation.RoleStationSM, RoleStation.RoleStationSM.Instance, RoleStation, object>.GameInstance(master)
    {
    }
  }
}
