...
public class TrackerTool : KMonoBehaviour
{
  ...
  private void AddNewWorldTrackers(int worldID)
  {
    ...
    if (Game.IsDlcActiveForCurrentSave("DLC3_ID"))
      this.worldTrackers.Add((WorldTracker) new ElectrobankJoulesTracker(worldID));
    ...
    {
      this.worldTrackers.Add((WorldTracker) new WorkTimeTracker(worldID, Db.Get().ChoreGroups[idx]));
      this.worldTrackers.Add((WorldTracker) new ChoreCountTracker(worldID, Db.Get().ChoreGroups[idx]));
    }
    ...
    foreach (EdiblesManager.FoodInfo allFoodType in EdiblesManager.GetAllFoodTypes())
      this.AddResourceTracker(worldID, (Tag) allFoodType.Id);
  ...
  {
    if (this.worldTrackers.Find((Predicate<WorldTracker>) (match => match is ResourceTracker && match.WorldID == worldID && ((ResourceTracker) match).tag == tag)) != null)
      return;
  ...
  {
    return (ResourceTracker) this.worldTrackers.Find((Predicate<WorldTracker>) (match => match is ResourceTracker && match.WorldID == worldID && ((ResourceTracker) match).tag == tag));
  }

  public WorldTracker GetWorldTracker<T>(int worldID) where T : WorldTracker
  {
    return this.worldTrackers.Find((Predicate<WorldTracker>) (match => match is T obj && obj.WorldID == worldID));
  }

  public ChoreCountTracker GetChoreGroupTracker(int worldID, ChoreGroup choreGroup)
  {
    return (ChoreCountTracker) this.worldTrackers.Find((Predicate<WorldTracker>) (match => match is ChoreCountTracker && match.WorldID == worldID && ((ChoreCountTracker) match).choreGroup == choreGroup));
  }

  public WorkTimeTracker GetWorkTimeTracker(int worldID, ChoreGroup choreGroup)
  {
    return (WorkTimeTracker) this.worldTrackers.Find((Predicate<WorldTracker>) (match => match is WorkTimeTracker && match.WorldID == worldID && ((WorkTimeTracker) match).choreGroup == choreGroup));
  }

  public MinionTracker GetMinionTracker<T>(MinionIdentity identity) where T : MinionTracker
  {
    return this.minionTrackers[identity].Find((Predicate<MinionTracker>) (match => match is T));
  }
