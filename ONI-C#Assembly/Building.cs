  ...
  IApproachable
{
  ...
  public Orientation Orientation
  {
    get
    {
      return !((UnityEngine.Object) this.rotatable != (UnityEngine.Object) null) ? Orientation.Neutral : this.rotatable.GetOrientation();
    }
  }
  ...
  public bool PlacementCellsContainCell(int cell)
  {
    ...
    {
      if (this.PlacementCells[index] == cell)
        return true;
    }
    return false;
  }
  ...
  public static void CreateBuildingMeltedNotification(GameObject building)
  {
    ...
    Notifier cmp = building.AddOrGet<Notifier>();
    Notification notification = new Notification((string) MISC.NOTIFICATIONS.BUILDING_MELTED.NAME, NotificationType.Bad, (Func<List<Notification>, object, string>) ((notificationList, data) => (string) MISC.NOTIFICATIONS.BUILDING_MELTED.TOOLTIP + notificationList.ReduceMessages(false)), (object) ("/t• " + cmp.GetProperName()), custom_click_callback: (Notification.ClickCallback) (o => GameUtil.FocusCamera(pos)), clear_on_click: true);
    cmp.Add(notification);
  }

  public void SetDescription(string desc) => this.description = desc;

  public string Desc => this.Def.Desc;

  public string DescFlavour => this.descriptionFlavour;

  public string DescEffect => this.Def.Effect;
  ...
  protected override void OnSpawn()
  {
    ...
    if (this.GetComponent<KPrefabID>().HasTag(RoomConstraints.ConstraintTags.IndustrialMachinery))
      this.scenePartitionerEntry = GameScenePartitioner.Instance.Add(this.name, (object) this.gameObject, this.GetExtents(), GameScenePartitioner.Instance.industrialBuildings, (Action<object>) null);
  ...
  {
    return !((UnityEngine.Object) this.rotatable != (UnityEngine.Object) null) ? offset : this.rotatable.GetRotatedCellOffset(offset);
  }

  public int GetBottomLeftCell() => Grid.PosToCell(this.transform.GetPosition());
  ...
  public List<Descriptor> RequirementDescriptors(BuildingDef def)
  {
    ...
    if (component1.Def.RequiredSkillPerkID != null)
    {
      Descriptor descriptor = new Descriptor();
      string replacement = GameUtil.NamesOfSkillsWithSkillPerk(component1.Def.RequiredSkillPerkID);
      if (Game.IsDlcActiveForCurrentSave("DLC3_ID"))
      {
        string tooltip = UI.BUILDINGEFFECTS.TOOLTIPS.REQUIRESSKILLEDOPERATION_DLC3.Replace("{Skill}", replacement).Replace("{Booster}", GameUtil.NamesOfBoostersWithSkillPerk(component1.Def.RequiredSkillPerkID));
        descriptor.SetupDescriptor((string) UI.BUILDINGEFFECTS.REQUIRESSKILLEDOPERATION_DLC3, tooltip, Descriptor.DescriptorType.Requirement);
      }
      else
      {
        string tooltip = UI.BUILDINGEFFECTS.TOOLTIPS.REQUIRESSKILLEDOPERATION.Replace("{Skill}", replacement);
        descriptor.SetupDescriptor((string) UI.BUILDINGEFFECTS.REQUIRESSKILLEDOPERATION, tooltip, Descriptor.DescriptorType.Requirement);
      }
      descriptorList.Add(descriptor);
    }
