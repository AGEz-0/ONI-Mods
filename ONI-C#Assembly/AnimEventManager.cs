...
public class AnimEventManager : Singleton<AnimEventManager>
{
  ...
  private const int INITIAL_VECTOR_SIZE = 256 /*0x0100*/;
  private KCompactedVector<AnimEventManager.AnimData> animData = new KCompactedVector<AnimEventManager.AnimData>(256 /*0x0100*/);
  private KCompactedVector<AnimEventManager.EventPlayerData> eventData = new KCompactedVector<AnimEventManager.EventPlayerData>(256 /*0x0100*/);
  private KCompactedVector<AnimEventManager.AnimData> uiAnimData = new KCompactedVector<AnimEventManager.AnimData>(256 /*0x0100*/);
  private KCompactedVector<AnimEventManager.EventPlayerData> uiEventData = new KCompactedVector<AnimEventManager.EventPlayerData>(256 /*0x0100*/);
  private KCompactedVector<AnimEventManager.IndirectionData> indirectionData = new KCompactedVector<AnimEventManager.IndirectionData>();
    ...
    List<AnimEventManager.EventPlayerData> event_data)
  {
    ...
    for (int index1 = 0; index1 < event_data.Count; ++index1)
    {
      ...
      if (!((Object) eventPlayerData.controller == (Object) null) && eventPlayerData.mode != KAnim.PlayMode.Paused)
      {
  ...
  public struct EventPlayerData
  {
    ...
    public string name => this.controller.name;

    public float normalizedTime => this.elapsedTime / this.controller.CurrentAnim.totalTime;

    public Vector3 position => this.controller.transform.GetPosition();
  ...
  private struct IndirectionData(
    ...
    bool is_ui_data)
  {
    public bool isUIData = is_ui_data;
    public HandleVector<int>.Handle animDataHandle = anim_data_handle;
    public HandleVector<int>.Handle eventDataHandle = event_data_handle;
  }
  ...
  {
    public readonly AnimEventManager eventManager = eventManager;
    public readonly KCompactedVector<AnimEventManager.AnimData> animData = animData;
    public readonly KCompactedVector<AnimEventManager.EventPlayerData> eventData = eventData;
    public readonly KCompactedVector<AnimEventManager.AnimData> uiAnimData = uiAnimData;
    public readonly KCompactedVector<AnimEventManager.EventPlayerData> uiEventData = uiEventData;
  }
}
