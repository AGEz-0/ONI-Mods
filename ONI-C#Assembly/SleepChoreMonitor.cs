...
public class SleepChoreMonitor : GameStateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.checkforbed.Enter("SetBed", (StateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance, IStateMachineTarget, object>.State.Callback) (smi =>
    {
      ...
      {
        if ((UnityEngine.Object) this.bed.Get(smi) != (UnityEngine.Object) null && smi.IsBedReachable())
          smi.GoTo((StateMachine.BaseState) this.passingout_bedassigned);
        else
    ...
    this.passingout.EventTransition(GameHashes.AssignablesChanged, this.checkforbed).EventHandlerTransition(GameHashes.AssignableReachabilityChanged, this.checkforbed, (Func<SleepChoreMonitor.Instance, object, bool>) ((smi, data) => smi.IsBedReachable())).ToggleChore(new Func<SleepChoreMonitor.Instance, Chore>(this.CreatePassingOutChore), this.satisfied, this.satisfied);
    this.passingout_bedassigned.ParamTransition<GameObject>((StateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance, IStateMachineTarget, object>.Parameter<GameObject>) this.bed, this.checkforbed, (StateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance, IStateMachineTarget, object>.Parameter<GameObject>.Callback) ((smi, p) => (UnityEngine.Object) p == (UnityEngine.Object) null)).EventTransition(GameHashes.AssignablesChanged, this.checkforbed).EventTransition(GameHashes.AssignableReachabilityChanged, this.checkforbed, (StateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance, IStateMachineTarget, object>.Transition.ConditionCallback) (smi => !smi.IsBedReachable())).ToggleChore(new Func<SleepChoreMonitor.Instance, Chore>(this.CreateExhaustedSleepChore), this.satisfied, this.satisfied);
  ...
  {
    return (Chore) new SleepChore(Db.Get().ChoreTypes.Sleep, smi.master, this.bed.Get(smi), false, true, new StatusItem[1]
    {
  ...
  public new class Instance(IStateMachineTarget master) : 
    GameStateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance, IStateMachineTarget, object>.GameInstance(master)
  {
