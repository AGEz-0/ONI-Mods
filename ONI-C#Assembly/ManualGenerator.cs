...
public class ManualGenerator : RemoteWorkable, ISingleSliderControl, ISliderControl
{
  ...
  private static readonly EventSystem.IntraObjectHandler<ManualGenerator> OnCopySettingsDelegate = new EventSystem.IntraObjectHandler<ManualGenerator>((Action<ManualGenerator, object>) ((component, data) => component.OnCopySettings(data)));
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.Subscribe<ManualGenerator>(-905833192, ManualGenerator.OnCopySettingsDelegate);
  ...
  protected void OnActiveChanged(object is_active)
  {
    if (!this.operational.IsActive)
      return;
    this.GetComponent<KSelectable>().SetStatusItem(Db.Get().StatusItemCategories.Power, Db.Get().BuildingStatusItems.ManualGeneratorChargingUp);
  }
    ...
    GameStateMachine<ManualGenerator.GeneratePowerSM, ManualGenerator.GeneratePowerSM.Instance>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.working.loop.PlayAnim("working_loop", KAnim.PlayMode.Loop).EventTransition(GameHashes.ActiveChanged, this.off, (StateMachine<ManualGenerator.GeneratePowerSM, ManualGenerator.GeneratePowerSM.Instance, IStateMachineTarget, object>.Transition.ConditionCallback) (smi => (UnityEngine.Object) this.masterTarget.Get(smi) != (UnityEngine.Object) null && !smi.master.GetComponent<Operational>().IsActive));
    }
    ...
    public new class Instance(IStateMachineTarget master) : 
      GameStateMachine<ManualGenerator.GeneratePowerSM, ManualGenerator.GeneratePowerSM.Instance, IStateMachineTarget, object>.GameInstance(master)
    {
    }
  }
}
