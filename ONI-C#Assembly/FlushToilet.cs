...
public class FlushToilet : 
  ...
  IBasicBuilding
{
  private static readonly HashedString[] CLOGGED_ANIMS = new HashedString[2]
  {
  ...
  protected override void OnSpawn()
  {
    ...
    this.gunkMeter = new MeterController((KAnimControllerBase) component2, "meter_target", "meter_gunky", this.meterOffset, Grid.SceneLayer.NoLayer, new Vector3(0.4f, 3.2f, 0.1f), Array.Empty<string>());
  ...
  {
    ToiletWorkableUse component1 = this.GetComponent<ToiletWorkableUse>();
    ...
    foreach (GameObject gameObject in (List<GameObject>) result)
    {
      ...
      float num2 = Mathf.Min(component2.Mass, massConsumedPerUse);
      component2.Mass -= num2;
      massConsumedPerUse -= num2;
      num1 += num2 * component2.Temperature;
    }
    result.Recycle();
    float massRemovedFromDupe = component1.lastAmountOfWasteMassRemovedFromDupe;
    float num3 = num1 + massRemovedFromDupe * this.newPeeTemperature;
    float mass = this.massConsumedPerUse + massRemovedFromDupe;
    float temperature = num3 / mass;
    byte index = Db.Get().Diseases.GetIndex((HashedString) this.diseaseId);
    this.storage.AddLiquid(component1.lastElementRemovedFromDupe, mass, temperature, index, this.diseasePerFlush);
  }
    ...
    GameStateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.GameInstance
  {
    ...
    public bool IsClogged => this.sm.isClogged.Get(this);
    ...
    public void CreateCleanChore()
    {
      ...
      ToiletWorkableClean component = this.GetComponent<ToiletWorkableClean>();
    ...
    private void OnCleanComplete(object o) => this.sm.isClogged.Set(false, this);
    ...
    {
      bool flag = this.GetComponent<ToiletWorkableUse>().last_user_id == (Tag) BionicMinionConfig.ID;
      this.SetDirtyStateMeterPercentage(flag ? 0.0f : 1f, flag ? 1f : 0.0f);
    }

    public void SetDirtyStateMeterPercentage(float contaminationPercentage, float gunkPercentage)
    {
      this.master.contaminationMeter.SetPositionPercent(contaminationPercentage);
      this.master.gunkMeter.SetPositionPercent(gunkPercentage);
    }

    public void UpdateDirtyState()
    {
      ToiletWorkableUse component = this.GetComponent<ToiletWorkableUse>();
      float percentComplete = component.GetPercentComplete();
      bool flag = component.last_user_id == (Tag) BionicMinionConfig.ID;
      this.SetDirtyStateMeterPercentage(flag ? 0.0f : percentComplete, flag ? percentComplete : 0.0f);
    }

    public void AddDisseaseToWorker()
    {
      this.master.AddDisseaseToWorker(this.master.GetComponent<ToiletWorkableUse>().worker);
    }
    ...
    public void Flush()
    {
      ...
      this.master.contaminationMeter.SetPositionPercent(flag ? 0.0f : 1f);
      this.master.gunkMeter.SetPositionPercent(flag ? 1f : 0.0f);
      this.smi.ShowFillMeter();
    ...
    public void ShowFillMeter()
    {
      ...
      this.master.gunkMeter.gameObject.SetActive(false);
    }
    ...
    public bool HasContaminatedMass()
    {
      ...
      {
        PrimaryElement component = gameObject.GetComponent<PrimaryElement>();
        if (!((UnityEngine.Object) component == (UnityEngine.Object) null) && (component.ElementID == SimHashes.DirtyWater || component.ElementID == GunkMonitor.GunkElement) && (double) component.Mass > 0.0)
          return true;
      }
      return false;
    }
    ...
    public void ShowContaminatedMeter()
    {
      ...
      this.master.contaminationMeter.gameObject.SetActive(!flag);
      this.master.gunkMeter.gameObject.SetActive(flag);
    }
  }
  ...
  public class States : GameStateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.ready.DefaultState(this.ready.idle).ToggleTag(GameTags.Usable).Enter((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback) (smi =>
      {
        ...
        smi.master.gunkMeter.SetPositionPercent(0.0f);
      })).PlayAnim("on").EventHandler(GameHashes.FlushGunk, new GameStateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.GameEvent.Callback(this.OnFlushedGunk)).EventTransition(GameHashes.ConduitConnectionChanged, this.disconnected, (StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.Transition.ConditionCallback) (smi => !smi.HasValidConnections())).ParamTransition<bool>((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.Parameter<bool>) this.outputBlocked, this.backedup, GameStateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.IsTrue).ToggleRecurringChore(new Func<FlushToilet.SMInstance, Chore>(this.CreateUrgentUseChore)).ToggleRecurringChore(new Func<FlushToilet.SMInstance, Chore>(this.CreateBreakUseChore));
      this.ready.idle.ParamTransition<bool>((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.Parameter<bool>) this.isClogged, this.clogged, GameStateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.IsTrue).Enter((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback) (smi => smi.GetComponent<Operational>().SetActive(false))).ToggleMainStatusItem(Db.Get().BuildingStatusItems.FlushToilet).WorkableStartTransition((Func<FlushToilet.SMInstance, Workable>) (smi => (Workable) smi.master.GetComponent<ToiletWorkableUse>()), this.ready.inuse);
      this.ready.inuse.Enter((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback) (smi => smi.ShowContaminatedMeter())).ToggleMainStatusItem(Db.Get().BuildingStatusItems.FlushToiletInUse).Update((Action<FlushToilet.SMInstance, float>) ((smi, dt) => smi.UpdateDirtyState())).WorkableCompleteTransition((Func<FlushToilet.SMInstance, Workable>) (smi => (Workable) smi.master.GetComponent<ToiletWorkableUse>()), this.ready.completed).WorkableStopTransition((Func<FlushToilet.SMInstance, Workable>) (smi => (Workable) smi.master.GetComponent<ToiletWorkableUse>()), this.flushed);
      this.ready.completed.Enter((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback) (smi => smi.AddDisseaseToWorker())).EnterTransition(this.clogged, (StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.Transition.ConditionCallback) (smi => smi.IsClogged)).EnterGoTo(this.flushing);
      this.clogged.PlayAnims((Func<FlushToilet.SMInstance, HashedString[]>) (smi => FlushToilet.CLOGGED_ANIMS)).Enter((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback) (smi => smi.ShowContaminatedMeter())).Enter(new StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback(this.SetDirtyStatesForClogged)).Enter(new StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback(this.CreateCleanChore)).Exit(new StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.State.Callback(this.CancelCleanChore)).ParamTransition<bool>((StateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.Parameter<bool>) this.isClogged, this.unclogged, GameStateMachine<FlushToilet.States, FlushToilet.SMInstance, FlushToilet, object>.IsFalse);
    ...
    public void CreateCleanChore(FlushToilet.SMInstance smi) => smi.CreateCleanChore();

    public void CancelCleanChore(FlushToilet.SMInstance smi) => smi.CancelCleanChore();
