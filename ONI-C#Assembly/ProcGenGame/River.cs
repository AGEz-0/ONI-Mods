...
public class River(ProcGen.River other) : ProcGen.River(other), SymbolicMapElement
{
    ...
    SeededRandom rnd)
  {
    ...
    for (int index1 = 0; index1 < this.pathElements.Count; ++index1)
    {
      ...
      for (int index2 = 0; index2 < line.Count; ++index2)
      {
        ...
        {
          Vector2 vector2_4 = (Vector2) line[index2] + normalized * (this.widthCenter + num);
          ...
          Vector2 vector2_5 = (Vector2) line[index2] - normalized * (this.widthCenter + num);
          int cell4 = Grid.XYToCell((int) vector2_5.x, (int) vector2_5.y);
  ...
  {
    return new River(rivers.Find((Predicate<ProcGen.River>) (river => Grid.PosToCell(river.SourcePosition()) == cell || Grid.PosToCell(river.SinkPosition()) == cell)));
  }
