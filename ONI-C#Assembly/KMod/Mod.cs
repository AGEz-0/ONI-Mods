...
public class Mod : IHasDlcRestrictions
{
  ...
  public string title
  {
    get => this.label.title;
    set => this.label.title = value;
  }
  ...
  {
    this.ModDevLog($"{this.label} ({this.label.id}): Setting up mod.");
    this.available_content = (Content) 0;
    if (this.file_source == null)
    {
      if (this.label.id.EndsWith(".zip"))
      {
        DebugUtil.DevAssert(false, "Does this actually get used ever?");
    ...
    {
      Debug.LogWarning((object) $"{this.label}: File source does not appear to be valid, skipping. ({this.label.install_path})");
    }
    else
    {
      ...
      {
        Debug.LogWarning((object) $"{this.label}: No archive supports this game version, skipping content.");
      ...
      else
      {
        ...
        {
          Debug.LogWarning((object) $"{this.label}: No supported content for mod, skipping content.");
        ...
        else
        {
          ...
          {
            Debug.LogWarning((object) $"{this.label}: DLLs found but not using the correct API version.");
          ...
          else
          {
            ...
            Debug.Log((object) $"{this.label}: Successfully loaded from path '{(string.IsNullOrEmpty(this.relative_root) ? (object) "root" : (object) this.relative_root)}' with content '{this.available_content.ToString()}'.");
          }
        }
      }
    }
  ...
  {
    Mod.PackagedModInfo restrictions = this.GetModInfoForFolder("");
    if (restrictions == null)
    {
      if (this.ScanContentFromSourceForTranslationsOnly(""))
      {
        this.ModDevLogWarning($"{this.label}: No mod_info.yaml found, but since it's a translation we will load it.");
        restrictions = new Mod.PackagedModInfo()
        {
      ...
      {
        Debug.Log((object) $"{this.label}: Is missing a mod_info.yaml file and will not be loaded, which is required. See the stickied post in the Mods and Tools section on the Klei forums.");
        return (Mod.ArchivedVersion) null;
      }
    }
    this.requiredDlcIds = restrictions.requiredDlcIds;
    this.forbiddenDlcIds = restrictions.forbiddenDlcIds;
    Mod.ArchivedVersion archivedVersion1 = new Mod.ArchivedVersion()
    {
      relativePath = "",
      info = restrictions
    };
    ...
    {
      this.ModDevLog($"\t{this.label}: No archived_versions for this mod, using root version directly.");
      return !DlcManager.IsCorrectDlcSubscribed((IHasDlcRestrictions) restrictions) ? (Mod.ArchivedVersion) null : archivedVersion1;
    }
    ...
    Mod.ArchivedVersion archivedVersion2 = source.Where<Mod.ArchivedVersion>((Func<Mod.ArchivedVersion, bool>) (v => DlcManager.IsCorrectDlcSubscribed((IHasDlcRestrictions) v.info))).ToList<Mod.ArchivedVersion>().Where<Mod.ArchivedVersion>((Func<Mod.ArchivedVersion, bool>) (v => v.info.APIVersion == 2 || v.info.APIVersion == 0)).ToList<Mod.ArchivedVersion>().Where<Mod.ArchivedVersion>((Func<Mod.ArchivedVersion, bool>) (v => v.info.minimumSupportedBuild <= 679336)).OrderByDescending<Mod.ArchivedVersion, int>((Func<Mod.ArchivedVersion, int>) (v => v.info.minimumSupportedBuild)).FirstOrDefault<Mod.ArchivedVersion>();
    ...
    return archivedVersion2 ?? (Mod.ArchivedVersion) null;
  }
  ...
  private Mod.PackagedModInfo GetModInfoForFolder(string relative_root)
  {
    ...
    string str = string.IsNullOrEmpty(relative_root) ? "root" : relative_root;
    if (!flag1)
    {
      this.ModDevLogWarning($"\t{this.title}: has no mod_info.yaml in folder '{str}'");
      return (Mod.PackagedModInfo) null;
    }
    string readText = this.file_source.Read(System.IO.Path.Combine(relative_root, "mod_info.yaml"));
    if (string.IsNullOrEmpty(readText))
    {
      this.ModDevLogError($"\t{this.label}: Failed to read {"mod_info.yaml"} in folder '{str}', skipping");
      return (Mod.PackagedModInfo) null;
    }
    YamlIO.ErrorHandler handle_error = (YamlIO.ErrorHandler) ((e, force_warning) => YamlIO.LogError(e, !this.IsDev));
    Mod.PackagedModInfo modInfoForFolder = YamlIO.Parse<Mod.PackagedModInfo>(readText, new FileHandle(), handle_error);
    if (modInfoForFolder == null)
    {
      this.ModDevLogError($"\t{this.label}: Failed to parse {"mod_info.yaml"} in folder '{str}', text is {readText}");
      return (Mod.PackagedModInfo) null;
    }
    if (modInfoForFolder.supportedContent != null && modInfoForFolder.requiredDlcIds == null && modInfoForFolder.forbiddenDlcIds == null)
    {
      modInfoForFolder.supportedContent = modInfoForFolder.supportedContent.ToUpperInvariant();
      this.ModDevLogWarning($"\t{this.label}: {"mod_info.yaml"} in folder '{str}' is using supportedContent which has been deprecated. See stickied post on the Klei forums.");
      int num = modInfoForFolder.supportedContent.Contains("ALL") ? 1 : 0;
      bool flag2 = modInfoForFolder.supportedContent.Contains("VANILLA_ID");
      bool flag3 = modInfoForFolder.supportedContent.Contains("EXPANSION1_ID");
      if (num != 0)
      {
        modInfoForFolder.requiredDlcIds = (string[]) null;
        modInfoForFolder.forbiddenDlcIds = (string[]) null;
      }
      else
      {
        string pattern = "\\b\\w+_ID\\b";
        List<string> stringList = new List<string>();
        foreach (System.Text.RegularExpressions.Match match in Regex.Matches(modInfoForFolder.supportedContent, pattern))
        {
          if (!(match.Value == "VANILLA_ID") && !(match.Value == "EXPANSION1_ID" & flag2))
          {
            if (match.Value != "EXPANSION1_ID")
              this.ModDevLogWarning($"\t{this.label}: {"mod_info.yaml"} in folder '{str}' found a DLC '{match.Value}' it didn't recognize, ignoring.");
            else
              stringList.Add(match.Value);
          }
        }
        if (stringList.Count > 0)
          modInfoForFolder.requiredDlcIds = stringList.ToArray();
        if (!flag3)
          modInfoForFolder.forbiddenDlcIds = DlcManager.EXPANSION1;
      }
    }
    if (modInfoForFolder.requiredDlcIds != null)
    {
      for (int index = 0; index < modInfoForFolder.requiredDlcIds.Length; ++index)
      {
        modInfoForFolder.requiredDlcIds[index] = modInfoForFolder.requiredDlcIds[index].ToUpperInvariant();
        if (!DlcManager.IsDlcId(modInfoForFolder.requiredDlcIds[index]))
          this.ModDevLogWarning($"\t{this.label}: {"mod_info.yaml"} in folder '{str}' is using an unrecognized DLC in requiredDlcIds '{modInfoForFolder.requiredDlcIds[index]}'");
      }
    }
    if (modInfoForFolder.forbiddenDlcIds != null)
    {
      for (int index = 0; index < modInfoForFolder.forbiddenDlcIds.Length; ++index)
      {
        modInfoForFolder.forbiddenDlcIds[index] = modInfoForFolder.forbiddenDlcIds[index].ToUpperInvariant();
        if (!DlcManager.IsDlcId(modInfoForFolder.forbiddenDlcIds[index]))
          this.ModDevLogWarning($"\t{this.label}: {"mod_info.yaml"} in folder '{str}' is using an unrecognized DLC in forbiddenDlcIds '{modInfoForFolder.forbiddenDlcIds[index]}'");
      }
    }
    if (modInfoForFolder.lastWorkingBuild != 0)
    {
      this.ModDevLogError($"\t{this.label}: {"mod_info.yaml"} in folder '{str}' is using `{"lastWorkingBuild"}`, please upgrade this to `{"minimumSupportedBuild"}`");
      if (modInfoForFolder.minimumSupportedBuild == 0)
        modInfoForFolder.minimumSupportedBuild = modInfoForFolder.lastWorkingBuild;
    }
    this.ModDevLog($"\t{this.label}: Found valid mod_info.yaml in folder '{str}': requiredDlcIds='{modInfoForFolder.requiredDlcIds.DebugToCommaSeparatedList()}', forbiddenDlcIds='{modInfoForFolder.forbiddenDlcIds.DebugToCommaSeparatedList()}' at {modInfoForFolder.minimumSupportedBuild}");
  ...
  public bool Uninstall()
  {
    ...
    {
      Debug.Log((object) $"Can't uninstall {this.label.ToString()}: still has loaded content: {this.loaded_content.ToString()}");
      this.status = Mod.Status.UninstallPending;
      return false;
    }
    if (!this.IsLocal && !FileUtil.DeleteDirectory(this.label.install_path))
    {
      Debug.Log((object) $"Can't uninstall {this.label.ToString()}: directory deletion failed");
      this.status = Mod.Status.UninstallPending;
      return false;
    }
    this.status = Mod.Status.NotInstalled;
  ...
  public Texture2D GetPreviewImage()
  {
    ...
    try
    {
      byte[] data = File.ReadAllBytes(System.IO.Path.Combine(this.ContentPath, path2_1));
      Texture2D tex = new Texture2D(2, 2);
      tex.LoadImage(data);
      return tex;
    }
    ...
    {
      Debug.LogWarning((object) $"Mod {this.label} seems to have a preview.png but it didn't load correctly.");
      return (Texture2D) null;
    }
  }
