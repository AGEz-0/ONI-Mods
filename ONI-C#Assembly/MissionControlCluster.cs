  ...
  GameStateMachine<MissionControlCluster, MissionControlCluster.Instance, IStateMachineTarget, MissionControlCluster.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.Operational.EventTransition(GameHashes.OperationalChanged, this.Inoperational, new StateMachine<MissionControlCluster, MissionControlCluster.Instance, IStateMachineTarget, MissionControlCluster.Def>.Transition.ConditionCallback(this.ValidateOperationalTransition)).EventTransition(GameHashes.UpdateRoom, this.Operational.WrongRoom, GameStateMachine<MissionControlCluster, MissionControlCluster.Instance, IStateMachineTarget, MissionControlCluster.Def>.Not(new StateMachine<MissionControlCluster, MissionControlCluster.Instance, IStateMachineTarget, MissionControlCluster.Def>.Transition.ConditionCallback(this.IsInLabRoom))).Enter(new StateMachine<MissionControlCluster, MissionControlCluster.Instance, IStateMachineTarget, MissionControlCluster.Def>.State.Callback(this.OnEnterOperational)).DefaultState(this.Operational.NoRockets).Update((System.Action<MissionControlCluster.Instance, float>) ((smi, dt) => smi.UpdateWorkableRocketsInRange((object) null)), UpdateRate.SIM_1000ms);
  ...
  public new class Instance(IStateMachineTarget master, MissionControlCluster.Def def) : 
    GameStateMachine<MissionControlCluster, MissionControlCluster.Instance, IStateMachineTarget, MissionControlCluster.Def>.GameInstance(master, def)
  {
    ...
    public void UpdateWorkableRocketsInRange(object data)
    {
      ...
      for (int idx = 0; idx < Components.Clustercrafts.Count; ++idx)
      {
        if (ClusterGrid.Instance.IsInRange(Components.Clustercrafts[idx].Location, myWorldLocation, 2) && !this.IsOwnWorld(Components.Clustercrafts[idx]) && this.CanBeBoosted(Components.Clustercrafts[idx]))
        {
          ...
          {
            if (!((UnityEngine.Object) controlClusterWorkable.gameObject == (UnityEngine.Object) this.gameObject) && (UnityEngine.Object) controlClusterWorkable.TargetClustercraft == (UnityEngine.Object) Components.Clustercrafts[idx])
            {
              flag = true;
              break;
            }
    ...
    {
      return (double) clustercraft.controlStationBuffTimeRemaining == 0.0 && clustercraft.HasResourcesToMove() && clustercraft.IsFlightInProgress();
    }
    ...
    private bool IsOwnWorld(Clustercraft candidateClustercraft)
    {
      ...
      return !((UnityEngine.Object) interiorWorld == (UnityEngine.Object) null) && myWorldId == interiorWorld.id;
    }
