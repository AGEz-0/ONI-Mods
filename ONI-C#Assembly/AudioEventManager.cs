...
public class AudioEventManager : KMonoBehaviour
{
  ...
  {
    this.OnPrefabInit();
    this.spatialSplats.Reset(Grid.WidthInCells, Grid.HeightInCells, 16 /*0x10*/, 16 /*0x10*/);
  }

  public static float LoudnessToDB(float loudness)
  {
    return (double) loudness <= 0.0 ? 0.0f : 10f * Mathf.Log10(loudness);
  }
  ...
  public static string GetLoudestNoisePolluterAtCell(int cell)
  {
    ...
    foreach (NoiseSplat noiseSplat in audioEventManager.spatialSplats.GetAllIntersecting(pos))
    {
  ...
  public List<AudioEventManager.PolluterDisplay> GetPollutersForCell(int cell)
  {
    ...
    Vector2I xy = Grid.CellToXY(cell);
    foreach (NoiseSplat noiseSplat in this.spatialSplats.GetAllIntersecting(new Vector2((float) xy.x, (float) xy.y)))
    {
  ...
  {
    Debug.Log((object) $" [{itemId.ToString()}] \t{message}");
  }
