  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.neutral.EventHandler(GameHashes.TagsChanged, (GameStateMachine<JoyBehaviourMonitor, JoyBehaviourMonitor.Instance, IStateMachineTarget, object>.GameEvent.Callback) ((smi, data) =>
    {
      ...
      if (!changedEventData.added)
        return;
      if (changedEventData.tag == GameTags.PleasantConversation && (double) UnityEngine.Random.Range(0.0f, 100f) <= 1.0)
        smi.GoToOverjoyed();
      smi.GetComponent<KPrefabID>().RemoveTag(GameTags.PleasantConversation);
    })).EventHandler(GameHashes.ScheduleBlocksTick, (StateMachine<JoyBehaviourMonitor, JoyBehaviourMonitor.Instance, IStateMachineTarget, object>.State.Callback) (smi =>
    {
      if (!smi.ShouldBeOverjoyed())
        return;
      smi.GoToOverjoyed();
    }));
    this.overjoyed.Transition(this.neutral, (StateMachine<JoyBehaviourMonitor, JoyBehaviourMonitor.Instance, IStateMachineTarget, object>.Transition.ConditionCallback) (smi => (double) GameClock.Instance.GetTime() >= (double) smi.transitionTime)).ToggleExpression((Func<JoyBehaviourMonitor.Instance, Expression>) (smi => smi.happyExpression)).ToggleAnims((Func<JoyBehaviourMonitor.Instance, HashedString>) (smi => (HashedString) smi.happyLocoAnim)).ToggleAnims((Func<JoyBehaviourMonitor.Instance, HashedString>) (smi => (HashedString) smi.happyLocoWalkAnim)).ToggleTag(GameTags.Overjoyed).Exit((StateMachine<JoyBehaviourMonitor, JoyBehaviourMonitor.Instance, IStateMachineTarget, object>.State.Callback) (smi => smi.GetComponent<KPrefabID>().RemoveTag(GameTags.PleasantConversation))).OnSignal(this.exitEarly, this.neutral);
  }
