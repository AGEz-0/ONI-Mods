...
public class HoverTextDrawer
{
  ...
  public void DrawText(string text, TextStyleSetting style, Color color, bool override_color = true)
  {
    ...
    this.currentPos.x += widget.renderedWidth;
  ...
  {
    this.DrawIcon(icon, Color.white, min_width);
  }
  ...
  public void DrawIcon(Sprite icon, Color color, int image_size = 18, int horizontal_spacing = 2)
  {
    ...
    HoverTextDrawer.Pool<Image>.Entry entry1 = this.iconWidgets.Draw(this.currentPos + this.skin.shadowImageOffset);
    entry1.widget.sprite = icon;
    entry1.widget.color = this.skin.shadowImageColor;
  ...
  public void EndShadowBar()
  {
    ...
    entry1.rect.anchoredPosition = this.shadowStartPos + new Vector2(-this.skin.shadowBarBorder.x, this.skin.shadowBarBorder.y);
  ...
  private class Pool<WidgetType> where WidgetType : MonoBehaviour
  {
    ...
    public Pool(GameObject prefab, RectTransform master_root)
    {
      ...
      this.root.SetParent((Transform) master_root);
    ...
    public HoverTextDrawer.Pool<WidgetType>.Entry Draw(Vector2 pos)
    {
      ...
      {
        GameObject gameObject = Util.KInstantiateUI(this.prefab, this.root.gameObject);
    ...
    public void EndDrawing()
    {
      ...
      {
        if (this.entries[drawnWidgets].widget.gameObject.activeSelf)
          this.entries[drawnWidgets].widget.gameObject.SetActive(false);
      }
    }
    ...
    public void SetEnabled(bool enabled)
    {
      if (enabled)
        ...
        this.root.gameObject.GetComponent<CanvasGroup>().alpha = 0.0f;
    }

    public void Cleanup()
    {
      foreach (HoverTextDrawer.Pool<WidgetType>.Entry entry in this.entries)
        UnityEngine.Object.Destroy((UnityEngine.Object) entry.widget.gameObject);
      this.entries.Clear();
    }
