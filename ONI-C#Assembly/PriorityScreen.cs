...
public class PriorityScreen : KScreen
{
  ...
  public void InstantiateButtons(Action<PrioritySetting> on_click, bool playSelectionSound = true)
  {
    ...
    for (int index = 1; index <= 9; ++index)
    {
      ...
      PriorityButton priorityButton = Util.KInstantiateUI<PriorityButton>(this.buttonPrefab_basic.gameObject, this.buttonPrefab_basic.transform.parent.gameObject);
  ...
  public void SetScreenPriority(PrioritySetting priority, bool play_sound = false)
  {
    ...
    for (int index = 0; index < this.buttons_basic.Count; ++index)
    {
      ...
      this.buttons_basic[index].tooltip.SetSimpleTooltip(string.Format((string) (this.button_toggleHigh.isOn ? UI.PRIORITYSCREEN.HIGH : UI.PRIORITYSCREEN.BASIC), (object) (index + 1)));
  ...
  public static void PlayPriorityConfirmSound(PrioritySetting priority)
  {
    ...
    int num3 = (int) instance.setParameterByName(nameof (priority), num2);
