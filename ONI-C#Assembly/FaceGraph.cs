...
public class FaceGraph : KMonoBehaviour
{
  ...
  private static HashedString HASH_HEAD_MASTER_SWAP_KANIM = (HashedString) "head_master_swap_kanim";
  private static KAnimHashedString ANIM_HASH_SNAPTO_EYES = (KAnimHashedString) "snapto_eyes";
  private static KAnimHashedString ANIM_HASH_SNAPTO_MOUTH = (KAnimHashedString) "snapto_mouth";
  private static KAnimHashedString ANIM_HASH_NEUTRAL = (KAnimHashedString) "neutral";
  ...
  {
    HashedString hash = (HashedString) FaceGraph.ANIM_HASH_NEUTRAL;
    ...
    for (int index1 = 0; index1 < shapes_file.GetData().animCount && !flag1; ++index1)
    {
      ...
      if (anim2.hash == hash)
      {
        anim1 = anim2;
        KAnim.Anim.Frame frame;
        if (anim1.TryGetFrame(shapes_file.GetData().build.batchTag, 0, out frame))
        {
    ...
    KAnim.Build.SymbolFrameInstance symbolFrameInstance = KAnimBatchManager.Instance().GetBatchGroupData(variation_symbol.build.batchTag).symbolFrameInstances[variation_symbol.firstFrameIdx + frameElement.frame] with
    {
      buildImageIdx = this.m_symbolOverrideController.GetAtlasIdx(variation_symbol.build.GetTexture(0))
    };
  ...
  private void UpdateFace()
  {
    ...
    if (this.currentExpression != null)
    {
      ...
      Accessory accessory2 = full_id != HashedString.Invalid ? headEffects.Lookup(full_id) : (Accessory) null;
