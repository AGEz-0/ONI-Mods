...
public class PrimaryElement : KMonoBehaviour, ISaveLoadable
{
  ...
  private void OnDeserialized()
  {
    ...
    {
      DeserializeWarnings.Instance.PrimaryElementTemperatureIsNan.Warn($"{this.name} has invalid temperature of {this.Temperature}. Resetting temperature.");
      temperature = this.Element.defaultValues.temperature;
    }
    this._Temperature = temperature;
    this.Temperature = temperature;
    if (this.Element == null)
      DeserializeWarnings.Instance.PrimaryElementHasNoElement.Warn(this.name + "Primary element has no element.");
    ...
    {
      DebugUtil.DevLogError((UnityEngine.Object) this.gameObject, "deserialized element with 0 mass. Destroying");
      Util.KDestroyGameObject(this.gameObject);
    }
    else
    {
      ...
      {
        DiseaseHeader header = GameComps.DiseaseContainers.GetHeader(this.diseaseHandle) with
        {
          diseaseIdx = index,
  ...
  private void SanitizeMassAndTemperature()
  {
    ...
    {
      DebugUtil.DevLogError($"{this.gameObject.name} is attempting to serialize a temperature of <= 0K. Resetting to default. world={this.gameObject.DebugGetMyWorldName()}");
      this._Temperature = this.Element.defaultValues.temperature;
    }
    if ((double) this.Mass <= (double) PrimaryElement.MAX_MASS)
      return;
    DebugUtil.DevLogError($"{this.gameObject.name} is attempting to serialize very large mass {this.Mass}. Resetting to default. world={this.gameObject.DebugGetMyWorldName()}");
  ...
  private void SetMass(float mass)
  {
    if (((double) mass > (double) PrimaryElement.MAX_MASS || (double) mass < 0.0) && this.ElementID != SimHashes.Regolith)
      DebugUtil.DevLogErrorFormat((UnityEngine.Object) this.gameObject, "{0} is getting an abnormal mass set {1}.", (object) this.gameObject.name, (object) mass);
  ...
  private void SetTemperature(float temperature)
  {
    ...
    {
      DebugUtil.LogErrorArgs((UnityEngine.Object) this.gameObject, (object) $"Invalid temperature [{temperature.ToString()}]");
    }
    else
    {
  ...
  public byte DiseaseIdx
  {
    get
    {
      ...
      if (this.useSimDiseaseInfo)
      {
        ...
        diseaseIdx = GameComps.DiseaseContainers.GetHeader(this.diseaseHandle).diseaseIdx;
      return diseaseIdx;
    }
  }
  ...
  public int DiseaseCount
  {
    get
    {
      ...
      if (this.useSimDiseaseInfo)
      {
        ...
        diseaseCount = GameComps.DiseaseContainers.GetHeader(this.diseaseHandle).diseaseCount;
      return diseaseCount;
    }
  }
  ...
  private void SetDiseaseVisualProvider(GameObject visualizer)
  {
    ...
    if (!(handle != HandleVector<int>.InvalidHandle))
      return;
    DiseaseContainer payload = GameComps.DiseaseContainers.GetPayload(handle) with
    {
  ...
  public void RedirectDisease(GameObject target)
  {
    ...
    this.diseaseRedirectTarget = (bool) (UnityEngine.Object) target ? target.GetComponent<PrimaryElement>() : (PrimaryElement) null;
