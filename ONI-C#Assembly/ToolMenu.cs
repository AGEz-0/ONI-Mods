...
public class ToolMenu : KScreen
{
  ...
  public override float GetSortKey() => 5f;
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.priorityScreen = Util.KInstantiateUI<PriorityScreen>(this.Prefab_priorityScreen.gameObject, this.gameObject);
  ...
  private void RefreshScale(object data = null)
  {
    ...
    foreach (ToolMenu.ToolCollection sandboxTool in this.sandboxTools)
    {
      ...
      if ((UnityEngine.Object) componentInChildren != (UnityEngine.Object) null)
        componentInChildren.fontSize = ScreenResolutionMonitor.UsingGamepadUIMode() ? (float) num2 : (float) num1;
    }
  ...
  private Texture2D CreatePlaneTexture(out byte[] textureBytes, int width, int height)
  {
    ...
    Texture2D planeTexture = new Texture2D(width, height, TextureUtil.TextureFormatToGraphicsFormat(TextureFormat.RGBA32), TextureCreationFlags.None);
    planeTexture.name = "toolEffectDisplayPlane";
    planeTexture.wrapMode = TextureWrapMode.Clamp;
  ...
  private void CreateSandBoxTools()
  {
    ...
    this.sandboxTools.Add(ToolMenu.CreateToolCollection(STRINGS.UI.TOOLS.SANDBOX.HEATGUN.NAME, "temperature", Action.SandboxHeatGun, "SandboxHeatTool", STRINGS.UI.SANDBOXTOOLS.SETTINGS.HEATGUN.TOOLTIP, false));
    this.sandboxTools.Add(ToolMenu.CreateToolCollection(STRINGS.UI.TOOLS.SANDBOX.STRESSTOOL.NAME, "crew_state_happy", Action.SandboxStressTool, "SandboxStressTool", STRINGS.UI.SANDBOXTOOLS.SETTINGS.STRESS.TOOLTIP, false));
    ...
    this.sandboxTools.Add(ToolMenu.CreateToolCollection(STRINGS.UI.TOOLS.SANDBOX.FOW.NAME, "reveal", Action.SandboxReveal, "SandboxFOWTool", STRINGS.UI.SANDBOXTOOLS.SETTINGS.FOW.TOOLTIP, false));
    this.sandboxTools.Add(ToolMenu.CreateToolCollection(STRINGS.UI.TOOLS.SANDBOX.CRITTER.NAME, "critter", Action.SandboxCritterTool, "SandboxCritterTool", STRINGS.UI.SANDBOXTOOLS.SETTINGS.CRITTER.TOOLTIP, false));
    this.sandboxTools.Add(ToolMenu.CreateToolCollection(STRINGS.UI.TOOLS.SANDBOX.SPAWN_STORY_TRAIT.NAME, "sandbox_storytrait", Action.SandboxStoryTraitTool, "SandboxStoryTraitTool", STRINGS.UI.SANDBOXTOOLS.SETTINGS.SPAWN_STORY_TRAIT.TOOLTIP, false));
  }
  ...
  private void CreateBasicTools()
  {
    ...
    this.basicTools.Add(ToolMenu.CreateToolCollection(STRINGS.UI.TOOLS.DISCONNECT.NAME, "icon_action_disconnect", Action.Disconnect, "DisconnectTool", STRINGS.UI.TOOLS.DISCONNECT.TOOLTIP, false));
  }
  ...
  private void InstantiateCollectionsUI(IList<ToolMenu.ToolCollection> collections)
  {
    ...
    for (int index1 = 0; index1 < collections.Count; ++index1)
    {
      ...
      tc.toggle = Util.KInstantiateUI(collections[index1].tools.Count > 1 ? this.collectionIconPrefab : (collections == this.sandboxTools ? this.sandboxToolIconPrefab : (collections[index1].largeIcon ? this.toolIconLargePrefab : this.toolIconPrefab)), parent3, true);
      ...
      if (tc.tools != null)
      {
        ...
        for (int index2 = 0; index2 < tc.tools.Count; ++index2)
        {
          ...
          GameObject gameObject5 = Util.KInstantiateUI(collections == this.sandboxTools ? this.sandboxToolIconPrefab : (collections[index1].largeIcon ? this.toolIconLargePrefab : this.toolIconPrefab), tc.MaskContainer, true);
    ...
    if ((num <= 0 ? 0 : (num % 2 == 0 ? 1 : 0)) != 0)
    {
      gameObject2.GetComponent<HorizontalLayoutGroup>().padding.left = 26;
      gameObject3.GetComponent<HorizontalLayoutGroup>().padding.right = 26;
    }
  ...
  protected void BuildRowToggles(IList<ToolMenu.ToolCollection> row)
  {
    for (int index = 0; index < row.Count; ++index)
    {
      ...
      if (!((UnityEngine.Object) toolCollection.toggle == (UnityEngine.Object) null))
      {
        ...
        Sprite sprite = Assets.GetSprite((HashedString) toolCollection.icon);
        ...
        if ((bool) (UnityEngine.Object) component1)
        {
          if (row[index].tools.Count == 1)
          {
            ...
            component1.AddMultiStringTooltip(row[index].tools[0].text, this.TooltipHeader);
            component1.AddMultiStringTooltip(newString, this.ToggleToolTipTextStyleSetting);
          }
          else
          {
    ...
    for (int index1 = 0; index1 < row.Count; ++index1)
    {
      ...
      if (!((UnityEngine.Object) toolCollection.toggle == (UnityEngine.Object) null))
      {
        for (int index2 = 0; index2 < toolCollection.tools.Count; ++index2)
        {
          ...
          if ((bool) (UnityEngine.Object) component1)
          {
            string newString = toolCollection.tools.Count > 1 ? GameUtil.ReplaceHotkeyString(toolCollection.tools[index2].tooltip, toolCollection.hotkey, toolCollection.tools[index2].hotkey) : GameUtil.ReplaceHotkeyString(toolCollection.tools[index2].tooltip, toolCollection.tools[index2].hotkey);
            component1.ClearMultiStringTooltip();
            component1.AddMultiStringTooltip(newString, this.ToggleToolTipTextStyleSetting);
          }
        }
  ...
  public struct CellColorData(int cell, Color color)
  {
