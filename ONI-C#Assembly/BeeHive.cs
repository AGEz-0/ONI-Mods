  ...
  GameStateMachine<BeeHive, BeeHive.StatesInstance, IStateMachineTarget, BeeHive.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    })).EventHandler(GameHashes.Died, (StateMachine<BeeHive, BeeHive.StatesInstance, IStateMachineTarget, BeeHive.Def>.State.Callback) (smi =>
    {
      ...
      component2.DropAll(smi.master.transform.position, true, true);
    }));
    ...
    this.enabled.growingStates.idle.Update((System.Action<BeeHive.StatesInstance, float>) ((smi, dt) => smi.DeltaGrowth(dt / 600f / BeeHiveTuning.HIVE_GROWTH_TIME)), UpdateRate.SIM_4000ms);
    ...
    GameStateMachine<BeeHive, BeeHive.StatesInstance, IStateMachineTarget, BeeHive.Def>.GameInstance
  {
    ...
    {
      Util.KInstantiate(Assets.GetPrefab((Tag) this.def.larvaPrefabID), this.transform.GetPosition()).SetActive(true);
    }

    public void SpawnNewBeeFromHive()
    {
      Util.KInstantiate(Assets.GetPrefab((Tag) this.def.beePrefabID), this.transform.GetPosition()).SetActive(true);
    }
