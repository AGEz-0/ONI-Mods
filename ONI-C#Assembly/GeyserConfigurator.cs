...
public class GeyserConfigurator : KMonoBehaviour
{
  ...
  public static GeyserConfigurator.GeyserType FindType(HashedString typeId)
  {
    ...
    if (typeId != HashedString.Invalid)
      type = GeyserConfigurator.geyserTypes.Find((Predicate<GeyserConfigurator.GeyserType>) (t => (HashedString) t.id == typeId));
    if (type == null)
      Debug.LogError((object) $"Tried finding a geyser with id {typeId.ToString()} but it doesn't exist!");
    return type;
  }
  ...
  private GeyserConfigurator.GeyserInstanceConfiguration CreateRandomInstance(
    ...
    float max)
  {
    KRandom randomSource = new KRandom(SaveLoader.Instance.clusterDetailSave.globalWorldSeed + (int) this.transform.GetPosition().x + (int) this.transform.GetPosition().y);
  ...
  public class GeyserInstanceConfiguration
  {
    ...
    {
      return !this.modifier.modifyElement || this.modifier.newElement == (SimHashes) 0 ? this.geyserType.element : this.modifier.newElement;
    }
