...
public class CrewListScreen<EntryType> : KScreen where EntryType : CrewListEntry
{
  ...
  {
    this.EntryObjects.ForEach((Action<EntryType>) (eo => eo.RefreshCrewPortraitContent()));
  }
  ...
  protected virtual void PositionColumnTitles()
  {
    ...
    else
    {
      ...
      for (int index = 0; index < childCount; ++index)
      {
        ...
        {
          GameObject go = Util.KInstantiate(this.Prefab_ColumnTitle);
  ...
  protected void SortByName(bool reverse)
  {
    ...
    sortedEntries.Sort((Comparison<EntryType>) ((a, b) => (a.Identity.GetProperName() + a.gameObject.GetInstanceID().ToString()).CompareTo(b.Identity.GetProperName() + b.gameObject.GetInstanceID().ToString())));
