...
public class Shower : Workable, IGameObjectEffectDescriptor
{
  ...
  protected override void OnStopWork(WorkerBase worker) => this.smi.SetActive(false);
  ...
  public class ShowerSM : GameStateMachine<Shower.ShowerSM, Shower.ShowerSM.Instance, Shower>
  {
    ...
    {
      WorkChore<Shower> showerChore = new WorkChore<Shower>(Db.Get().ChoreTypes.Shower, (IStateMachineTarget) smi.master, allow_in_red_alert: false, schedule_block: Db.Get().ScheduleBlockTypes.Hygiene, allow_prioritization: false, priority_class: PriorityScreen.PriorityClass.high);
      showerChore.AddPrecondition(ChorePreconditions.instance.IsNotABionic, (object) smi);
      return (Chore) showerChore;
    }
