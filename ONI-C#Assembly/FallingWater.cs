...
public class FallingWater : KMonoBehaviour, ISim200ms
{
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.mistPool = new GameObjectPool(new Func<GameObject>(this.InstantiateMist), 16 /*0x10*/);
  }
    ...
    bool disable_randomness = false)
  {
    ...
    if ((double) temperature <= 0.0 || (double) base_mass <= 0.0)
      Debug.LogError((object) $"Unexpected water mass/temperature values added to the falling water manager T({temperature}) M({base_mass})");
    ...
    if (!skip_sound)
    {
      ...
      if (!this.topSounds.TryGetValue(cell1, out soundInfo))
      {
        ...
        soundInfo.handle = LoopingSoundManager.StartSound(this.liquid_top_loop, (Vector3) root_pos);
      }
    ...
    while ((double) base_mass > 0.0)
    {
      ...
      double num2 = (double) mass / (double) base_mass;
      base_mass -= mass;
      double num3 = (double) base_disease_count;
      int b = (int) (num2 * num3);
      int disease_count = Mathf.Min(a, b);
      a = Mathf.Max(0, a - disease_count);
      ...
      Vector2 vector2_2 = disable_randomness ? Vector2.zero : new Vector2(UnityEngine.Random.Range(-this.multipleOffsetRange.x, this.multipleOffsetRange.x), UnityEngine.Random.Range(-this.multipleOffsetRange.y, this.multipleOffsetRange.y));
  ...
  public void SpawnLiquidSplash(float x, int cell, bool forceSplash = false)
  {
    ...
    Vector2 pos2D = (Vector2) Grid.CellToPos2D(cell) with
    {
      x = x - 0.5f
    };
  ...
  public void UpdateParticles(float dt)
  {
    ...
    for (int index1 = 0; index1 < count; ++index1)
    {
      ...
      int x;
      int y1;
      Grid.PosToXY(position, out x, out y1);
      physic.velocity += vector2;
      Vector3 vector3 = position + (Vector3) (physic.velocity * dt);
      physic.position = (Vector2) vector3;
      this.physics[index1] = physic;
      int y2;
      Grid.PosToXY((Vector3) physic.position, out int _, out y2);
  ...
  private void UpdateMistFX(float t)
  {
    ...
    foreach (KeyValuePair<Pair<int, bool>, FallingWater.MistInfo> keyValuePair in this.mistAlive)
    {
      ...
      {
        keyValuePair.Value.fx.Play((HashedString) "end");
  ...
  private void AddToSim(int cell, int particleIdx, ref int num_particles)
  {
    ...
    if ((double) time - (double) num1 < (double) this.minSpawnDelay)
      return;
    this.lastSpawnTime[cell] = time;
    Vector3 posCcc = Grid.CellToPosCCC(cell, Grid.SceneLayer.TileMain) with
    {
      z = 0.0f
    };
    ...
    int num2 = (int) instance.setParameterByName("liquidDepth", SoundUtil.GetLiquidDepth(cell));
    int num3 = (int) instance.setParameterByName("liquidVolume", this.GetParticleVolume(particleProperty.mass));
  ...
  public void Render()
  {
    ...
    for (int index = 0; index < num1; ++index)
    {
      ...
      int frame;
      float x2 = (float) (frame = this.physics[index].frame) * this.uvFrameSize.x;
    ...
    Graphics.DrawMesh(this.mesh, this.renderOffset, Quaternion.identity, this.material, layer, (Camera) null, 0, this.propertyBlock);
  }
  ...
  private KBatchedAnimController SpawnMist()
  {
    ...
    component.Play((HashedString) "loop", KAnim.PlayMode.Loop);
    return component;
  }
    ...
    if (this.serializedParticleProperties != null)
    {
      ...
      foreach (FallingWater.SerializedParticleProperties particleProperty in this.serializedParticleProperties)
        this.particleProperties.Add(new FallingWater.ParticleProperties()
        {
          ...
          diseaseIdx = particleProperty.diseaseID != HashedString.Invalid ? diseases.GetIndex(particleProperty.diseaseID) : byte.MaxValue,
          mass = particleProperty.mass,
          temperature = particleProperty.temperature,
          diseaseCount = particleProperty.diseaseCount
        });
  ...
  private struct ParticleProperties(
    ...
    bool debug_track)
  {
    public ushort elementIdx = elementIdx;
