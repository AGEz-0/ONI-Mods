...
public class CringeMonitor : GameStateMachine<CringeMonitor, CringeMonitor.Instance>
{
  ...
  public new class Instance(IStateMachineTarget master) : 
    GameStateMachine<CringeMonitor, CringeMonitor.Instance, IStateMachineTarget, object>.GameInstance(master)
  {
    private StatusItem statusItem;

    public void SetCringeSourceData(object data)
    {
      this.statusItem = new StatusItem("CringeSource", (string) data, (string) null, "", StatusItem.IconType.Exclamation, NotificationType.BadMinor, false, OverlayModes.None.ID);
    }

    public Reactable GetReactable()
    {
      SelfEmoteReactable reactable = new SelfEmoteReactable(this.master.gameObject, (HashedString) "Cringe", Db.Get().ChoreTypes.EmoteHighPriority, localCooldown: 0.0f);
