...
public class HotTub : StateMachineComponent<HotTub.StatesInstance>, IGameObjectEffectDescriptor
{
  ...
  {
    get => 100f * this.waterStorage.GetMassAvailable(SimHashes.Water) / this.hotTubCapacity;
  }
  ...
  protected override void OnSpawn()
  {
    ...
    this.waterMeter = new MeterController((KAnimControllerBase) this.GetComponent<KBatchedAnimController>(), "meter_water_target", "meter_water", Meter.Offset.Infront, Grid.SceneLayer.NoLayer, new string[1]
    {
      "meter_water_target"
    });
    this.smi.UpdateWaterMeter();
    this.tempMeter = new MeterController((KAnimControllerBase) this.GetComponent<KBatchedAnimController>(), "meter_temperature_target", "meter_temp", Meter.Offset.Infront, Grid.SceneLayer.NoLayer, new string[1]
    {
  ...
  private Chore CreateChore(int i)
  {
    ...
    chore.AddPrecondition(ChorePreconditions.instance.IsNotABionic, (object) workable);
    return (Chore) chore;
  }
  ...
  List<Descriptor> IGameObjectEffectDescriptor.GetDescriptors(GameObject go)
  {
    ...
    descs.Add(new Descriptor((string) Strings.Get($"STRINGS.DUPLICANTS.MODIFIERS.{"WarmTouch".ToUpper()}.PROVIDERS_NAME"), (string) Strings.Get($"STRINGS.DUPLICANTS.MODIFIERS.{"WarmTouch".ToUpper()}.PROVIDERS_TOOLTIP")));
  ...
  public class States : GameStateMachine<HotTub.States, HotTub.StatesInstance, HotTub>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      }), UpdateRate.SIM_4000ms).EventHandler(GameHashes.OnStorageChange, (StateMachine<HotTub.States, HotTub.StatesInstance, HotTub, object>.State.Callback) (smi =>
      {
    ...
    GameStateMachine<HotTub.States, HotTub.StatesInstance, HotTub, object>.GameInstance
  {
    ...
    {
      this.smi.master.waterMeter.SetPositionPercent(Mathf.Clamp(this.smi.master.waterStorage.GetMassAvailable(SimHashes.Water) / this.smi.master.hotTubCapacity, 0.0f, 1f));
    }
    ...
    {
      return (double) this.smi.master.GetComponent<PrimaryElement>().Temperature > (double) this.smi.master.maxOperatingTemperature;
    }
    ...
    {
      this.smi.master.waterStorage.ConsumeIgnoringDisease(new Tag(-839728230), this.smi.master.bleachStoneConsumption * dt);
    }
  }
}
