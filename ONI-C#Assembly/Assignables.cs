...
public class Assignables : KMonoBehaviour
{
  ...
  public AssignableSlotInstance[] GetSlots(AssignableSlot slot)
  {
    ...
    if (slot == null)
      return (AssignableSlotInstance[]) null;
    List<AssignableSlotInstance> all = this.slots.FindAll((Predicate<AssignableSlotInstance>) (s => s.slot == slot));
    return all != null && all.Count > 0 ? all.ToArray() : (AssignableSlotInstance[]) null;
  }
