...
public class Clinic : Workable, IGameObjectEffectDescriptor, ISingleSliderControl, ISliderControl
{
  ...
  private void InstantExtremeRadiationRecovery()
  {
    if (!Game.IsDlcActiveForCurrentSave("EXPANSION1_ID"))
      return;
    RadiationMonitor.Instance smi = this.worker.GetSMI<RadiationMonitor.Instance>();
    if ((double) smi.sm.radiationExposure.Get(smi) < 900.0 * (double) smi.difficultySettingMod)
      return;
    double num = (double) smi.master.gameObject.GetAmounts().Get(Db.Get().Amounts.RadiationBalance).SetValue(600f * smi.difficultySettingMod);
  }
  ...
  private Chore CreateBionicWorkChore(
    ChoreType chore_type,
    bool allow_prioritization,
    bool allow_in_red_alert,
    PriorityScreen.PriorityClass priority_class,
    bool ignore_schedule_block = false)
  {
    ...
    int num3 = flag ? 1 : 0;
    int priority_class1 = (int) priorityClass;
    WorkChore<Clinic> bionicWorkChore = new WorkChore<Clinic>(chore_type1, (IStateMachineTarget) this, allow_in_red_alert: num1 != 0, ignore_schedule_block: num2 != 0, allow_prioritization: num3 != 0, priority_class: (PriorityScreen.PriorityClass) priority_class1, add_to_daily_report: false);
    bionicWorkChore.AddPrecondition(ChorePreconditions.instance.IsBionic, (object) null);
    return (Chore) bionicWorkChore;
  }
  ...
  {
    Health component = (UnityEngine.Object) minion != (UnityEngine.Object) null ? minion.GetComponent<Health>() : (Health) null;
    if ((UnityEngine.Object) component != (UnityEngine.Object) null)
    {
      float num = component.hitPoints / component.maxHitPoints;
      if ((UnityEngine.Object) component != (UnityEngine.Object) null)
  ...
  public class ClinicSM : GameStateMachine<Clinic.ClinicSM, Clinic.ClinicSM.Instance, Clinic>
  {
    ...
    public new class Instance(Clinic master) : 
      GameStateMachine<Clinic.ClinicSM, Clinic.ClinicSM.Instance, Clinic, object>.GameInstance(master)
    {
      ...
      public EffectInstance StartEffect(string effect, bool should_save)
      {
        if (this.master.IsValidEffect(effect))
        {
          WorkerBase worker = this.smi.master.worker;
