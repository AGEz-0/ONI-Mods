...
public class SpeedControlScreen : KScreen
{
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.speedButtonWidget_slow.GetComponent<ToolTip>().AddMultiStringTooltip(GameUtil.ReplaceHotkeyString((string) STRINGS.UI.TOOLTIPS.SPEEDBUTTON_SLOW, Action.CycleSpeed), this.TooltipTextStyle);
    this.speedButtonWidget_medium.GetComponent<ToolTip>().AddMultiStringTooltip(GameUtil.ReplaceHotkeyString((string) STRINGS.UI.TOOLTIPS.SPEEDBUTTON_MEDIUM, Action.CycleSpeed), this.TooltipTextStyle);
    this.speedButtonWidget_fast.GetComponent<ToolTip>().AddMultiStringTooltip(GameUtil.ReplaceHotkeyString((string) STRINGS.UI.TOOLTIPS.SPEEDBUTTON_FAST, Action.CycleSpeed), this.TooltipTextStyle);
  ...
  public void ResetToolTip()
  {
    ...
    if (this.pauseButton.isOn)
    {
      ...
      this.pauseButtonWidget.GetComponent<ToolTip>().AddMultiStringTooltip(GameUtil.ReplaceHotkeyString((string) STRINGS.UI.TOOLTIPS.UNPAUSE, Action.TogglePause), this.TooltipTextStyle);
    }
    else
    {
      this.pauseButtonWidget.GetComponent<ToolTip>().ClearMultiStringTooltip();
      this.pauseButtonWidget.GetComponent<ToolTip>().AddMultiStringTooltip(GameUtil.ReplaceHotkeyString((string) STRINGS.UI.TOOLTIPS.PAUSE, Action.TogglePause), this.TooltipTextStyle);
    }
  }
  ...
  private void PlaySpeedChangeSound(float speed)
  {
    ...
    int num = (int) instance.setParameterByName("Speed", speed);
    SoundEvent.EndOneShot(instance);
  }

  public void DebugStepFrame()
  {
    DebugUtil.LogArgs((object) $"Stepping one frame {GameClock.Instance.GetTime()} ({(ValueType) (float) ((double) GameClock.Instance.GetTime() / 600.0)})");
  ...
  private IEnumerator DebugStepFrameDelay()
  {
    ...
    DebugUtil.LogArgs((object) "Stepped one frame", (object) (float) ((double) Time.time - (double) this.stepTime), (object) "seconds");
    this.Pause(false);
  }
}
