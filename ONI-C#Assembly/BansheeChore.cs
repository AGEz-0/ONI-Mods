...
public class BansheeChore : Chore<BansheeChore.StatesInstance>
{
    ...
    GameStateMachine<BansheeChore.States, BansheeChore.StatesInstance, BansheeChore, object>.GameInstance
  {
    ...
    public void FindAudience()
    {
      ...
      for (int index = 0; index < worldItems.Count; ++index)
      {
        if (!worldItems[index].IsNullOrDestroyed() && !((UnityEngine.Object) worldItems[index].gameObject == (UnityEngine.Object) this.gameObject))
        {
          ...
          if (component.CanReach(cell) && !worldItems[index].GetComponent<Effects>().HasEffect("WailedAt"))
          {
    ...
    public int FindIdleCell()
    {
      ...
      IdleCellQuery query = PathFinderQueries.idleCellQuery.Reset(this.GetComponent<MinionBrain>(), UnityEngine.Random.Range(30, 90));
    ...
    public void BotherAudience(float dt)
    {
      ...
      {
        if (!worldItem.IsNullOrDestroyed() && !((UnityEngine.Object) worldItem.gameObject == (UnityEngine.Object) this.smi.master.gameObject))
        {
          ...
          if (Grid.GetCellDistance(cell1, Grid.PosToCell((KMonoBehaviour) worldItem)) <= TUNING.STRESS.BANSHEE_WAIL_RADIUS)
          {
            ...
            {
              worldItem.GetComponent<Effects>().Add("WailedAt", true);
              worldItem.GetSMI<ThreatMonitor.Instance>().ClearMainThreat();
              FleeChore fleeChore = new FleeChore(worldItem.GetComponent<IStateMachineTarget>(), this.smi.master.gameObject);
            }
          }
        }
      }
    ...
    GameStateMachine<BansheeChore.States, BansheeChore.StatesInstance, BansheeChore>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.wailPreEffect.Add(new AttributeModifier("AirConsumptionRate", DUPLICANTSTATS.STANDARD.BaseStats.OXYGEN_USED_PER_SECOND * 75f));
      ...
      this.wailRecoverEffect.Add(new AttributeModifier("AirConsumptionRate", DUPLICANTSTATS.STANDARD.BaseStats.OXYGEN_USED_PER_SECOND * 10f));
