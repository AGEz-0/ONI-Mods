  ...
  GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance>
{
    ...
    GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
      ...
      : base(master)
    {
      ...
      this.gameObject.Subscribe(-1582839653, new System.Action<object>(this.OnBionicTagsChanged));
      this.inateImmunities = DUPLICANTSTATS.GetStatsFor(this.gameObject).DiseaseImmunities.IMMUNITIES;
      OxygenBreather component = this.GetComponent<OxygenBreather>();
      if (!((UnityEngine.Object) component != (UnityEngine.Object) null))
        return;
      component.onBreathableGasConsumed += new System.Action<SimHashes, float, float, byte, int>(this.OnAirConsumed);
    }

    protected override void OnCleanUp()
    {
      this.gameObject.Unsubscribe(-1582839653, new System.Action<object>(this.OnBionicTagsChanged));
      base.OnCleanUp();
    }
    ...
    public void OnAirConsumed(
      ...
      int disseaseCount)
    {
      if (disseaseIDX == byte.MaxValue)
        return;
      this.InjectDisease(Db.Get().Diseases[(int) disseaseIDX], disseaseCount, ElementLoader.FindElementByHash(elementConsumed).tag, Sickness.InfectionVector.Inhalation);
    }
    ...
    {
      return Db.Get().Attributes.GermResistance.Lookup(this.gameObject).GetTotalValue();
    }
    ...
    public bool AssessInhaledGerms(ExposureType exposure_type)
    {
      ...
      this.inhaleExposureTick.TryGetValue((HashedString) exposure_type.germ_id, out inhaleTickInfo);
      if (inhaleTickInfo == null)
      {
        inhaleTickInfo = new GermExposureMonitor.Instance.InhaleTickInfo();
        this.inhaleExposureTick[(HashedString) exposure_type.germ_id] = inhaleTickInfo;
      }
    ...
    public bool IsImmuneToDisease(string sicknessID)
    {
      if (this.inateImmunities == null)
        return false;
      for (int index = 0; index < this.inateImmunities.Length; ++index)
      {
        if (sicknessID == this.inateImmunities[index])
          return true;
      }
      return false;
    }
      ...
      Sickness.InfectionVector vector)
    {
      foreach (ExposureType exposureType in GERM_EXPOSURE.TYPES)
      {
        if (disease.id == (HashedString) exposureType.germ_id && !this.IsImmuneToDisease(exposureType.sickness_id) && count > exposureType.exposure_threshold && this.HasMinExposurePeriodElapsed(exposureType.germ_id) && this.IsExposureValidForTraits(exposureType))
        {
          ...
          if (sickness == null || sickness.infectionVectors.Contains(vector))
          {
            ...
            switch (exposureState)
            {
              ...
              case GermExposureMonitor.ExposureState.Contact:
                ...
                {
                  this.lastDiseaseSources[disease.id] = new GermExposureMonitor.Instance.DiseaseSourceInfo(source, vector, contractionChance1, this.transform.GetPosition());
                  ...
                  {
                    if (flag2)
                      this.inhaleExposureTick[(HashedString) exposureType.germ_id].ticks = 0;
              ...
              case GermExposureMonitor.ExposureState.Exposed:
                if ((double) exposureTier < 3.0)
                {
                  ...
                  {
                    this.lastDiseaseSources[disease.id] = new GermExposureMonitor.Instance.DiseaseSourceInfo(source, vector, contractionChance2, this.transform.GetPosition());
                    if (!exposureType.infect_immediately)
                    {
                      bool flag4 = true;
    ...
    {
      float num;
      this.lastExposureTime.TryGetValue((HashedString) germ_id, out num);
    ...
    {
      ReportManager.Instance.ReportValue(ReportManager.ReportType.DiseaseStatus, (float) this.primaryElement.DiseaseCount, StringFormatter.Replace((string) UI.ENDOFDAYREPORT.NOTES.GERMS, "{0}", this.master.name), this.master.gameObject.GetProperName());
    }
    ...
    private void OnBionicTagsChanged(object o)
    {
      if (o == null)
        return;
      TagChangedEventData changedEventData = (TagChangedEventData) o;
      if (!(changedEventData.tag == GameTags.BionicBedTime) || changedEventData.added)
        return;
    ...
    public string GetLastDiseaseSource(string id)
    {
      ...
      if (this.lastDiseaseSources.TryGetValue((HashedString) id, out diseaseSourceInfo))
      {
    ...
    {
      GermExposureMonitor.Instance.DiseaseSourceInfo diseaseSourceInfo;
      return this.lastDiseaseSources.TryGetValue((HashedString) germ_id, out diseaseSourceInfo) ? diseaseSourceInfo.position : this.transform.GetPosition();
    }
    ...
    public float GetExposureWeight(string id)
    {
      ...
      GermExposureMonitor.Instance.DiseaseSourceInfo diseaseSourceInfo;
      return this.lastDiseaseSources.TryGetValue((HashedString) id, out diseaseSourceInfo) ? diseaseSourceInfo.factor * exposureTier : 0.0f;
    }
