  ...
  IGameObjectEffectDescriptor
{
    ...
    bool high_lux)
  {
    ...
    {
      Descriptor descriptor = new Descriptor(locString1.Replace("{attrib}", (string) Strings.Get($"STRINGS.DUPLICANTS.ATTRIBUTES.{selfModifier.AttributeId.ToUpper()}.NAME")).Replace("{amount}", selfModifier.GetFormattedString()).Replace("{lux}", GameUtil.GetFormattedLux(BeachChairConfig.TAN_LUX)), locString2.Replace("{attrib}", (string) Strings.Get($"STRINGS.DUPLICANTS.ATTRIBUTES.{selfModifier.AttributeId.ToUpper()}.NAME")).Replace("{amount}", selfModifier.GetFormattedString()).Replace("{lux}", GameUtil.GetFormattedLux(BeachChairConfig.TAN_LUX)));
  ...
  public class States : GameStateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.ready.idle.PlayAnim("on", KAnim.PlayMode.Loop).WorkableStartTransition((Func<BeachChair.StatesInstance, Workable>) (smi => (Workable) smi.master.GetComponent<BeachChairWorkable>()), this.ready.working_pre);
      ...
      })).ToggleStatusItem(Db.Get().BuildingStatusItems.TanningLightInsufficient).WorkableStopTransition((Func<BeachChair.StatesInstance, Workable>) (smi => (Workable) smi.master.GetComponent<BeachChairWorkable>()), this.ready.post).Target(this.worker).PlayAnim("working_unlit_pre");
      ...
      this.ready.working_lit.DefaultState(this.ready.working_lit.working).Enter((StateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.State.Callback) (smi =>
      {
        ...
        component.workingPstComplete = hashedStringArray;
      })).ToggleStatusItem(Db.Get().BuildingStatusItems.TanningLightSufficient).WorkableStopTransition((Func<BeachChair.StatesInstance, Workable>) (smi => (Workable) smi.master.GetComponent<BeachChairWorkable>()), this.ready.post).Target(this.worker).PlayAnim("working_lit_pre");
      this.ready.working_lit.working.ParamTransition<bool>((StateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.Parameter<bool>) this.lit, this.ready.working_lit.post, GameStateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.IsFalse).Target(this.worker).QueueAnim("working_lit_loop", true).ScheduleGoTo((Func<BeachChair.StatesInstance, float>) (smi => UnityEngine.Random.Range(5f, 15f)), (StateMachine.BaseState) this.ready.working_lit.silly);
      this.ready.working_lit.silly.ParamTransition<bool>((StateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.Parameter<bool>) this.lit, this.ready.working_lit.post, GameStateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.IsFalse).Target(this.worker).PlayAnim((Func<BeachChair.StatesInstance, string>) (smi => this.SILLY_ANIMS[UnityEngine.Random.Range(0, this.SILLY_ANIMS.Length)])).OnAnimQueueComplete(this.ready.working_lit.working);
    ...
    {
      Workable component = (Workable) smi.master.GetComponent<BeachChairWorkable>();
  ...
  public class StatesInstance(BeachChair smi) : 
    GameStateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.GameInstance(smi)
  {
  }
}
