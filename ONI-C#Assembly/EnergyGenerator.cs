  ...
  ISliderControl
{
  ...
  {
    get => "STRINGS.UI.UISIDESCREENS.MANUALDELIVERYGENERATORSIDESCREEN.TITLE";
  }
  ...
  string ISliderControl.GetSliderTooltip(int index)
  {
    ...
    return string.Format((string) Strings.Get("STRINGS.UI.UISIDESCREENS.MANUALDELIVERYGENERATORSIDESCREEN.TOOLTIP"), (object) component.RequestedItemTag.ProperName(), (object) (float) ((double) this.batteryRefillPercent * 100.0));
  }
  ...
  private void Emit(EnergyGenerator.OutputItem output, float dt, PrimaryElement root_pe)
  {
    ...
    if (output.store)
    {
      if (elementByHash.IsGas)
        ...
        this.storage.Store(elementByHash.substance.SpawnResource(this.transform.GetPosition(), num1, root_pe.Temperature, byte.MaxValue, 0), true);
    }
    else
    {
  ...
  [Serializable]
  public struct InputItem(Tag tag, float consumption_rate, float max_stored_mass)
  {
    public Tag tag = tag;
    public float consumptionRate = consumption_rate;
    public float maxStoredMass = max_stored_mass;
  }
    ...
    float min_temperature = 0.0f)
  {
    ...
    public float creationRate = creation_rate;
    public bool store = store;
    public CellOffset emitOffset = emit_offset;
    public float minTemperature = min_temperature;
