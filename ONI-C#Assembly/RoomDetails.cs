...
public class RoomDetails
{
  ...
  public static readonly RoomDetails.Detail PLANT_COUNT = new RoomDetails.Detail((Func<Room, string>) (room =>
  {
    int num = 0;
    foreach (KPrefabID plant in room.cavity.plants)
    {
      if (!plant.HasTag(GameTags.PlantBranch))
        ++num;
    }
    return string.Format((string) ROOMS.DETAILS.PLANT_COUNT.NAME, (object) num);
  }));
