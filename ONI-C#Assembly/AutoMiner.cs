...
public class AutoMiner : StateMachineComponent<AutoMiner.Instance>, ISim1000ms
{
  ...
  protected override void OnSpawn()
  {
    ...
    this.arm_go.transform.SetPosition((Vector3) component.GetSymbolTransform(new HashedString("gun_target"), out bool _).GetColumn(3) with
    {
  ...
  public void UpdateRotation(float dt)
  {
    if (!this.HasDigCell)
      return;
    this.RotateArm(Vector3.Normalize(Grid.CellToPosCCC(this.dig_cell, Grid.SceneLayer.TileMain) with
    {
      z = 0.0f
    } - this.arm_go.transform.GetPosition() with
    {
      z = 0.0f
    }), false, dt);
  }
  ...
  {
    return this.HasDigCell ? Grid.Element[this.dig_cell] : (Element) null;
  }
  ...
  public void UpdateDig(float dt)
  {
    ...
    Vector3 posCcc = Grid.CellToPosCCC(this.dig_cell, Grid.SceneLayer.FXFront2) with
    {
      z = 0.0f
    };
    Vector3 position = this.arm_go.transform.GetPosition() with
    {
      z = 0.0f
    };
  ...
  private static bool ValidDigCell(int cell)
  {
    ...
    if (flag)
    {
      ...
      flag = (UnityEngine.Object) component != (UnityEngine.Object) null && component.IsOpen() && !component.IsPendingClose();
    }
    return Grid.Solid[cell] && (!Grid.Foundation[cell] || flag) && Grid.Element[cell].hardness < (byte) 150;
  }
  ...
  public static bool DigBlockingCB(int cell)
  {
    ...
    return Grid.Foundation[cell] && Grid.Solid[cell] && !flag || Grid.Element[cell].hardness >= (byte) 150;
  }
  ...
  public class Instance(AutoMiner master) : 
    GameStateMachine<AutoMiner.States, AutoMiner.Instance, AutoMiner, object>.GameInstance(master)
  {
  }
