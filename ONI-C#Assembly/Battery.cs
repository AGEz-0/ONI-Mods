  ...
  IEnergyProducer
{
  ...
  private void OnTagsChanged(object data)
  {
    ...
    {
      Game.Instance.circuitManager.Connect((IEnergyConsumer) this);
      this.GetComponent<KSelectable>().SetStatusItem(Db.Get().StatusItemCategories.Power, Db.Get().BuildingStatusItems.BatteryJoulesAvailable, (object) this);
    }
    else
    {
      Game.Instance.circuitManager.Disconnect((IEnergyConsumer) this, false);
      this.GetComponent<KSelectable>().RemoveStatusItem(Db.Get().BuildingStatusItems.BatteryJoulesAvailable);
    }
  }
  ...
  public List<Descriptor> GetDescriptors(GameObject go)
  {
    ...
    {
      descriptors.Add(new Descriptor((string) UI.BUILDINGEFFECTS.REQUIRESPOWERGENERATOR, (string) UI.BUILDINGEFFECTS.TOOLTIPS.REQUIRESPOWERGENERATOR, Descriptor.DescriptorType.Requirement));
      descriptors.Add(new Descriptor(string.Format((string) UI.BUILDINGEFFECTS.BATTERYCAPACITY, (object) GameUtil.GetFormattedJoules(this.capacity, "")), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.BATTERYCAPACITY, (object) GameUtil.GetFormattedJoules(this.capacity, ""))));
    ...
    {
      descriptors.Add(new Descriptor((string) UI.BUILDINGEFFECTS.TRANSFORMER_INPUT_WIRE, (string) UI.BUILDINGEFFECTS.TOOLTIPS.TRANSFORMER_INPUT_WIRE, Descriptor.DescriptorType.Requirement));
