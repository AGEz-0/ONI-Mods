...
public class BuildMenuCategoriesScreen : KIconToggleMenu
{
  ...
  public override float GetSortKey() => 7f;
    ...
    BuildMenuBuildingsScreen buildings_screen)
  {
    ...
    else if (typeof (IList<BuildMenu.DisplayInfo>).IsAssignableFrom(data.GetType()))
    {
      ...
      foreach (BuildMenu.DisplayInfo displayInfo in (IEnumerable<BuildMenu.DisplayInfo>) data)
      {
        ...
        string text = (string) Strings.Get($"STRINGS.UI.NEWBUILDCATEGORIES.{str}.NAME");
        ...
        int hotkey = (int) displayInfo.hotkey;
        string tooltip = (string) Strings.Get($"STRINGS.UI.NEWBUILDCATEGORIES.{str}.TOOLTIP");
        KIconToggleMenu.ToggleInfo toggleInfo2 = new KIconToggleMenu.ToggleInfo(text, icon, (object) user_data, (Action) hotkey, tooltip);
  ...
  public override void SetHasFocus(bool has_focus)
  {
    ...
    if (!((UnityEngine.Object) this.focusIndicator != (UnityEngine.Object) null))
      return;
    this.focusIndicator.color = (Color) (has_focus ? this.focusedColour : this.unfocusedColour);
  }
