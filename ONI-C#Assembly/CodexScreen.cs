...
public class CodexScreen : KScreen
{
  ...
  protected override void OnActivate()
  {
    ...
    this.searchInputField.OnValueChangesPaused = (System.Action) (() => this.FilterSearch(this.searchInputField.text));
    KInputTextField searchInputField = this.searchInputField;
    searchInputField.onFocus = searchInputField.onFocus + (System.Action) (() => this.editingSearch = true);
      ...
      for (int index2 = 0; index2 < codexEntry.subEntries[index1].contentContainers.Count; ++index2)
      {
        for (int index3 = 0; index3 < codexEntry.subEntries[index1].contentContainers[index2].content.Count; ++index3)
        {
          if (codexEntry.subEntries[index1].contentContainers[index2].content[index3] is CodexText codexText && codexText.messageID == (string) MISC.NOTIFICATIONS.BASICCONTROLS.NAME)
          {
            ...
            if (!string.IsNullOrEmpty(this.activeEntryID))
              this.ChangeArticle("MISCELLANEOUSTIPS0");
          }
        }
      }
    }
  ...
  private HashSet<CodexEntry> FilterSearch(string input)
  {
    ...
    if (string.IsNullOrEmpty(input))
    {
      ...
      {
        if (Game.IsCorrectDlcActiveForCurrentSave((IHasDlcRestrictions) entry.Value) && !entry.Value.searchOnly)
    ...
    else
    {
      foreach (KeyValuePair<string, CodexEntry> entry in CodexCache.entries)
      {
        if (Game.IsCorrectDlcActiveForCurrentSave((IHasDlcRestrictions) entry.Value))
        {
          try
          {
            if (SearchUtil.IsPassingScore(FuzzySearch.CanonicalizeAndScore(input, entry.Value.name).score))
              this.searchResults.Add(entry.Value);
            bool flag = false;
            if (!flag && entry.Value.title != null && SearchUtil.IsPassingScore(FuzzySearch.CanonicalizeAndScore(input, (string) Strings.Get(entry.Value.title)).score))
            {
              this.subEntrySearchResults.UnionWith((IEnumerable<SubEntry>) entry.Value.subEntries);
              flag = true;
            }
            if (!flag && entry.Value.category != null && SearchUtil.IsPassingScore(FuzzySearch.CanonicalizeAndScore(input, entry.Value.category).score))
            {
              this.subEntrySearchResults.UnionWith((IEnumerable<SubEntry>) entry.Value.subEntries);
              flag = true;
            }
  ...
  private void CategorizeEntries()
  {
    ...
    CodexScreen.SetupCategory(categories, "CREATURES::GeneralInfo");
    ...
    CodexScreen.HistoryDirection historyMovement = CodexScreen.HistoryDirection.NewArticle)
  {
    ...
    for (int index3 = 0; index3 < CodexCache.entries[id].contentContainers.Count; ++index3)
    {
      ...
      {
        GameObject gameObject5 = this.contentContainerPool.GetFreeElement(this.contentContainers.gameObject, true).gameObject;
        ...
        if (contentContainer.content != null)
        {
          foreach (ICodexWidget codexWidget2 in contentContainer.content)
          {
            ...
            if (codexWidget2 == codexWidget1)
              targetWidgetTransform = gameObject6.rectTransform();
          }
        }
      }
    }
  ...
  {
    if (target == null || (UnityEngine.Object) target.go == (UnityEngine.Object) null)
      return;
    ...
    if (this.scrollToTargetRoutine != null)
      this.StopCoroutine(this.scrollToTargetRoutine);
    this.scrollToTargetRoutine = this.StartCoroutine(this.ScrollToTarget(child));
  }
