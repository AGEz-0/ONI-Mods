  ...
  GameStateMachine<StressBehaviourMonitor, StressBehaviourMonitor.Instance>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.stressed.DefaultState((GameStateMachine<StressBehaviourMonitor, StressBehaviourMonitor.Instance, IStateMachineTarget, object>.State) this.stressed.tierOne).ToggleExpression(Db.Get().Expressions.Unhappy).ToggleAnims((Func<StressBehaviourMonitor.Instance, HashedString>) (smi => (HashedString) smi.tierOneLocoAnim)).Transition(this.satisfied, (StateMachine<StressBehaviourMonitor, StressBehaviourMonitor.Instance, IStateMachineTarget, object>.Transition.ConditionCallback) (smi => smi.gameObject.GetSMI<StressMonitor.Instance>() != null && !smi.gameObject.GetSMI<StressMonitor.Instance>().IsStressed()));
    ...
    GameStateMachine<StressBehaviourMonitor, StressBehaviourMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
    ...
    {
      double num = (double) this.sm.timeInTierTwoStressResponse.Set(timerValue, this);
    }
  }
}
