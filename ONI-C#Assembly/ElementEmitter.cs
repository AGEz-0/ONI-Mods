...
public class ElementEmitter : SimComponent
{
  ...
  protected override void OnSimActivate()
  {
    ...
    {
      float emit_temperature = (double) this.outputElement.minOutputTemperature == 0.0 ? this.GetComponent<PrimaryElement>().Temperature : this.outputElement.minOutputTemperature;
  ...
  protected override void OnSimDeactivate()
  {
    ...
    if (!this.showDescriptor)
      return;
    this.statusHandle = this.GetComponent<KSelectable>().RemoveStatusItem(this.statusHandle);
  }
  ...
  public void ForceEmit(float mass, byte disease_idx, int disease_count, float temperature = -1f)
  {
    ...
    if (elementByHash.IsGas || elementByHash.IsLiquid)
      SimMessages.AddRemoveSubstance(Grid.PosToCell(this.transform.GetPosition()), this.outputElement.elementHash, CellEventLogger.Instance.ElementConsumerSimUpdate, mass, temperature1, disease_idx, disease_count);
    else if (elementByHash.IsSolid)
      elementByHash.substance.SpawnResource(this.transform.GetPosition() + new Vector3(0.0f, 0.5f, 0.0f), mass, temperature1, disease_idx, disease_count, forceTemperature: true);
    PopFXManager.Instance.SpawnFX(PopFXManager.Instance.sprite_Resource, ElementLoader.FindElementByHash(this.outputElement.elementHash).name, this.gameObject.transform);
  }
