...
public class UnstableGroundManager : KMonoBehaviour
{
  ...
  protected override void OnPrefabInit()
  {
    ...
    foreach (UnstableGroundManager.EffectInfo effect in this.effects)
    {
      ...
      GameObjectPool pool = new GameObjectPool((Func<GameObject>) (() => this.InstantiateObj(prefab)), 16 /*0x10*/);
    ...
    int disease_count)
  {
    ...
    if (substance == null || substance.fallingStartSound.IsNull || !CameraController.Instance.IsAudibleSound(posCcc, substance.fallingStartSound))
      return;
    ...
    while (index1 < this.fallingObjects.Count)
    {
      ...
      if (!((UnityEngine.Object) fallingObject == (UnityEngine.Object) null))
      {
        ...
        if (!Grid.IsValidCell(index2) || Grid.Element[index2].IsSolid || ((int) Grid.Properties[index2] & 4) != 0)
        {
          ...
          if (elementByHash != null && elementByHash.substance != null && !elementByHash.substance.fallingStopSound.IsNull && CameraController.Instance.IsAudibleSound(position, elementByHash.substance.fallingStopSound))
  ...
  private void OnSerializing()
  {
    ...
    foreach (GameObject fallingObject in this.fallingObjects)
    {
      ...
      int hashValue = diseaseIdx != byte.MaxValue ? Db.Get().Diseases[(int) diseaseIdx].id.HashValue : 0;
