...
public class BaseUtilityBuildTool : DragTool
{
  ...
  {
    go.GetComponent<KBatchedAnimController>().Play((HashedString) anim);
  }
  ...
  private IEnumerator VisUpdater()
  {
    while (true)
    {
      ...
      for (int index = 0; index < this.path.Count; ++index)
      {
        ...
        string str = this.conduitMgr.GetVisualizerString(visualizer.cell) + "_place";
        ...
        component.TintColour = (Color32) (this.def.IsValidBuildLocation((GameObject) null, visualizer.cell, Orientation.Neutral, false, out string _) ? Color.white : Color.red);
    ...
    for (int index = 0; index < this.path.Count; ++index)
    {
      ...
      if ((UnityEngine.Object) go == (UnityEngine.Object) null)
      {
        ...
        if ((DebugHandler.InstantBuildMode || Game.Instance.SandboxModeActive && SandboxToolParameterMenu.instance.settings.InstantBuild) && this.def.IsValidBuildLocation(this.visualizer, posCbc, Orientation.Neutral) && this.def.IsValidPlaceLocation(this.visualizer, posCbc, Orientation.Neutral, out string _))
        {
          ...
          double temperature = (double) Mathf.Min(this.def.Temperature, b);
          float time = GameClock.Instance.GetTime();
          string facadeId = this.facadeID;
          double timeBuilt = (double) time;
          go = def.Build(cell, Orientation.Neutral, (Storage) null, selectedElements, (float) temperature, facadeId, timeBuilt: (float) timeBuilt);
        }
        else
        {
          go = this.def.TryPlace((GameObject) null, posCbc, Orientation.Neutral, this.selectedElements, this.facadeID);
          if ((UnityEngine.Object) go != (UnityEngine.Object) null)
          {
  ...
  protected struct PathNode
  {
    ...
    {
      this.visualizer.GetComponent<KBatchedAnimController>().Play((HashedString) anim);
    }
  }
}
