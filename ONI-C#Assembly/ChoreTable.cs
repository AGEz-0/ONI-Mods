...
public class ChoreTable
{
  ...
  public class Builder
  {
    ...
    public bool HasChoreType(System.Type choreType)
    {
      return this.infos.Exists((Predicate<ChoreTable.Builder.Info>) (info => info.def.GetType() == choreType));
    }
    ...
    public bool TryGetChoreDef<T>(out T def) where T : StateMachine.BaseDef
    {
      ...
      {
        if (this.infos[index].def != null && typeof (T).IsAssignableFrom(this.infos[index].def.GetType()))
        {
          def = (T) this.infos[index].def;
          return true;
        }
      }
      def = default (T);
    ...
    where StateMachineInstanceType : StateMachine.Instance
  {
    public ChoreTableChore(
      ...
      KPrefabID prefab_id)
      : base(chore_type, (IStateMachineTarget) prefab_id, prefab_id.GetComponent<ChoreProvider>())
    {
