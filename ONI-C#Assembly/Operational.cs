...
public class Operational : KMonoBehaviour
{
  ...
  {
    return type == Operational.Flag.Type.Functional ? this.IsFunctional : this.IsOperational;
  }
  ...
  private void UpdateOperational()
  {
    ...
    if (this.IsOperational)
      this.GetComponent<KPrefabID>().AddTag(GameTags.Operational);
    else
  ...
  {
    return this.uptimeData.Count > 0 ? this.uptimeData[this.uptimeData.Count - 1] : 0.0f;
  }
