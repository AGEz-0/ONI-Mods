...
public class SweepBotConfig : IEntityConfig
{
  ...
  public GameObject CreatePrefab()
  {
    ...
    placedEntity.AddOrGetDef<DrinkMilkMonitor.Def>().consumesMilk = false;
    ...
    navigator.maxProbingRadius = 32 /*0x20*/;
    navigator.sceneLayer = Grid.SceneLayer.Creatures;
    kprefabId.AddTag(GameTags.Creatures.Walker);
    ChoreTable.Builder chore_table = new ChoreTable.Builder().Add((StateMachine.BaseDef) new FallStates.Def()).Add((StateMachine.BaseDef) new AnimInterruptStates.Def()).Add((StateMachine.BaseDef) new SweepBotTrappedStates.Def()).Add((StateMachine.BaseDef) new DeliverToSweepLockerStates.Def()).Add((StateMachine.BaseDef) new ReturnToChargeStationStates.Def()).PushInterruptGroup().Add((StateMachine.BaseDef) new DrinkMilkStates.Def()
    {
      shouldBeBehindMilkTank = true
    }).PopInterruptGroup().Add((StateMachine.BaseDef) new SweepStates.Def()).Add((StateMachine.BaseDef) new IdleStates.Def());
    placedEntity.AddOrGet<LoopingSounds>();
