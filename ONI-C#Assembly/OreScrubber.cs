...
public class OreScrubber : StateMachineComponent<OreScrubber.SMInstance>, IGameObjectEffectDescriptor
{
  ...
  private class ScrubOreReactable(
    Workable workable,
    ChoreType chore_type,
    WorkableReactable.AllowedDirection allowed_direction = WorkableReactable.AllowedDirection.Any) : 
    WorkableReactable(workable, (HashedString) "ScrubOre", chore_type, allowed_direction)
  {
  ...
  public class SMInstance(OreScrubber master) : 
    GameStateMachine<OreScrubber.States, OreScrubber.SMInstance, OreScrubber, object>.GameInstance(master)
  {
    ...
    public Dictionary<Tag, float> GetNeededMass()
    {
      ...
      {
        {
          this.master.consumedElement.CreateTag(),
          this.master.massConsumedPerUse
        }
      };
    }
