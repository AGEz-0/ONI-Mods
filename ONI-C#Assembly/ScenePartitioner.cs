...
public class ScenePartitioner : ISim1000ms
{
  ...
  public ScenePartitioner(int node_size, int layer_count, int scene_width, int scene_height)
  {
    ...
    for (int index1 = 0; index1 < this.nodes.GetLength(0); ++index1)
    {
      for (int index2 = 0; index2 < this.nodes.GetLength(1); ++index2)
      {
        ...
        {
          this.nodes[index1, index2, index3].entries = new List<ScenePartitionerEntry>();
          this.nodes[index1, index2, index3].entries_set = new HashSet<ScenePartitionerEntry>();
        }
      }
    }
    ...
    for (int index1 = 0; index1 < this.nodes.GetLength(0); ++index1)
    {
      for (int index2 = 0; index2 < this.nodes.GetLength(1); ++index2)
      {
        for (int index3 = 0; index3 < this.nodes.GetLength(2); ++index3)
        {
          ...
          foreach (ScenePartitionerEntry entries in this.nodes[index1, index2, index3].entries_set)
          {
            if (entries != null)
            {
              entries.partitioner = (ScenePartitioner) null;
              entries.obj = (object) null;
            }
          }
          this.nodes[index1, index2, index3].entries.Clear();
          this.nodes[index1, index2, index3].entries_set.Clear();
        }
      }
    }
    ...
    else
    {
      ...
      for (int y = nodeExtents.y; y < nodeExtents.y + nodeExtents.height; ++y)
      {
        for (int x = nodeExtents.x; x < nodeExtents.x + nodeExtents.width; ++x)
        {
          ...
          if (this.nodes[layer, y, x].entries_set.Add(entry))
  ...
  private void Widthdraw(ScenePartitionerEntry entry)
  {
    ...
    for (int y = nodeExtents.y; y < nodeExtents.y + nodeExtents.height; ++y)
    {
      for (int x = nodeExtents.x; x < nodeExtents.x + nodeExtents.width; ++x)
      {
        if (this.nodes[layer, y, x].entries_set.Remove(entry))
        {
          this.nodes[layer, y, x].entries.Remove(entry);
          if (!this.nodes[layer, y, x].dirty)
          {
            this.nodes[layer, y, x].dirty = true;
            this.dirtyNodes.Add(new ScenePartitioner.DirtyNode()
  ...
  public void Sim1000ms(float dt)
  {
    foreach (ScenePartitioner.DirtyNode dirtyNode in this.dirtyNodes)
    {
      ...
      {
        if (ScenePartitioner.removeCallback(entries[index]))
    ...
    int query_id)
  {
    ...
    for (int index1 = num2; index1 < num1; ++index1)
    {
      for (int index2 = num3; index2 < num4; ++index2)
      {
        ...
        this.nodes[layer1, index1, index2].entries_set.ExceptWith((IEnumerable<ScenePartitionerEntry>) other);
        List<ScenePartitionerEntry> entries = this.nodes[layer1, index1, index2].entries;
        foreach (ScenePartitionerEntry partitionerEntry in (List<ScenePartitionerEntry>) other)
          entries.Remove(partitionerEntry);
        other.Recycle();
      }
    }
  }

  public IEnumerable<object> AsyncSafeEnumerate(
    int x,
    int y,
    int width,
    int height,
    ScenePartitionerLayer layer)
  {
    ...
    for (int node_y = num; node_y < max_y; ++node_y)
    {
      for (int node_x = start_x; node_x < max_x; ++node_x)
      {
        foreach (ScenePartitionerEntry entry in this.nodes[layer_idx, node_y, node_x].entries)
        {
          if (entry != null && entry.obj != null && x + width - 1 >= entry.x && x <= entry.x + entry.width - 1 && y + height - 1 >= entry.y && y <= entry.y + entry.height - 1)
            yield return entry.obj;
        }
      }
    }
  }

  public void AsyncSafeVisit<ContextType>(
    int x,
    int y,
    int width,
    int height,
    ScenePartitionerLayer layer,
    Func<object, ContextType, bool> visitor,
    ContextType context)
  {
    Extents nodeExtents = this.GetNodeExtents(x, y, width, height);
    int num1 = Math.Min(nodeExtents.y + nodeExtents.height, this.nodes.GetLength(1));
    ...
    for (int index1 = num2; index1 < num1; ++index1)
    {
      for (int index2 = num3; index2 < num4; ++index2)
      {
        foreach (ScenePartitionerEntry entry in this.nodes[layer1, index1, index2].entries)
        {
          if (entry != null && entry.obj != null && x + width - 1 >= entry.x && x <= entry.x + entry.width - 1 && y + height - 1 >= entry.y && y <= entry.y + entry.height - 1 && !visitor(entry.obj, context))
            return;
        }
      }
    }
