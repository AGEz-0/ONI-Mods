...
public class Diggable : Workable
{
  ...
  private void OnSolidChanged(object data)
  {
    ...
    else if (Grid.Element[this.cached_cell].hardness >= (byte) 251)
    {
      bool flag = false;
    ...
    else if (Grid.Element[this.cached_cell].hardness >= (byte) 200)
    {
      bool flag = false;
      foreach (Chore.PreconditionInstance precondition in this.chore.GetPreconditions())
      {
    ...
    else if (Grid.Element[this.cached_cell].hardness >= (byte) 150)
    {
      bool flag = false;
      foreach (Chore.PreconditionInstance precondition in this.chore.GetPreconditions())
      {
    ...
    else if (Grid.Element[this.cached_cell].hardness >= (byte) 50)
    {
      bool flag = false;
      foreach (Chore.PreconditionInstance precondition in this.chore.GetPreconditions())
      {
    ...
    {
      this.requiredSkillPerk = (string) null;
      this.chore.GetPreconditions().Remove(this.chore.GetPreconditions().Find((Predicate<Chore.PreconditionInstance>) (o => o.condition.id == ChorePreconditions.instance.HasSkillPerk.id)));
    }
  ...
  public Element GetTargetElement() => Grid.Element[this.cached_cell];
