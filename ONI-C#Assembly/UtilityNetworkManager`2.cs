  ...
  where ItemType : MonoBehaviour
{
  ...
  public void ClearCell(int cell, bool is_physical_building)
  {
    ...
    if (xy.x > 0 && (connections & UtilityConnections.Left) != (UtilityConnections) 0)
    ...
    if (xy.y > 0 && (connections & UtilityConnections.Down) != (UtilityConnections) 0)
  ...
  public void AddToNetworks(int cell, object item, bool is_endpoint)
  {
    if (item != null)
    {
      if (is_endpoint)
      {
        ...
        {
          Debug.LogWarning((object) $"Cell {cell} already has a utility network endpoint assigned. Adding {item.ToString()} will stomp previous endpoint, destroying the object that's already there.");
          KMonoBehaviour endpoint = this.endpoints[cell] as KMonoBehaviour;
          if ((UnityEngine.Object) endpoint != (UnityEngine.Object) null)
      ...
      else
      {
        ...
        {
          Debug.LogWarning((object) $"Cell {cell} already has a utility network connector assigned. Adding {item.ToString()} will stomp previous item, destroying the object that's already there.");
          KMonoBehaviour original = this.items[cell] as KMonoBehaviour;
          if ((UnityEngine.Object) original != (UnityEngine.Object) null)
            Util.KDestroyGameObject((Component) original);
        }
  ...
  private UtilityConnections GetNeighboursAsConnections(int cell, HashSet<int> nodes)
  {
    ...
    Vector2I xy = Grid.CellToXY(cell);
    if (xy.x > 0 && nodes.Contains(Grid.CellLeft(cell)))
    ...
    if (xy.y > 0 && nodes.Contains(Grid.CellBelow(cell)))
