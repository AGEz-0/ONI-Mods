    ...
    for (int idx = 0; idx < Components.BuildingCompletes.Count; ++idx)
    {
      ...
      if (!_excludeEntities.Contains(buildingComplete.gameObject))
      {
        ...
        if (this.SaveAllBuildings || this.SelectedCells.Contains(cell))
        {
          ...
          {
            foreach (GameObject go in component8.items)
            {
              float _units = 0.0f;
              ...
              bool _isOre = false;
              PrimaryElement component9 = go.GetComponent<PrimaryElement>();
              if ((UnityEngine.Object) component9 != (UnityEngine.Object) null)
              {
                _units = component9.Units;
                _element2 = component9.ElementID;
                _temp = component9.Temperature;
                _disease2 = component9.DiseaseIdx != byte.MaxValue ? Db.Get().Diseases[(int) component9.DiseaseIdx].Id : (string) null;
                _disease_count2 = component9.DiseaseCount;
              }
