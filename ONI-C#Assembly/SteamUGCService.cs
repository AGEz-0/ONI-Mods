...
public class SteamUGCService : MonoBehaviour
{
  ...
  public void Awake()
  {
    ...
    int subscribedItems = (int) SteamUGC.GetSubscribedItems(publishedFileIdTArray, numSubscribedItems);
    this.downloads.UnionWith((IEnumerable<PublishedFileId_t>) publishedFileIdTArray);
  }

  public bool IsSubscribed(PublishedFileId_t item)
  {
    return this.downloads.Contains(item) || this.proxies.Contains(item) || this.queries.Contains(item) || this.publishes.Any<SteamUGCDetails_t>((Func<SteamUGCDetails_t, bool>) (candidate => candidate.m_nPublishedFileId == item)) || this.mods.Exists((Predicate<SteamUGCService.Mod>) (candidate => candidate.fileId == item));
  }
  ...
  private void Update()
  {
    ...
    this.previews.RemoveWhere((Predicate<SteamUGCDetails_t>) (publish => loaded_previews.Any<SteamUGCService.Mod>((Func<SteamUGCService.Mod, bool>) (mod => mod.fileId == publish.m_nPublishedFileId)) || cancelled_previews.Contains(publish.m_nPublishedFileId)));
    ...
    {
      if (((int) SteamUGC.GetItemState(publish.m_nPublishedFileId) & 48 /*0x30*/) == 0)
      {
        Debug.LogFormat("Steam: updating info for mod {0}", (object) publish.m_rgchTitle);
    ...
    {
      Debug.LogFormat("Steam: proxy mod {0}", (object) proxy);
      pooledList.Add(new SteamUGCService.Mod(proxy));
    }
    this.proxies.Clear();
    ListPool<PublishedFileId_t, SteamUGCService>.PooledList added = ListPool<PublishedFileId_t, SteamUGCService>.Allocate();
    ...
    foreach (SteamUGCService.Mod mod1 in (List<SteamUGCService.Mod>) pooledList)
    {
      ...
      {
        this.mods[index] = mod;
        updated.Add(mod.fileId);
      }
    }
    pooledList.Recycle();
    bool flag = this.details_query == UGCQueryHandle_t.Invalid;
    if (added.Count != 0 || updated.Count != 0 || flag && this.removals.Count != 0 || loaded_previews.Count != 0)
    {
    ...
    foreach (PublishedFileId_t download in this.downloads)
    {
      ...
      if (((itemState & EItemState.k_EItemStateInstalled) == EItemState.k_EItemStateNone || (itemState & EItemState.k_EItemStateNeedsUpdate) != EItemState.k_EItemStateNone) && (itemState & (EItemState.k_EItemStateDownloading | EItemState.k_EItemStateDownloadPending)) == EItemState.k_EItemStateNone && SteamUGC.DownloadItem(download, false))
        this.awaiting_download.Add(download);
    }
    if (!(this.details_query == UGCQueryHandle_t.Invalid))
      return;
    this.queries.UnionWith((IEnumerable<PublishedFileId_t>) this.downloads);
    this.queries.ExceptWith((IEnumerable<PublishedFileId_t>) this.awaiting_download);
    ...
    this.on_query_completed.Set(SteamUGC.SendQueryUGCRequest(this.details_query));
  }
    ...
    switch (pCallback.m_eResult)
    {
      case EResult.k_EResultOK:
        for (uint index = 0; index < pCallback.m_unNumResultsReturned; ++index)
        {
          ...
          if (SteamUGC.GetQueryUGCResult(this.details_query, index, out pDetails))
          {
            ...
            KCrashReporter.ReportDevNotification("SteamUGCService.GetQueryUGCResult details_query is an invalid handle!", Environment.StackTrace);
        }
        break;
    ...
    bool getFirstMatch = false)
  {
    ...
    string pchFolder;
    SteamUGC.GetItemInstallInfo(item, out ulong _, out pchFolder, 1024U /*0x0400*/, out uint _);
    try
    {
  ...
  public class Mod
  {
    ...
    public Mod(SteamUGCDetails_t item, Texture2D previewImage)
    {
      ...
      this.tags = new List<string>((IEnumerable<string>) item.m_rgchTags.Split(',', StringSplitOptions.None));
