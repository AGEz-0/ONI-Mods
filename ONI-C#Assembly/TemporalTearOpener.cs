  ...
  GameStateMachine<TemporalTearOpener, TemporalTearOpener.Instance, IStateMachineTarget, TemporalTearOpener.Def>
{
  ...
  {
    return new StatusItem("Temporal_Tear_Opener_Insufficient_Colonies", "BUILDING", "", StatusItem.IconType.Info, NotificationType.Neutral, false, OverlayModes.None.ID, false)
    {
  ...
  {
    return new StatusItem("Temporal_Tear_Opener_Progress", "BUILDING", "", StatusItem.IconType.Info, NotificationType.Neutral, false, OverlayModes.None.ID, false)
    {
      resolveStringCallback = (Func<string, object, string>) ((str, data) =>
      {
        TemporalTearOpener.Instance instance = (TemporalTearOpener.Instance) data;
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.check_requirements.DefaultState(this.check_requirements.has_target).Enter((StateMachine<TemporalTearOpener, TemporalTearOpener.Instance, IStateMachineTarget, TemporalTearOpener.Def>.State.Callback) (smi =>
    {
      ...
      smi.GetComponent<KBatchedAnimController>().Play((HashedString) "port_close");
      smi.GetComponent<KBatchedAnimController>().Queue((HashedString) "off", KAnim.PlayMode.Loop);
    }));
    ...
    this.charging.DefaultState(this.charging.idle).ToggleStatusItem(TemporalTearOpener.s_progressStatus, (Func<TemporalTearOpener.Instance, object>) (smi => (object) smi)).UpdateTransition(this.check_requirements.has_los, (Func<TemporalTearOpener.Instance, float, bool>) ((smi, dt) => !smi.HasLineOfSight())).UpdateTransition(this.check_requirements.enough_colonies, (Func<TemporalTearOpener.Instance, float, bool>) ((smi, dt) => !smi.HasSufficientColonies())).Enter((StateMachine<TemporalTearOpener, TemporalTearOpener.Instance, IStateMachineTarget, TemporalTearOpener.Def>.State.Callback) (smi =>
    {
      ...
      smi.GetComponent<KBatchedAnimController>().Play((HashedString) "port_open");
      smi.GetComponent<KBatchedAnimController>().Queue((HashedString) "inert", KAnim.PlayMode.Loop);
    }));
    ...
    this.opening_tear_beam.Enter((StateMachine<TemporalTearOpener, TemporalTearOpener.Instance, IStateMachineTarget, TemporalTearOpener.Def>.State.Callback) (smi => smi.CreateBeamFX())).PlayAnim("working_loop", KAnim.PlayMode.Loop).ScheduleGoTo(5f, (StateMachine.BaseState) this.opening_tear_finish);
    ...
    ISidescreenButtonControl
  {
    ...
    {
      if ((UnityEngine.Object) EnterTemporalTearSequence.tearOpenerGameObject == (UnityEngine.Object) this.gameObject)
    ...
    public int CountColonies()
    {
      ...
      for (int idx = 0; idx < Components.Telepads.Count; ++idx)
      {
        ...
        if ((UnityEngine.Object) component == (UnityEngine.Object) null || component.IsActivated)
          ++num;
      }
      return num;
    }
    ...
    {
      return Mathf.Min(this.m_particlesConsumed / this.def.numParticlesToOpen, 1f);
    }
    ...
    {
      get => (string) BUILDINGS.PREFABS.TEMPORALTEAROPENER.SIDESCREEN.TEXT;
    }

    public string SidescreenButtonTooltip
    {
      get => (string) BUILDINGS.PREFABS.TEMPORALTEAROPENER.SIDESCREEN.TOOLTIP;
    }

    public bool SidescreenEnabled()
    {
      return this.GetCurrentState() == this.sm.ready || DebugHandler.InstantBuildMode;
    }
    ...
    {
      ConfirmDialogScreen component = GameScreenManager.Instance.StartScreen(ScreenPrefabs.Instance.ConfirmDialogScreen.gameObject).GetComponent<ConfirmDialogScreen>();
    ...
    {
      smi.GoTo((StateMachine.BaseState) this.sm.opening_tear_beam_pre);
    }
