...
public class LightBuffer : MonoBehaviour
{
  ...
  private void LateUpdate()
  {
    ...
    {
      if (!((UnityEngine.Object) light2D == (UnityEngine.Object) null) && visibleAreaExtended.Contains(light2D.cachedCell))
      {
        Vector3 position = light2D.transform.GetPosition();
        int cell = Grid.PosToCell(position);
        int num;
        pooledDictionary.TryGetValue(cell, out num);
        if (num < this.maxLights)
        {
          pooledDictionary[cell] = num + 1;
