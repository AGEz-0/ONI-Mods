...
public class ReportScreen : KScreen
{
  ...
  private GameObject AddSpacer(int group)
  {
    ...
    {
      gameObject = Util.KInstantiateUI(this.lineItemSpacer, this.contentFolder);
  ...
  private GameObject CreateHeader(ReportManager.ReportGroup reportGroup)
  {
    ...
    if ((UnityEngine.Object) header == (UnityEngine.Object) null)
    {
      ...
      header.name = "LineItemHeader" + this.lineItems.Count.ToString();
    ...
    bool is_line_active)
  {
    ...
    if (!is_line_active)
    {
      if ((UnityEngine.Object) orUpdateLine != (UnityEngine.Object) null && orUpdateLine.activeSelf)
        orUpdateLine.SetActive(false);
    }
    else
    {
      if ((UnityEngine.Object) orUpdateLine == (UnityEngine.Object) null)
      {
        orUpdateLine = Util.KInstantiateUI(this.lineItem, this.contentFolder, true);
        orUpdateLine.name = "LineItem" + this.lineItems.Count.ToString();
  ...
  {
    this.PlaySound3D(GlobalAssets.GetSound("HUD_Click_Close"));
    this.Show(false);
  }
}
