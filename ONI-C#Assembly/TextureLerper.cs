...
public class TextureLerper
{
    ...
    TextureFormat texture_format)
  {
    ...
    this.textureCam.rect = new UnityEngine.Rect(0.0f, 0.0f, 1f, 1f);
  ...
  private Vector4 GetVisibleCellRange()
  {
    ...
    double x_offset = (double) num;
    double y_offset = (double) num;
    double z_offset = (double) num;
    Vector3 pos = Grid.CellToPos(cell, (float) x_offset, (float) y_offset, (float) z_offset);
    int x = Math.Max(0, (int) ((double) pos.x / (double) cellSizeInMeters));
    int y = Math.Max(0, (int) ((double) pos.y / (double) cellSizeInMeters));
    ...
    int z = Mathf.Min(a1, Grid.WidthInCells - 1);
    int w = Mathf.Min(a2, Grid.HeightInCells - 1);
