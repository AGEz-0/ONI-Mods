...
public class DupeGreetingManager : KMonoBehaviour, ISim200ms
{
  ...
  {
    return !minion.GetComponent<Facing>().GetFacing() ? Grid.OffsetCell(Grid.PosToCell((KMonoBehaviour) minion), offset, 0) : Grid.OffsetCell(Grid.PosToCell((KMonoBehaviour) minion), -offset, 0);
  }
  ...
  private Reactable GetReactable(MinionIdentity minion)
  {
    ...
    SelfEmoteReactable reactable = new SelfEmoteReactable(minion.gameObject, (HashedString) "NavigatorPassingGreeting", Db.Get().ChoreTypes.Emote, 1000f);
    reactable.SetEmote(emote).SetThought(Db.Get().Thoughts.Chatty);
    reactable.RegisterEmoteStepCallbacks((HashedString) "react", new Action<GameObject>(this.BeginReacting), (Action<GameObject>) null);
