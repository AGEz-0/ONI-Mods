...
public class ReportManager : KMonoBehaviour
{
  ...
  public ReportManager.DailyReport YesterdaysReport
  {
    get
    {
      return this.dailyReports.Count <= 1 ? (ReportManager.DailyReport) null : this.dailyReports[this.dailyReports.Count - 1];
    }
  }
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.Subscribe(Game.Instance.gameObject, -1917495436, new Action<object>(this.OnSaveGameReady));
    this.noteStorage = new ReportManager.NoteStorage();
  }

  protected override void OnCleanUp() => ReportManager.Instance = (ReportManager) null;
  ...
  {
    this.Subscribe(GameClock.Instance.gameObject, -722330267, new Action<object>(this.OnNightTime));
  ...
  private void OnNightTime(object data)
  {
    ...
    ManagementMenuNotification menuNotification = new ManagementMenuNotification(Action.ManageReport, NotificationValence.Good, (string) null, string.Format((string) UI.ENDOFDAYREPORT.NOTIFICATION_TITLE, (object) day), NotificationType.Good, (Func<List<Notification>, object, string>) ((n, d) => string.Format((string) UI.ENDOFDAYREPORT.NOTIFICATION_TOOLTIP, (object) day)), custom_click_callback: (Notification.ClickCallback) (d => ManagementMenu.Instance.OpenReports(day)));
  ...
  public class ReportEntry
  {
    ...
    public struct Note(float value, string note)
    {
      ...
      public void Serialize(BinaryWriter writer)
      {
        ...
        foreach (KeyValuePair<int, Dictionary<ReportManager.NoteStorage.NoteEntries.NoteEntryKey, float>> entry in this.entries)
        {
          ...
          foreach (KeyValuePair<ReportManager.NoteStorage.NoteEntries.NoteEntryKey, float> keyValuePair in entry.Value)
          {
            ...
            writer.WriteSingleFast(keyValuePair.Value);
          }
        }
      }
