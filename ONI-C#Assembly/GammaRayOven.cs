  ...
  protected override void OnSpawn()
  {
    ...
    this.workable.OnWorkTickActions += (Action<WorkerBase, float>) ((worker, dt) =>
    {
      ...
      if (this.diseaseCountKillRate <= 0)
        return;
      this.GetComponent<PrimaryElement>().ModifyDiseaseCount(-Math.Max(1, (int) ((double) this.diseaseCountKillRate * (double) dt)), nameof (GammaRayOven));
    });
    this.GetComponent<Radiator>().emitter.enabled = false;
    this.Subscribe(824508782, new Action<object>(this.UpdateRadiator));
  }

  private void UpdateRadiator(object data)
  {
    this.GetComponent<Radiator>().emitter.enabled = this.operational.IsActive;
  }
