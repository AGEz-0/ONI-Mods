...
{
  private static Func<int, CreatureHelpers.fleeThreatInfo, float> fleeCellRater = (Func<int, CreatureHelpers.fleeThreatInfo, float>) ((cell, threat) => (float) Grid.GetCellDistance(cell, threat.threatCell) + (CreatureHelpers.isInFavoredFleeDirection(cell, threat.threatCell, threat.selfCell) ? 2f : 0.0f));
  private static Func<int, CreatureHelpers.fleeThreatInfo, bool> fleeCellVaidator = (Func<int, CreatureHelpers.fleeThreatInfo, bool>) ((cell, info) => CreatureHelpers.CanFleeTo(cell, info.nav));
  ...
  public static GameObject GetFleeTargetLocatorObject(GameObject self, GameObject threat)
  {
    ...
    if ((UnityEngine.Object) fleeThreatInfo.nav == (UnityEngine.Object) null)
    {
      Debug.LogWarning((object) (self.name + " is trying to flee, bus has no navigator component attached."));
      return (GameObject) null;
    }
    int best = GameUtil.FloodFillFindBest<CreatureHelpers.fleeThreatInfo>(CreatureHelpers.fleeCellRater, fleeThreatInfo, CreatureHelpers.fleeCellVaidator, Grid.PosToCell(self), 300);
