...
public class RailGun : StateMachineComponent<RailGun.StatesInstance>, ISim200ms, ISecondaryInput
{
  ...
  public static readonly HashedString PORT_ID = (HashedString) "LogicLaunching";
  ...
  {
    this.resourceMeter = new MeterController((KAnimControllerBase) this.GetComponent<KBatchedAnimController>(), "meter_storage_target", "meter_storage", Meter.Offset.Infront, Grid.SceneLayer.NoLayer, Array.Empty<string>());
    this.particleMeter = new MeterController((KAnimControllerBase) this.GetComponent<KBatchedAnimController>(), "meter_orb_target", "meter_orb", Meter.Offset.Infront, Grid.SceneLayer.NoLayer, Array.Empty<string>());
  }
  ...
  {
    return Game.Instance.logicCircuitManager.GetNetworkForCell(this.GetComponent<LogicPorts>().GetPortCell(RailGun.PORT_ID));
  }
  ...
  private void CheckLogicWireState()
  {
    ...
    this.GetComponent<KSelectable>().ToggleStatusItem(RailGun.infoStatusItemLogic, network != null, (object) this);
  }
  ...
  private void LaunchProjectile()
  {
    ...
    GameObject go = Util.KInstantiate(Assets.GetPrefab((Tag) "RailGunPayload"), Grid.CellToPosCBC(cell, Grid.SceneLayer.Front));
    ...
    while ((double) num1 < (double) this.launchMass && (double) this.resourceStorage.MassStored() > 0.0)
      num1 += this.resourceStorage.Transfer(component, GameTags.Stored, this.launchMass - num1, hide_popups: true);
    ...
    smi.StartSM();
    smi.Launch(this.gameObject.GetMyWorldLocation(), this.destinationSelector.GetDestination());
  }
  ...
  public class StatesInstance(RailGun smi) : 
    GameStateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.GameInstance(smi)
  {
    ...
    {
      return (double) this.smi.master.resourceStorage.MassStored() >= (double) this.smi.master.launchMass;
    }
    ...
    {
      return this.smi.master.destinationSelector.GetDestinationWorld() != this.smi.master.GetMyWorldId();
    }
    ...
    {
      this.m_cachedPath = ClusterGrid.Instance.GetPath(this.gameObject.GetMyWorldLocation(), this.smi.master.destinationSelector.GetDestination(), this.smi.master.destinationSelector);
    }
  ...
  public class States : GameStateMachine<RailGun.States, RailGun.StatesInstance, RailGun>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.on.wait_for_storage.PlayAnim("on", KAnim.PlayMode.Loop).EventTransition(GameHashes.ClusterDestinationChanged, this.on.power_off, (StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Transition.ConditionCallback) (smi => !smi.HasEnergy())).EventTransition(GameHashes.OnStorageChange, (GameStateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.State) this.on.working, (StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Transition.ConditionCallback) (smi => smi.HasResources() && (double) smi.sm.cooldownTimer.Get(smi) <= 0.0)).EventTransition(GameHashes.OperationalChanged, (GameStateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.State) this.on.working, (StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Transition.ConditionCallback) (smi => smi.HasResources() && (double) smi.sm.cooldownTimer.Get(smi) <= 0.0)).EventTransition(GameHashes.RailGunLaunchMassChanged, (GameStateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.State) this.on.working, (StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Transition.ConditionCallback) (smi => smi.HasResources() && (double) smi.sm.cooldownTimer.Get(smi) <= 0.0)).ParamTransition<float>((StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Parameter<float>) this.cooldownTimer, (GameStateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.State) this.on.cooldown, (StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Parameter<float>.Callback) ((smi, p) => (double) p > 0.0));
      ...
      double num1;
      this.on.cooldown.loop.PlayAnim("cooldown_loop", KAnim.PlayMode.Loop).ParamTransition<float>((StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Parameter<float>) this.cooldownTimer, this.on.cooldown.pst, (StateMachine<RailGun.States, RailGun.StatesInstance, RailGun, object>.Parameter<float>.Callback) ((smi, p) => (double) p <= 0.0)).Update((Action<RailGun.StatesInstance, float>) ((smi, dt) => num1 = (double) this.cooldownTimer.Delta(-dt, smi)), UpdateRate.SIM_1000ms);
