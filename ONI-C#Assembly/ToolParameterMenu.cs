...
public class ToolParameterMenu : KMonoBehaviour
{
    ...
    Dictionary<string, ToolParameterMenu.ToggleState> parameters)
  {
    ...
    foreach (KeyValuePair<string, ToolParameterMenu.ToggleState> parameter in parameters)
    {
      ...
      gameObject.GetComponentInChildren<LocText>().text = (string) Strings.Get($"STRINGS.UI.TOOLS.FILTERLAYERS.{parameter.Key}.NAME");
      ToolTip componentInChildren = gameObject.GetComponentInChildren<ToolTip>();
      if ((UnityEngine.Object) componentInChildren != (UnityEngine.Object) null)
        componentInChildren.SetSimpleTooltip((string) Strings.Get($"STRINGS.UI.TOOLS.FILTERLAYERS.{parameter.Key}.TOOLTIP"));
