...
public class RocketClusterDestinationSelector : ClusterDestinationSelector
{
  ...
  protected override void OnClusterLocationChanged(object data)
  {
    ...
    if (!this.m_repeat)
      return;
    if ((!((UnityEngine.Object) ClusterGrid.Instance.GetVisibleEntityOfLayerAtCell(locationChangedEvent.newLocation, EntityLayer.POI) != (UnityEngine.Object) null) ? 0 : (this.CanRocketHarvest() ? 1 : 0)) != 0)
      this.WaitForPOIHarvest();
    else
  ...
  private void OnStorageChange(object data)
  {
    ...
    foreach (Ref<RocketModuleCluster> clusterModule in (IEnumerable<Ref<RocketModuleCluster>>) this.GetComponent<Clustercraft>().ModuleInterface.ClusterModules)
    {
