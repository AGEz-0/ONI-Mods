...
public class SolidConduitFlow : IConduitFlow
{
  ...
  private KCompactedVector<SolidConduitFlow.StoredInfo> conveyorPickupables = new KCompactedVector<SolidConduitFlow.StoredInfo>();
  ...
  {
    return 1 << (int) (direction - 1 & (SolidConduitFlow.FlowDirection) 31 /*0x1F*/);
  }
  ...
  [Serializable]
  public struct Conduit(int idx) : IEquatable<SolidConduitFlow.Conduit>
  {
    public int idx = idx;
  ...
  public struct ConduitContents(HandleVector<int>.Handle pickupable_handle)
  {
    public HandleVector<int>.Handle pickupableHandle = pickupable_handle;
