  ...
  GameStateMachine<SeedPlantingStates, SeedPlantingStates.Instance, IStateMachineTarget, SeedPlantingStates.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    string name = (string) CREATURES.STATUSITEMS.PLANTINGSEED.NAME;
    string tooltip = (string) CREATURES.STATUSITEMS.PLANTINGSEED.TOOLTIP;
    StatusItemCategory main = Db.Get().StatusItemCategories.Main;
    HashedString render_overlay = new HashedString();
    StatusItemCategory category = main;
    root.ToggleStatusItem(name, tooltip, render_overlay: render_overlay, category: category).Exit(new StateMachine<SeedPlantingStates, SeedPlantingStates.Instance, IStateMachineTarget, SeedPlantingStates.Def>.State.Callback(SeedPlantingStates.UnreserveSeed)).Exit(new StateMachine<SeedPlantingStates, SeedPlantingStates.Instance, IStateMachineTarget, SeedPlantingStates.Def>.State.Callback(SeedPlantingStates.DropAll)).Exit(new StateMachine<SeedPlantingStates, SeedPlantingStates.Instance, IStateMachineTarget, SeedPlantingStates.Def>.State.Callback(SeedPlantingStates.RemoveMouthOverride));
  ...
  private static void AddMouthOverride(SeedPlantingStates.Instance smi)
  {
    ...
    KAnim.Build.Symbol symbol = smi.GetComponent<KBatchedAnimController>().AnimFiles[0].GetData().build.GetSymbol((KAnimHashedString) (smi.def.prefix + "sq_mouth_cheeks"));
    if (symbol == null)
      return;
    component.AddSymbolOverride((HashedString) "sq_mouth", symbol, 1);
  }

  private static void RemoveMouthOverride(SeedPlantingStates.Instance smi)
  {
    smi.GetComponent<SymbolOverrideController>().TryRemoveSymbolOverride((HashedString) "sq_mouth", 1);
  }
  ...
  {
    PlantableSeed component = (bool) (UnityEngine.Object) smi.targetSeed ? smi.targetSeed.GetComponent<PlantableSeed>() : (PlantableSeed) null;
    PlantablePlot plot;
    if ((bool) (UnityEngine.Object) component && SeedPlantingStates.CheckValidPlotCell(smi, component, smi.targetDirtPlotCell, out plot))
    {
  ...
  private static void FindDirtPlot(SeedPlantingStates.Instance smi)
  {
    ...
    if (query.result_cells.Count <= 0)
      return;
    smi.targetDirtPlotCell = query.result_cells[UnityEngine.Random.Range(0, query.result_cells.Count)];
  }
  ...
  {
    GameObject gameObject = (bool) (UnityEngine.Object) smi.targetSeed ? smi.targetSeed.gameObject : (GameObject) null;
