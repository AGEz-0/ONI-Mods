...
public class KBatchedAnimController : KAnimControllerBase, KAnimConverter.IAnimConverter
{
  ...
  public override void UpdateHiddenSymbol(KAnimHashedString symbolToUpdate)
  {
    ...
    for (int index = 0; index < batchGroupData.frameElementSymbols.Count; ++index)
    {
      ...
      {
        bool is_visible = !this.hiddenSymbolsSet.Contains(batchGroupData.frameElementSymbols[index].hash);
        this.symbolInstanceGpuData.SetVisible(index, is_visible);
      }
    }
    this.SetDirty();
  }
  ...
  protected override void OnStart()
  {
    ...
    if (instance != null)
    {
      instance.RegisterMovementStateChanged(this.transform, new Action<Transform, bool>(KBatchedAnimController.OnMovementStateChanged));
  ...
  protected override void OnDestroy()
  {
    if (App.IsExiting)
      return;
    Singleton<CellChangeMonitor>.Instance?.UnregisterMovementStateChanged(this.transform, new Action<Transform, bool>(KBatchedAnimController.OnMovementStateChanged));
